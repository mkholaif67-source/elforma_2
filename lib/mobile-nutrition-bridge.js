...
function navyBodyFat(gender, heightCm, waistCm, neckCm, hipCm) {
  var h = Number(heightCm)/2.54, w = Number(waistCm)/2.54, nk = Number(neckCm)/2.54, hp = Number(hipCm)/2.54, bf;
...
function safeWeeklyRate(goal, weightKg, bodyFat, gainStyle) {
  var w = Number(weightKg) > 0 ? Number(weightKg) : 75;
  var bf = bodyFat == null ? NaN : Number(bodyFat), pct;
  if (goal === 'cut') {
    pct = 0.0075;
...
function buildEngineContext(mobileProfile, extras) {
  ...
  var x = Object.assign({}, p, extras && typeof extras === 'object' ? extras : {});

  var gender = p.gender === 'female' ? 'female' : 'male';
  // [FIX-GENDER-HAMZA] المحرك كله بيفحص بالصيغة المهموسة 'أنثى' (حد أدنى 1200
  // سعر للإناث، IBW base 45.5، منطق cut للإناث...). الجسر كان بيبعت 'انثى' بدون
  // همزة فكل الفحوصات دي ما كانتش بتشتغل لمستخدمات التطبيق (الموقع بيبعتها صح).
  var genderAr = gender === 'female' ? 'أنثى' : 'ذكر';
  ...
  // [FIX-ACTIVITY] Try all possible field names; default sedentary (conservative) not light
  var activity = ACTIVITY[p.dailyActivity] || ACTIVITY[p.activityLevel] || ACTIVITY[p.activity] || ACTIVITY.sedentary;
  ...
  // [FIX-ZERO-CARDIO] Number(0) is falsy so || skips explicit zero — use null-check instead
  var cardioSessions = (x.cardioSessions != null) ? Number(x.cardioSessions)
    : ((p.cardioSessions != null) ? Number(p.cardioSessions) : 0);
  ...
  var bodyFat = Number(x.bodyFat ?? p.bodyFat);
  if (!Number.isFinite(bodyFat) || bodyFat <= 0) bodyFat = navyBodyFat(gender, height, x.waist ?? p.waist, x.neck ?? p.neck, x.hips ?? p.hips);
  ...
  var profile = {
    ...
    goal: goal,
    gainStyle: gainStyle,
    coachTargetCals: Number(p.coachTargetCals) || undefined,
  ...
  return {
    ...
    derived: {
      ...
      bodyFat: bodyFat,
      bodyFatSource: Number(x.bodyFat ?? p.bodyFat) > 0 ? 'measured' : (bodyFat ? 'navy' : 'engine-estimate'),
