...
function acceptsGzip(req) {
  const entries=String((req.headers && req.headers['accept-encoding']) || '').toLowerCase().split(',');
  let wildcard=false;
  for(const entry of entries){
    const parts=entry.trim().split(';');
    const name=parts.shift().trim();
    const q=parts.map(p=>p.trim()).find(p=>p.startsWith('q='));
    const quality=q ? Number(q.slice(2)) : 1;
    if(name==='gzip')return Number.isFinite(quality)&&quality>0;
...
function enableCompression(req, res) {
  ...
  res.end = function (chunk, enc) {
    ...
    // Never encode a response already encoded by a handler or upstream layer.
    const encodingKey=Object.keys(head.headers).find(k=>k.toLowerCase()==='content-encoding');
    const encoded=encodingKey ? head.headers[encodingKey] : res.getHeader('Content-Encoding');
    if (encoded || buf.length < MIN_BYTES || !compressible(type)) {
