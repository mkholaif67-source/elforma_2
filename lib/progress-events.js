...
const q={
 ...
 nutrition:db.prepare('SELECT day,calories,protein,carbs,fat FROM nutrition_days WHERE user_id=? ORDER BY day ASC LIMIT ?'),
...
function period(v){const s=String(v||'');return /^\d{4}-\d{2}/.test(s)?s.slice(0,7):new Date().toISOString().slice(0,7)}
...
function sources(id){return {sessions:q.sessions.all(id,2000),sets:q.sets.all(id,20000),weights:q.weights.all(id,2000),nutrition:q.nutrition.all(id,1000)}}
...
function story(userId,month){const id=Number(userId),m=MONTH.test(String(month||''))?month:new Date().toISOString().slice(0,7);syncUser(id);const ev=q.events.all(id,m,100).map(decode),s=sources(id),weights=s.weights.filter(r=>period(r.day)===m),sessions=s.sessions.filter(r=>period(r.finished_at||r.started_at)===m),sets=s.sets.filter(r=>period(r.finished_at||r.started_at)===m),byType={};for(const e of ev)byType[e.type]=(byType[e.type]||0)+1;return {month:m,empty:!ev.length&&!weights.length&&!sessions.length,summary:{sessions:sessions.length,sets:sets.length,totalVolume:round(sets.reduce((n,r)=>n+(Number(r.weight)||0)*(Number(r.reps)||0),0)),weightStart:weights.length?Number(weights[0].weight):null,weightEnd:weights.length?Number(weights[weights.length-1].weight):null,weightChange:weights.length>1?round(Number(weights[weights.length-1].weight)-Number(weights[0].weight)):null,eventCount:ev.length,byType},events:ev,moments:ev.filter(e=>!e.seenAt).slice(0,5)}}
