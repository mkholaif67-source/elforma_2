...
function computeTargets(profile, inputs){
  ...
  r.calculation = energyTrace(profile, inputs, r);
...
function applyAgePolicy(profile, targets) {
  ...
  if (isFinite(out.targetCals) && isFinite(out.tdee) && out.tdee > 0) {
    ...
    if (_rq > 0 && isCut) {
      ...
      out.rateHonored = Math.abs(_appliedRate - _rq) <= 0.05;
      if (!out.rateHonored) {
        out.ageNotes.push('المعدل المطبق فعلا ' + _appliedRate + ' كجم/أسبوع من أصل ' + _rq + ' — ده المعدل الناتج عن احتياجك وضوابط الحساب، وقد يختلف عن اختيارك.');
      }
    }
  }
...
function buildWeeklyMeta(profile, inputs, targets){
  ...
  const mealCount = require('./meal-serving-policy').effectiveMealCount(num(p.mealCount != null ? p.mealCount : i['inp-meals'], 2, 8, 4), targets && targets.targetCals);
...
function energyTrace(profile, inputs, targets) {
  const i=inputs||{},p=profile||{};
  return {weightKg:Number(p.weight),heightCm:Number(p.height),age:Number(p.age),
    goal:p.goal,activity:Number(p.activity),steps:Number(i['inp-steps'])||0,
    trainingDays:Number(i['inp-train-days'])||0,bodyFat:Number(i['inp-bf'])||null,
    maintenanceCals:Number(targets.tdee),targetCals:Number(targets.targetCals),
    adjustmentCals:Math.round(Number(targets.targetCals)-Number(targets.tdee)),
    requestedWeeklyRate:Number(p.expectedWeeklyLoss)||0,
    estimatedWeeklyRate:Math.round(Math.abs(Number(targets.tdee)-Number(targets.targetCals))*7/7700*100)/100,
    coachAdjusted:Boolean(targets.coachAdjusted)};
}

function computeMealPlan(profile, inputs){
  assertProfile(profile, 'computeMealPlan');
  const requestedMealCount=Number(profile.mealCount)||4;
  if(requestedMealCount===2){
    const count=require('./meal-serving-policy').effectiveMealCount(2,computeTargets(profile,inputs).targetCals);
    if(count!==2) profile=Object.assign({},profile,{mealCount:count});
  }
  const c = ctx();
  ...
  try {
    ...
    targets.calculation = energyTrace(profile, effInputs, targets);
  ...
  if (plan) {
    try {
      ...
      if (_isTrain) _efInjectPreWorkout(plan, _pf.preWorkoutVariant != null ? _pf.preWorkoutVariant : (Number(meta.dayOfCycle)||0)+((Number(meta.week)||1)-1)*7);
      ...
      plan._saladVariant = (_pf.saladVariant != null ? _pf.saladVariant
        : (_pf.preWorkoutVariant != null ? _pf.preWorkoutVariant : (Number(meta.dayOfCycle)||0)+((Number(meta.week)||1)-1)*7));
      ...
      plan._twoMealMainDinner = meta.mealCount === 2;
      plan.requestedMealCount = requestedMealCount;
      plan.effectiveMealCount = meta.mealCount;
  ...
  if (plan) {
    try { _efEnsureSelectedMealCount(plan, Math.max(2, Math.min(5, Number(meta.mealCount)||3))); }
  ...
  if(plan){
    _efApplyMealFavorites(plan,inputs&&inputs.mealFavorites,inputs&&inputs.availableFoods);
    _efApplyFinalServingPolicy(plan);
    // Rebalance selected foods using the existing optimizer with its hard bounds.
    // Never enlarge rice/salad to close an impossible gap.
    require('./nutrition-plan-auditor').reconcile(plan,c.__EF_FOOD_CATALOG||[]);
    require('./nutrition-rules').enforce(plan);
    _efApplyFinalServingPolicy(plan);
    _efEnforceOwnerServingLimits(plan);
    _efEnforceDailyCaps(plan);
    _efApplyFinalServingPolicy(plan);
    // Only compatible, paired fat additions survive the optimizer.
    for(const m of plan.meals||[]){_efPairFats(m);m.foods=(m.foods||[]).filter(f=>!_efIsAddedFat(f)||f._isAddon);_efRecalcTotals(m);}
    _efBoundedFinalFill(plan);
    _efApplyFinalServingPolicy(plan);
    require('./nutrition-owner-policy').enforce(plan);
    _efRepairEmptySnacks(plan);
    _efTuneFinalQuantities(plan);
    require('./nutrition-owner-policy').enforce(plan);
    const totals=require('./nutrition-plan-auditor').totals(plan);
    plan._calorieGap=Math.round(plan.targetCals-totals.cals);
    plan.portionLimited=Math.abs(plan._calorieGap)>100;
    // Diagnostics describe the final plate, never a stale intermediate audit.
    const wanted=plan.targetMacros||{},pro=Number(wanted.protein||wanted.pro)||0,carb=Number(wanted.carbs||wanted.carb)||0,fat=Number(wanted.fat)||0;
    const pct=(a,b)=>b?Math.round((a-b)/b*1000)/10:0;
    plan._macroAudit={targetCals:plan.targetCals,actualCals:totals.cals,calorieGap:plan._calorieGap,
      targetMacros:{pro,carb,fat},actualMacros:{pro:totals.pro,carb:totals.carb,fat:totals.fat},
      driftPct:{protein:pct(totals.pro,pro),carbs:pct(totals.carb,carb),fat:pct(totals.fat,fat)}};
    const audit=plan._macroAudit;
    audit.passed=!plan.portionLimited&&(!pro||Math.abs(audit.driftPct.protein)<=20)&&(!carb||Math.abs(audit.driftPct.carbs)<=18)&&(!fat||Math.abs(audit.driftPct.fat)<=25);
    for(const m of plan.meals||[])_efPairFats(m);
  }
...
// Bounded calorie closure on existing foods only. The final pass cannot add
// starches or enlarge vegetables; every step respects serving AND day caps.
function _efBoundedFinalFill(plan){
  const real=(plan.meals||[]).filter(m=>!m._autoPreWorkout&&m.foods&&m.foods.length);
  const target=Number(plan.targetCals)||0;
  for(let pass=0;pass<500&&_efDayCalories(plan)<target-65;pass++){
    let best=null;
    const all=(plan.meals||[]).flatMap(m=>m.foods||[]);
    const totals=all.reduce((a,f)=>({pro:a.pro+(Number(f.pro)||0),carb:a.carb+(Number(f.carb)||0),fat:a.fat+(Number(f.fat)||0)}),{pro:0,carb:0,fat:0});
    const wanted=plan.targetMacros||{};
    for(const m of real){
      if(/snack|pre|post/.test(m.slotKey||''))continue;
      const ceiling=target*(real.length<=2?.55:.40);
      for(const f of m.foods){
        if(_efIsVegF(f)||_efCatOf(f)==='salad_veg'||_efIsAddedFat(f)||/زيتون/.test(_efNameOf(f)))continue;
        const cur=Number(f.grams)||0,cap=_efPortionCap(f),group=_efDailyGroup(f);
        if(cur+5>cap)continue;
        if(group&&all.filter(x=>_efDailyGroup(x)===group).reduce((a,x)=>a+(Number(x.grams)||0),0)+5>group.g)continue;
        const per=Number(f.food&&f.food.cal)||0,delta=per*.05;
        if(delta<=0||_efMealCals(m)+delta>ceiling||_efDayCalories(plan)+delta>target+20)continue;
        const proTarget=Number(wanted.protein||wanted.pro)||0;
        const dp=(Number(f.food.pro)||0)*.05;
        if(proTarget&&totals.pro+dp>proTarget*1.35&&dp>2)continue;
        const balance=_efMealCals(m)/target;
        const score=delta/(1+balance*5+(proTarget&&totals.pro>proTarget&&dp>1?4:0));
        if(!best||score>best.score)best={m,f,next:cur+5,score};
      }
    }
    if(!best)break;
    _efSetGramsFine(best.f,best.next);_efRecalcTotals(best.m);
  }
}

// Final quantities use a joint objective, rather than growing starch whenever
// calories are short while protein/fat are already near their limits.
function _efRepairEmptySnacks(plan){
 const policy=require('./nutrition-owner-policy');
 for(const meal of plan.meals||[]){
  if(!/snack/.test(String(meal.slotKey||''))||(meal.foods||[]).length)continue;
  const peers=(plan.meals||[]).filter(m=>m!==meal).sort((a,b)=>Number(/snack/.test(b.slotKey||''))-Number(/snack/.test(a.slotKey||'')));
  let donor=null,source=null;
  for(const m of peers){const f=(m.foods||[]).find(f=>policy.snackAllowed(f)&&Number(f.grams)>=20);if(f){donor=f;source=m;break;}}
  if(!donor)continue;
  // Split an already selected, health/pantry-compatible food. No new ingredient.
  const part=JSON.parse(JSON.stringify(donor)),grams=Math.floor(Number(donor.grams)/10)*5;
  _efSetGramsFine(donor,Number(donor.grams)-grams);_efSetGramsFine(part,grams);
  meal.foods=[part];_efRecalcTotals(source);_efRecalcTotals(meal);
 }
}

function _efTuneFinalQuantities(plan){
 const all=(plan.meals||[]).flatMap(m=>m.foods||[]),target=Number(plan.targetCals)||0,w=plan.targetMacros||{};
 const real=(plan.meals||[]).filter(m=>!m._autoPreWorkout),cap=real.length<=2?.55:.40;
 const sum=()=>all.reduce((a,f)=>[a[0]+(+f.cals||0),a[1]+(+f.pro||0),a[2]+(+f.carb||0),a[3]+(+f.fat||0)],[0,0,0,0]);
 function loss(){const t=sum();let value=3*Math.pow((t[0]-target)/Math.max(100,target*.05),2);
  value+=100000000*Math.pow(Math.max(0,t[0]-target-45,target-t[0]-95)/Math.max(1,target),2);
  [w.protein||w.pro,w.carbs||w.carb,w.fat].forEach((v,i)=>{if(v){value+=Math.pow((t[i+1]-v)/(v*.15),2);value+=10000*Math.pow(Math.max(0,Math.abs(t[i+1]-v)/v-[.195,.175,.245][i]),2);}});
  for(const m of real)if(!/snack/.test(m.slotKey||''))value+=10000000*Math.pow(Math.max(0,_efMealCals(m)/Math.max(1,t[0])-cap),2);
  return value;
 }
 for(let iter=0;iter<180;iter++){
  let best=null,score=loss();
  for(const meal of real)for(const f of meal.foods||[]){
   const name=_efNameOf(f);if(_efIsVegF(f)||_efIsAddedFat(f)||/بيض|زيتون/.test(name))continue;
   const current=Number(f.grams)||0,group=_efDailyGroup(f),upper=_efPortionCap(f);
   const lower=/رودس/.test(name)?80:/عيش|خبز|توست/.test(name)?50:/أرز|ارز|رز|مكرونة|بطاطس/.test(name)?70:_efIsAnimalMain(f)?80:/جبن|قريش|زبادي/.test(name)?60:10;
   for(const step of [-5,5]){const next=current+step;if(next<lower||next>upper)continue;
    if(step>0&&/أرز|ارز|رز/.test(name)&&(meal.foods||[]).filter(x=>/أرز|ارز|رز/.test(_efNameOf(x))).reduce((a,x)=>a+(+x.grams||0),0)+step>400)continue;
    if(step>0&&group&&all.filter(x=>_efDailyGroup(x)===group).reduce((a,x)=>a+(+x.grams||0),0)+step>group.g)continue;
    _efSetGramsFine(f,next);const trial=loss(),cal=sum()[0];_efSetGramsFine(f,current);
    if(cal>target+48&&cal>sum()[0])continue;
    if(cal<target-98&&cal<sum()[0])continue;
    if(trial<score-0.0001){score=trial;best={meal,f,next};}
   }
  }
  if(!best)break;_efSetGramsFine(best.f,best.next);_efRecalcTotals(best.meal);
 }
 for(const meal of plan.meals||[])_efRecalcTotals(meal);
}

// Optional meal-scoped priorities. At most one compatible substitution per
// meal, never an extra food and never a replacement for the existing rules.
function _efApplyMealFavorites(plan,preferences,pantry){
  if(!preferences||!Object.values(preferences).some(x=>Array.isArray(x)&&x.length))return;
  const rules=require('./nutrition-rules');
  function family(food){const f={food},n=_efNameOf(f),c=_efCatOf(f);
    if(/زبادي/.test(n))return 'yogurt';if(/جبن|قريش/.test(n))return 'cheese';
    if(/بيض/.test(n))return 'egg';if(_efIsAnimalMain(f))return 'animal';
    if(c==='fruit')return 'fruit';if(rules._pred.starchKind(f))return 'starch';
    if(/فول/.test(n)&&!/سوداني/.test(n))return 'foul';
    if(/سوداني|لب |فشار|شوكولات/.test(n))return 'snack';return null;
  }
  const all=foodsByIds([...new Set(Object.values(preferences).flat())].slice(0,120));
  const diet=_efDietKey(plan),health=plan._healthConditions||[];
  const good=all.filter(f=>!egyIsRare(f)&&!_efBannedName(plan,f.nameAr)&&passesHealth(f,health)&&
    !(f.avoidHealth||[]).some(x=>health.includes(x))&&(!f.allowedDiets||!f.allowedDiets.length||f.allowedDiets.includes(diet))&&
    (!Array.isArray(pantry)||!pantry.length||pantry.includes(f.id))&&Number(f.cal)>0);
  const counts=()=>{const out={};for(const v of rules.validate(plan))out[v.code]=(out[v.code]||0)+1;return out;};
  let index=0;
  for(const meal of plan.meals||[]){
    const slot=String(meal.slotKey||'');const scope=slot==='breakfast'?'breakfast':(/snack/.test(slot)?'snack':(/^(lunch|dinner|pre)$/.test(slot)?slot:null));
    const wanted=scope&&(preferences[scope]||(/^(lunch|dinner)$/.test(scope)?preferences.main:null));if(!Array.isArray(wanted)||!wanted.length)continue;
    const candidates=good.filter(f=>wanted.includes(f.id)&&require('./nutrition-owner-policy').favoriteAllowed(scope,f));if(!candidates.length)continue;
    const shift=((Number(plan._dayOfCycle)||0)+(Number(plan._weekNumber)||1)+index++)%candidates.length;
    let applied=false;
    for(let k=0;k<candidates.length&&!applied;k++){
      const food=candidates[(k+shift)%candidates.length],kind=family(food);
      if(!kind||meal.foods.some(f=>f.food.id===food.id))continue;
      if(scope==='breakfast'&&!['cheese','egg','yogurt','foul','starch','fruit'].includes(kind))continue;
      if(scope==='snack'&&!['yogurt','fruit','snack'].includes(kind))continue;
      for(let i=0;i<meal.foods.length;i++){
        const old=meal.foods[i];if(family(old.food)!==kind)continue;
        const grams=Math.max(5,Math.round((Number(old.cals)||0)/food.cal*20)*5);
        if(grams>_efPortionCap({food})||grams<20)continue;
        const baseline=counts(),entry={food:Object.assign({},food),grams:0,cals:0,pro:0,carb:0,fat:0};
        _efSetGrams(entry,grams);meal.foods[i]=entry;
        const after=counts();if(Object.keys(after).some(key=>after[key]>(baseline[key]||0))){meal.foods[i]=old;continue;}
        _efRecalcTotals(meal);applied=true;break;
      }
    }
  }
}

function _efApplyFinalServingPolicy(plan){
  let rods=0,olive=null;
  const meals=plan.meals||[];
  const day=Number(plan._dayOfCycle)||0,week=Number(plan._weekNumber)||1;
  for(const m of meals){
    m.foods=(m.foods||[]).filter(f=>{
      const n=_efNameOf(f);
      if(/رودس/.test(n)){
        if(rods)return false;
        rods=1; f.food=Object.assign({},f.food,{nameAr:'جبنة رودس طبيعي'});
        _efSetGramsFine(f,Math.max(80,Math.min(150,Number(f.grams)||80)));
      }
      if(/زيتون مخلل/.test(n)){if(!olive)olive=f;return false;}
      return true;
    });
    if(m.foods.some(f=>/جبنة رومي|جبنه رومي/.test(_efNameOf(f))))m.foods=m.foods.filter(f=>!/زيت زيتون/.test(_efNameOf(f)));
    for(const f of m.foods){
      const max=require('./meal-serving-policy').portionMax(f);
      if(Number(f.grams)>max)_efSetGramsFine(f,max);
    }
    let riceGrams=0;
    m.foods=m.foods.filter(f=>{if(!/أرز|ارز|رز/.test(_efNameOf(f)))return true;
      const keep=Math.min(Number(f.grams)||0,Math.max(0,400-riceGrams));riceGrams+=keep;
      if(keep<=0)return false;_efSetGramsFine(f,keep);return true;});
    const salad=m.foods.filter(f=>_efCatOf(f)==='salad_veg'||(/سلطة/.test(_efNameOf(f))));
    const total=salad.reduce((a,f)=>a+Number(f.grams||0),0);
    if(total>300)for(const f of salad)_efSetGramsFine(f,Math.floor(f.grams*300/total));
    _efRecalcTotals(m);
  }
  // Optional existing olives: 0..2 appearances/week, rotating main slot;
  // no injection when excluded by pantry/health or not selected by the builder.
  if(olive && week%3!==0 && (day===(week%7)||day===(week+3)%7)){
    const slots=meals.filter(m=>/^(breakfast|lunch|dinner)$/.test(m.slotKey||'')&&
      !(m.foods||[]).some(f=>/طحينة/.test(_efNameOf(f))));
    if(slots.length){const m=slots[(day+week)%slots.length];_efSetGramsFine(olive,Math.min(50,Math.max(40,Number(olive.grams)||40)));m.foods.push(olive);_efRecalcTotals(m);}
  }
}

function _efPortionCap(f){
  const hard=require('./meal-serving-policy').portionMax(f);
  if(Number.isFinite(hard))return hard;
...
function _efPreWorkoutMeal(targetCals, variant){
  ...
  const seasonal = _efFoodV3(month>=4&&month<=10 ? 'anb' : 'brtqal');
  if(seasonal) combos.push([{f:coffee,g:200},{f:seasonal,g:120}]);
...
function _efInjectPreWorkout(plan, variant){
  ...
  for (var i = 0; i < plan.meals.length; i++){
    ...
    if (sk === 'pre') { plan.meals[i]=_efPreWorkoutMeal(Number(plan.targetCals)||0,variant); return plan; }
  }
...
function _efMirrorDinner(plan){
  if(plan._twoMealMainDinner)return;
  var meals = (plan && plan.meals) || [];
...
function _efDistribute(real, heavy, amount){
  ...
  while(left>15 && guard++<8){
    ...
    if(!meal2) break;
    var addFood=_efIsBreakfast(meal2)?{id:'ayshasmr',nameAr:'عيش أسمر',cat:'carb',cal:247,pro:9,carb:48,fat:2}:{id:'rice_basmati',nameAr:'ارز بسمتي',cat:'carb',cal:130,pro:2.7,carb:28,fat:0.3};
...
function _efRebalanceDay(plan){
  ...
  while(guard++<120){
    ...
    if(grew<moved-20){ var deficit=moved-grew; var addFood=_efIsBreakfast(light)?{id:'ayshasmr',nameAr:'عيش أسمر',cat:'carb',cal:247,pro:9,carb:48,fat:2}:{id:'rice_basmati',nameAr:'ارز بسمتي',cat:'carb',cal:130,pro:2.7,carb:28,fat:0.3}; var hasCarb=false; var lf=light.foods||[]; for(var q=0;q<lf.length;q++){ if(_efCatOf(lf[q])==='carb'){hasCarb=true;break;} } var ids=lf.map(function(x){return (x.food&&x.food.id)||'';}); if(!hasCarb&&ids.indexOf(addFood.id)===-1 && !/snack|سناك|تحلية/.test(String(light.slotKey||'')+' '+String(light.label||''))){ var cap3=_efPortionCap({food:addFood}); var wg=Math.min(cap3,Math.round(deficit/addFood.cal*100)); wg=Math.round(wg/5)*5; if(wg>=10){ var entry={food:Object.assign({unit:'جم',mealTypes:[]},addFood),grams:0,cals:0,pro:0,carb:0,fat:0}; _efSetGrams(entry,wg); light.foods.push(entry); grew+=Number(entry.cals)||0; } } }
      ...
      if(!carb){
        ...
        var base=_efIsBreakfast(meal)
          ...
          : (hasTuna
            ...
            : _efPickRice(new Date().getDate()+1));
  ...
  if(cheeseId){
    ...
    if(alt&&!_efBannedName(plan,alt.nameAr))for(var b=0;b<meals.length;b++){
      ...
      for(var q=0;q<(bm.foods||[]).length;q++)if(/قريش/.test(_efNameOf(bm.foods[q]))){
        bm.foods[q].food=alt;_efSetGrams(bm.foods[q],cheeseId==='jbn_rwmy'?60:80);break;
      }
    }
  }
}
function _efCanonicalCarbBase(kind){
  ...
  return _efPickRice(new Date().getDate());
}
...
function _efEnsureCookedVegRotation(plan){
  ...
  // Pairing, not a second parity schedule, controls eligibility. The old
  // parity rule excluded every chicken/beef day in the weekly rotation.
  var choices=require('./egyptian-food-db').LUNCH_COOKED_VEG_SOUP.filter(function(f){return !/عدس|بطاطس/.test(f.nameAr);}).map(function(f){return Object.assign({id:f.id,nameAr:f.nameAr,cat:f.cat==='legume_cooked'?'cooked_veg':f.cat},f.per100);});
  for(var i=0;i<(plan.meals||[]).length;i++){
    ...
    var eligibleChoices=choices.filter(function(f){return require('./nutrition-rules').soupAllowedWithFoods(f,m.foods);});
    if(!eligibleChoices.length)continue;
    var existing=m.foods.find(function(f){return _efIsCookedVegV3(f)||_efIsSoupV3(f);});
    if(existing){
      if(!m._cookedRotation124){
        var absoluteDay=(Number(plan._dayOfCycle)||0)+((Number(plan._weekNumber)||1)-1)*7;
        var next=eligibleChoices[Math.abs(absoluteDay+i)%eligibleChoices.length];
        if(!_efBannedName(plan,next.nameAr)){existing.food=next;_efSetGrams(existing,Math.min(180,Number(existing.grams)||150));_efRecalcTotals(m);}
        m._cookedRotation124=true;
      }
      continue;
    }
    ...
    var base=eligibleChoices[Math.abs(day)%eligibleChoices.length];
...
function _efEnsureNaturalYogurtVariety(plan){
  var day=(Number(plan._dayOfCycle)||0)+(Number(plan._weekNumber)||1);if(day%2!==0)return;
  ...
  if(!dest)dest=meals.find(function(m){return m&&/breakfast|dinner/.test(String(m.slotKey||''))&&!m.foods.some(function(f){return _efIsAnimalMain(f)||_efIsCookedVegV3(f)||_efIsSoupV3(f);});});
  if(!dest)return;
  var base={id:'zbadytbyay',nameAr:'زبادي طبيعي',cat:'dairy',cal:61,pro:3.5,carb:4.7,fat:3.3};
  if(_efBannedName(plan,base.nameAr)||/carnivore|vegan/.test(_efDietKey(plan)))return;
  _efAddEntry(dest,base,150);_efRecalcTotals(dest);
}
function _efEnsureMainMealStructure(plan){
  ...
  if(plan._twoMealMainDinner){
    var dinner=meals.find(m=>m&&m.slotKey==='dinner');
    if(dinner&&!dinner.foods.some(_efIsAnimalMain)){
      var animal=_efRotationFood(plan);
      if(animal){
        dinner.foods=dinner.foods.filter(f=>!/(^|\s)بيض(\s|$)|جبن|فول مدمس/.test(_efNameOf(f)));
        _efAddEntry(dinner,animal,200);
      }
    }
  }
  ...
  for(var i=0;i<meals.length;i++){
    ...
    if(needCarb&&!hasCarb){
      ...
      _efAddEntry(m,tuna?{id:'ayshbldy',nameAr:'عيش بلدي',cat:'carb',cal:265,pro:8.5,carb:55,fat:1.5}:{id:'rice_basmati',nameAr:'ارز بسمتي',cat:'carb',cal:130,pro:2.7,carb:28,fat:.3},tuna?60:120);
    }
    if(needCarb){
...
function _efFillTwoMealDenseCarb(plan){
  ...
  var dense=_efIsBreakfast(meal)?{id:'ayshasmr',nameAr:'عيش أسمر',cat:'carb',cal:247,pro:9,carb:48,fat:2}:{id:'rice_basmati',nameAr:'ارز بسمتي',cat:'carb',cal:130,pro:2.7,carb:28,fat:.3};
...
function _efBalanceFinalOneCarb(plan){
  ...
  while(_efDayCalories(plan)<tgt-90&&guard++<1200){
    ...
    for(var i=0;i<mains.length&&!changed;i++){
      ...
      if(!f)continue;var per=(f.food&&Number(f.food.cal))||0,cur=Number(f.grams)||0;if(!per||cur+5>_efPortionCap(f)||per*.05>room+1)continue;
  ...
  if (_tgt > 0 && dCals < _tgt * 0.97){
    ...
    if (dCals < _tgt * 0.97){
      const _TU = {
        ...
        rice:   _efPickRice(new Date().getDate()),
...
var EGY_COMMON_ORDER=[
  // كربوهيدرات
  'رز ابيض','ارز بسمتي','رز بني','عيش بلدي','عيش اسمر','عيش ابيض','بطاطس مسلوقة','بطاطس مشوية','بطاطس بوريه','مكرونة','بطاطا مشوية','فول مدمس',
...
// اختيار الرز: بسمتي 2/3 | أبيض 1/3 (التفضيل للبسمتي)
function _efPickRice(seed){
  var s = typeof seed === 'number' ? seed : new Date().getDate();
  return (s % 3 === 2)
    ? {id:'rice_white', nameAr:'رز أبيض',  cat:'carb',cal:130,pro:2.7,carb:28,fat:0.3}
    : {id:'rice_basmati',nameAr:'ارز بسمتي',cat:'carb',cal:130,pro:2.7,carb:28,fat:0.3};
}
function searchFoods(options){
  ...
  return catalog.filter(function(food){
    ...
    if(options.mealSlot&&!require('./nutrition-owner-policy').favoriteAllowed(options.mealSlot,food))return false;
