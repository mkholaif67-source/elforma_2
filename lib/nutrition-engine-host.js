...
var _EF_DAILY_CAPS = [
  ...
  { key:'tuna',     g:200, test:function(n,c,id){ return /تونة|تونه/.test(n); } },
...
function _efEnforceDailyCaps(plan){
  ...
  for(var gi=0; gi<_EF_DAILY_CAPS.length; gi++){
    ...
    if(total<=grp.g) continue;
    var over=total-grp.g;
...
function _efSetGrams(f, g){
  ...
  g = _fineFat
    ? Math.max(2, Math.min(10, Math.round(g)))
...
var _EF_SALAD_VEG = {
  ...
  flfl:  { id:'flfl',  nameAr:'فلفل ألوان', cat:'veggie', cal:31, pro:1.0, carb:6.0, fat:0.3, g:50 },
...
function _efIsVegEntry(f){
  var c = String(_efCatOf(f) || '');
  return (c === 'veg' || c === 'veggie' || c === 'vegetable' || c === 'vegetables');
}
...
function _efIsCookedVegV3(f){ var n=_efNameOf(f); return /خضار مطبوخ|خضار سوتيه|سوتيه|بامية|باميه|ملوخية|ملوخيه|كوسة|كوسه|باذنجان|سبانخ|قرنبيط|بروكلي|فاصوليا خضراء|بسلة|بسله/.test(n); }
function _efIsSoupV3(f){ var n=_efNameOf(f); return /عدس|شوربة|شوربه/.test(n); }
...
function _efOwnerPre(plan){
  ...
  if(diet==='balanced'){
    for(var m2=0;m2<meals.length;m2++){
      var meal2=meals[m2]; if(!meal2||meal2._autoPreWorkout||!Array.isArray(meal2.foods)) continue;
      var slot2=String(meal2.slotKey||''); if(!(slot2==='breakfast'||slot2==='lunch'||slot2==='dinner'||_efIsBreakfast(meal2))) continue;
      if(!meal2.foods.some(_efIsSoupV3)) continue;
      var otherCarb=meal2.foods.some(function(f){ return _efCatOf(f)==='carb' && !_efIsSoupV3(f); });
      var cookedVeg=meal2.foods.some(_efIsCookedVegV3);
      if(otherCarb||cookedVeg){ meal2.foods=meal2.foods.filter(function(f){ return !_efIsSoupV3(f); }); }
    }
  }
...
function _efOwnerPost(plan){
  ...
  if(tTot>200 && tTot>0){ var sc=200/tTot; tEntries.forEach(function(f){ _efSetGrams(f, Math.max(20, Math.round((Number(f.grams)||0)*sc))); }); }
...
function _efIsVegF(f){ var c=_efCatOf(f); return c==='veg'||c==='veggie'||c==='vegetable'||c==='vegetables'; }
...
function _efSnapFat(g){ g=Math.round(Number(g)||0); return g<2?0:Math.min(10,g); }
...
function _efEnforceFatPairing(plan){
  ...
  for(var pm=0; pm<meals.length; pm++){
    ...
    var _oilOk=_names.some(function(n){ return /سلطة/.test(n) || (/فول/.test(n)&&!/سوداني/.test(n)) || (/جبن/.test(n)&&/بيضا|بيضاء|قريش|رودس/.test(n)); });
...
function _efSizeAddedFats(plan){
  ...
  var baseFat=0, added=[];
  for(var m=0;m<meals.length;m++){ var fs=(meals[m]&&meals[m].foods)||[]; for(var i=0;i<fs.length;i++){ var f=fs[i]; if(_efIsAddedFat(f)) added.push({meal:meals[m],f:f}); else baseFat+=Number(f.fat)||0; } }
  if(!added.length) return;
  var gap = tgtFat>0 ? (tgtFat-baseFat) : 0;
  for(var a=0;a<added.length;a++){ var f=added[a].f; var per=(f.food&&f.food.fat!=null)?Number(f.food.fat):100;
    if(gap<=1 || tgtFat<=0){ added[a].meal.foods=added[a].meal.foods.filter(function(x){return x!==f;}); continue; }
    var needG=per>0?(gap/(per/100)):0; var g=_efSnapFat(Math.min(10,needG));
    if(g<=0){ added[a].meal.foods=added[a].meal.foods.filter(function(x){return x!==f;}); continue; }
    _efSetGramsFine(f,g); gap-=(Number(f.fat)||0);
  }
}
...
function _efSanitizePlan(plan){
  ...
  // [FIX-HARDCAP] فرض سقف الوجبة كخطوة أخيرة مضمونة (بعد كل الموازنات).
  ...
  try { _efGrowMainsToTarget(plan); } catch(_gmErr){ plan.growMainsError = String((_gmErr&&_gmErr.message)||_gmErr); }
