...
async function send({ audience, title, body, data, image, channel, sound }) {
  ...
  for (let i = 0; i < tokens.length; i += chunkSize) {
    ...
    await Promise.all(chunk.map(async (token) => {
      ...
      // Admin notifications are rendered once by the authenticated mobile inbox.
      if (dataPayload.notifId && dataPayload.type !== 'silent') {
        delete message.notification;
        delete message.android.notification;
        message.apns = {headers:{'apns-priority':'5','apns-push-type':'background'},payload:{aps:{'content-available':1}}};
      }
      try {
