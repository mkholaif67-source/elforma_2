...
function bounds(entry, meal) {
  ...
  const hard=require('./meal-serving-policy').portionMax(entry);
  if(Number.isFinite(hard))return {min:isVeg(entry)?40:70,max:hard,step:5};
  ...
  if (/جبنة رودس|جبنه رودس/.test(n)) return { min: 80, max: 150, step: 5 };
...
function ensureCarbClosurePath(plan, catalog) {
  if(plan.effectiveMealCount)return; // The host owns meal composition; no hidden extra starch.
