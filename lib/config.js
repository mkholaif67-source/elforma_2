...
// --- Geo / currency detection (IP-based via trusted edge headers; tz fallback client-side) ---
// Edge country headers are security-sensitive input.  They are only accepted
// when the deployment explicitly says that traffic is behind its trusted
// proxy and that the proxy has been verified to overwrite these headers.
// Without both flags, an attacker can forge a country header and influence the
// EGP/USD payment decision.
function countryFromReq(req) {
  if (process.env.EF_TRUST_PROXY !== '1' || process.env.EF_TRUST_GEO_HEADERS !== '1') return null;
