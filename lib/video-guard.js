...
var _catalogue = null;
var _catalogueAt = 0;
var _cataloguePrevious = {};

function _walkCatalogue(node, map, depth, previous) {
  ...
  if (Array.isArray(node)) {
    for (var i = 0; i < node.length; i++) {
      ...
      if (item && typeof item === 'object' && typeof item.n === 'string') {
        ...
        // alt names a DIFFERENT exercise, not an alias for this video's movement.
        // Only exact authored names/ids may enter this catalogue.
        if (k && Array.isArray(item.vidPrevious)) {
          previous[k] = (previous[k] || []).concat(item.vidPrevious);
        }
        if (item.id != null) {
        ...
        _walkCatalogue(item, map, depth + 1, previous);
      }
    }
    return;
  }
  if (typeof node === 'object') {
    ...
    for (var j = 0; j < keys.length; j++) _walkCatalogue(node[keys[j]], map, depth + 1, previous);
  }
}
...
function catalogueMap() {
  ...
  try {
    ...
    try {
      ...
      var wrapped = src + '\n;this.__EF_GYM_DB = { gym: typeof GYM_DB !== "undefined" ? GYM_DB : null, home: typeof HOME_DB !== "undefined" ? HOME_DB : null, modules: typeof MODULE_DB !== "undefined" ? MODULE_DB : null };';
      new vm.Script(wrapped, { filename: 'engine-db.js' }).runInContext(ctx, { timeout: 5000 });
      parsed = sandbox.__EF_GYM_DB;
    } catch (e) { parsed = null; }
    if (parsed && typeof parsed === 'object') _walkCatalogue(parsed, map, 0, previous);
  ...
  _catalogue = map;
  _cataloguePrevious = previous;
  _catalogueAt = now;
  return map;
}

function invalidateCatalogue() { _catalogue = null; _cataloguePrevious = {}; _catalogueAt = 0; }
...
function resolve(rawVid, muscle, exerciseName) {
  ...
  // Explicit owner-authorized replacements only. Do not rewrite arbitrary
  // authored videos or supersede an online admin override/removal.
  if (key && id && source !== 'override') {
    var current = extractId(catalogueVid(exerciseName));
    var previous = _cataloguePrevious[key] || [];
    if (current && previous.indexOf(id) !== -1) {
      id = current;
      source = 'catalogue';
    }
  }
