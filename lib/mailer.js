...
function deliver(to, subject, html) {
  ...
  if (url && key) {
    ...
    // [MAIL-PROVIDERS] نفس المتغيرات بتخدم أكتر من مزود — الشكل بيتبنى حسب الـ URL:
    //   Resend:  POST /emails  { from, to[], subject, html } + Authorization: Bearer
    //   Brevo:   POST /v3/smtp/email { sender:{name,email}, to:[{email}], subject, htmlContent } + api-key
    // (Brevo بيسمح بالإرسال من إيميل مؤكد بدون شراء دومين — طريق مجاني شغال فورًا)
    const m = /^\s*([^<]+?)\s*<([^>]+)>\s*$/.exec(from);
    const fromName = m ? m[1] : 'ElForma';
    const fromEmail = m ? m[2] : from;
    const isBrevo = /api\.brevo\.com/i.test(url);
    const headers = { 'content-type': 'application/json' };
    if (isBrevo) headers['api-key'] = key; else headers['authorization'] = 'Bearer ' + key;
    const payload = isBrevo
      ? { sender: { name: fromName, email: fromEmail }, to: [{ email: to }], subject: subject, htmlContent: html }
      : { from: from, to: [to], subject: subject, html: html };
    // Fire-and-forget: a slow mail provider must never block the HTTP response
    // the user is waiting on. لكن الرفض بيتسجل بجسم الرد — رسالة المزود هي اللي
    // بتقولك السبب الحقيقي (مثال Resend sandbox بيرفض أي مستلم مش حسابك بـ 403).
    fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload) })
      .then(async (r) => {
        if (!r.ok) {
          const detail = await r.text().catch(() => '');
          console.error('[mailer] transport rejected the email:', r.status, String(detail).slice(0, 200));
        }
      })
