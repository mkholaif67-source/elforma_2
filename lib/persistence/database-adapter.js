...
function requestedEngine(env) {
  ...
  if (configured === 'auto') {
    if (env.DATABASE_URL) throw new Error('unsupported_database_engine:postgres (async migration not implemented)');
    if (env.TURSO_DATABASE_URL) return 'turso';
    return 'sqlite';
  }
  return configured;
}
...
function openTurso(dbPath, env) {
  ...
  const openRemote = () => new LibsqlDatabase(url, { authToken: env.TURSO_AUTH_TOKEN });
  let client;
  // الوضع الفعلي قد يختلف عن المطلوب: لو الريبليكا فشلت نرجع remote تلقائيًا.
  let effectiveMode = mode;
  if (mode === 'replica') {
    /* [FIX-WALCONFLICT v2 — replica معطّلة لهذه القاعدة، والـ deploy لن يقع بسببها أبدًا]
       المثبت من لوج الإنتاج (3 محاولات، frames ‎97 ← 105 ← 149‎): القاعدة اتكتب عليها
       شهور بوضع remote المباشر، فتوليد الـ WAL على الـ primary بيرفض أي ريبليكا
       جديدة (WalConflict). والأخطر: الجسّ الاختباري ممكن ينجح بالصدفة وبعد ثواني
       أول كتابة حقيقية تضرب WalConflict وتوقّع الإقلاع (حصل فعلًا 04:24 في
       verifyDatabaseContract). يعني مفيش تجربة ريبليكا آمنة على القاعدة دي.
       القرار: نتجاهل replica ونشغّل remote الثابت من شهور — ولو اللوحة فيها
       replica بالغلط، الإقلاع يفضل أخضر. لو يومًا اتعملت قاعدة Turso جديدة
       (dump/restore) وعايزين سرعة الريبليكا: امسح البلوك ده ضمن ترقية مدروسة. */
    console.warn('[db] EF_TURSO_MODE=replica لكن قاعدة Turso الحالية بترفض الريبليكا (WalConflict). شغالين على remote الثابت.');
  ...
  return {
    ...
    info: Object.freeze({
      name: 'turso',
      mode: effectiveMode,
      engine: 'turso-' + effectiveMode,
      dialect: 'sqlite',
      api: 'sync',
      durable: true,
      remoteWrites: true,
      replica: effectiveMode === 'replica',
      dbPath: effectiveMode === 'remote' ? null : dbPath,
    }),
    syncNow() {
      if (effectiveMode !== 'replica') return;
      try { client.sync(); }
...
function openDatabaseAdapter(options) {
  ...
  if (engine === 'postgres') {
    throw new Error('postgres_adapter_not_implemented: keep Turso configured; a tested async migration is required');
  }
