...
const db = require('./request-reads').wrapDatabase(databaseAdapter.client);
...
CREATE INDEX IF NOT EXISTS idx_workout_sessions_active_user
...
const mobileQ = {
  ...
  startSession: db.prepare(
    "INSERT INTO workout_sessions (user_id, plan_id, day_key, day_name, status, started_at) SELECT ?, ?, ?, ?, 'active', ? WHERE NOT EXISTS (SELECT 1 FROM workout_sessions WHERE user_id=? AND status='active') RETURNING id"
  ),
...
module.exports = {
  ...
  mobileProfile(userId) {
    const row=mobileQ.profile.get(userId);
    if(!row)return row;
    try { const p=JSON.parse(row.profile_json), clean=require('./profile-integrity').repairLegacyMeasurements(p);
      return clean===p?row:Object.assign({},row,{profile_json:JSON.stringify(clean)});
    } catch (_) { return row; }
  },
  ...
  startWorkoutSessionOnce(userId, planId, dayKey, dayName, startedAt) {
    const inserted = mobileQ.startSession.get(userId, planId || null, dayKey, dayName || null, startedAt || now(), userId);
    const active = inserted || mobileQ.activeSession.get(userId);
    if (!active) throw new Error('session_start_not_persisted');
    return {id:Number(active.id),created:Boolean(inserted)};
  },
