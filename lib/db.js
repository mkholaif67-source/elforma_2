...
// [PERSISTENCE] اختيار محرك قاعدة البيانات:
//  • افتراضيًا: SQLite محلي عبر node:sqlite (تجربة فورية بدون أي إعداد).
//  • لو اتظبطت TURSO_DATABASE_URL → نستخدم libsql كـ Embedded Replica:
//    نسخة محلية سريعة بتتزامن مع Turso السحابي، فالداتا دائمة
//    وتتحمل نجاح التطبيق حتى على استضافات بتمسح الملفات (serverless).
//  ملاحظة مهمة: في وضع السحابة لازم EF_SECRET يكون ثابتًا (متغيّر بيئة)
//  عشان المستخدم مايتسجّلش خروج بعد كل إعادة تشغيل.
let db;
let usingTurso = false;
// [FIX WalConflict] وضع الاتصال بـ Turso:
//  • remote (الافتراضي الجديد): الكتابة بتروح لـ Turso مباشرة، مفيش نسخة محلية
//    ولا WAL محلي — يعني مستحيل يحصل "WalConflict" ولا فقدان داتا لو الاستضافة
//    مسحت الملفات (Render المجاني بيمسح القرص عند كل restart/deploy).
//  • replica: النسخة المحلية المتزامنة (تنفع بس مع قرص دائم فعلًا).
const TURSO_MODE = String(process.env.EF_TURSO_MODE || 'remote').toLowerCase();
if (process.env.TURSO_DATABASE_URL) {
  // [FIX] TLS CA certs على Linux (Render/Docker)
  if (!process.env.NODE_EXTRA_CA_CERTS) process.env.NODE_EXTRA_CA_CERTS = '/etc/ssl/certs/ca-certificates.crt';
  try {
  const LibsqlDatabase = require('libsql');
  if (TURSO_MODE === 'replica') {
    // [FIX WalConflict] لازم نمسح أي نسخة محلية قديمة + ملفات ميتاداتا الريبليكا
    // (‎-client_wal_index / -info / -wal / -shm). لو الداتا اتمسحت والميتاداتا
    // فضلت، libsql يفضل يدفع فريمات من مكان غلط والسحابة ترفض: WalConflict.
    const stale = fs.existsSync(DB_PATH + '-client_wal_index') && !fs.existsSync(DB_PATH);
    if (stale || process.env.EF_TURSO_RESET_REPLICA === '1') {
      for (const suffix of ['', '-wal', '-shm', '-journal', '-info', '-client_wal_index']) {
        try { fs.rmSync(DB_PATH + suffix, { force: true }); } catch (_) {}
      }
      console.warn('[db] مسحنا نسخة محلية غير متسقة قبل المزامنة (منع WalConflict).');
    }
    db = new LibsqlDatabase(DB_PATH, {
      syncUrl: process.env.TURSO_DATABASE_URL,
      authToken: process.env.TURSO_AUTH_TOKEN,
      // يسحب أحدث نسخة من السحابة دوريًا (بالثانية) بالإضافة للمزامنة وقت الكتابة.
      syncInterval: Number(process.env.TURSO_SYNC_INTERVAL || 60),
    });
    try { db.sync(); } catch (e) { console.error('[db] Turso initial sync failed:', e && e.message); }
    console.log('[db] Using Turso (libsql embedded replica) at', DB_PATH);
  } else {
    // اتصال مباشر بالسحابة — بدون ملفات محلية خالص.
    db = new LibsqlDatabase(process.env.TURSO_DATABASE_URL, {
      authToken: process.env.TURSO_AUTH_TOKEN,
    });
    console.log('[db] Using Turso (remote, no local replica)');
  }
  usingTurso = true;
  } catch (e) {
    // A configured remote database is authoritative. Never fall back silently
    // to a different local database: that creates split-brain accounts and
    // makes successful admin writes disappear after a restart.
    const detail = e && e.message ? e.message : String(e);
    console.error('[db] configured Turso/libSQL database is unavailable:', detail);
    throw new Error('configured_database_unavailable: ' + detail);
  }
}
if (!usingTurso) {
  // [FIX-DEPLOY-SQLITE] رسالة مفهومة بدل ERR_UNKNOWN_BUILTIN_MODULE الغامض.
  let DatabaseSync;
  try {
    DatabaseSync = require('node:sqlite').DatabaseSync;
  } catch (e) {
    console.error('[db] node:sqlite مش متاح — شغل السيرفر ب\u0640 node --experimental-sqlite server.js أو NODE_OPTIONS=--experimental-sqlite (Node ' + process.version + ')');
    throw e;
  }
  db = new DatabaseSync(DB_PATH);
  console.log('[db] Using local SQLite (node:sqlite) at', DB_PATH);
}

// Pragmas for durability + concurrency.
// Pragmas for durability + concurrency — libsql's embedded-replica connection
// manages WAL/durability internally and rejects these, so only apply on
// the local node:sqlite path.
if (!usingTurso) {
...
// Prove the adapter and SQL dialect before preparing application statements.
// The server must not accept traffic against an unverified database engine.
...
module.exports = {
  ...
  storage: {
    engine: usingTurso ? ('turso-' + TURSO_MODE) : 'local-sqlite',
    durable: usingTurso || process.env.EF_STORAGE_DURABLE === '1',
    dbPath: (usingTurso && TURSO_MODE === 'remote') ? null : DB_PATH,
  ...
  syncNow() {
    // في الوضع المباشر (remote) الكتابة أصلاً وصلت السحابة، فمفيش حاجة تتزامن.
    if (!usingTurso || TURSO_MODE !== 'replica') return;
    try { db.sync(); } catch (e) { console.error('[db] syncNow failed:', e && e.message); }
  },
  createUser(email, phone, name, hash, salt) {
    ...
    try { if (usingTurso && TURSO_MODE === 'replica') db.sync(); } catch (_) {}
    return id;
  },
