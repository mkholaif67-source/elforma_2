...
const ownerPolicy = require('./nutrition-owner-policy');
...
function isCookedVeg(f) { return ownerPolicy.isCookedVeg(f); }
function isLentil(f) { return /عدس/.test(nameOf(f)); }
function isBrownLentil(f) { return /عدس.*بجب|عدس.*بجبه|عدس.*بجبة/.test(nameOf(f)); }
function isSoup(f) { return catOf(f) === 'soup' || /شوربة|شوربه|عدس.*أصفر|عدس.*اصفر/.test(nameOf(f)); }
// Optional soups must match the actual protein, never their own broth name.
function soupAllowedWithFoods(soup, foods) {
  if (!isSoup(soup)) return true;
  const mains = foods.filter(function(f) {
    return !isSoup(f) && !/مرق/.test(nameOf(f)) && /^(protein(?:_.*)?|meat|chicken)$/.test(catOf(f));
  });
  const chicken = mains.filter(function(f){ return /فراخ|دجاج/.test(nameOf(f)); });
  if (chicken.some(function(f){ return /مشو[يى]/.test(normalizedName(f)); })) return false;
  const name = normalizedName(soup), id = idOf(soup);
  if (/shorbet_dagag|shwrba_mrqa_frakh/.test(id) || /فراخ|دجاج/.test(name))
    return chicken.length > 0 && !mains.some(function(f){ return /لحم|لحمه|بقري/.test(nameOf(f)); });
  if (/shorbet_lahma|shwrba_mrqa_lhma/.test(id) || /لحم|لحمه/.test(name))
    return chicken.length === 0 && mains.some(function(f){ return /لحم|لحمه|بقري/.test(nameOf(f)); });
  return true;
}
...
function isPreEnergy(f, now) {
  ...
  return /موز|تفاح|تمر|شوكولاتة (?:دارك|داكنة)/.test(n) || (catOf(f)==='fruit' && isAllowedFruit(f,now));
}
...
function validate(plan) {
  ...
  meals.forEach(function (meal) {
    ...
    const hasMacarona = foods.some(isMacarona);
    const rawSalad = foods.filter(isRawSaladVeg);
    const bannedCookedProtein = foods.some(isCookedVegBannedProtein);
    if (foods.some(isBrownLentil)) v.push({code:'brown_lentil_auto_excluded',meal:label,msg:'العدس بجبة مستبعد من التوليد التلقائي'});
    if (foods.some(isLentil) && foods.some(f=>starchKind(f))) v.push({code:'lentil_starch_conflict',meal:label,msg:'العدس لا يجمع مع نشويات أخرى'});
    ...
    if (isMainMeal(meal) && !ownerPolicy.lightDinner(meal) && !hasCookedVeg && !hasSoup && rawSalad.length!==3)
    ...
    if (foods.some(function(f){ return !soupAllowedWithFoods(f, foods); }))
      v.push({ code: 'soup_protein_pairing', meal: label, msg: 'المرقة لازم تطابق البروتين، والفراخ المشوية بدون شوربة' });
  ...
  // خضار مطبوخ ≤ 350جم/يوم
  const cookedVegTotal = meals.reduce(function(s,m){
    return s + foodsOf(m).filter(isCookedVeg).reduce(function(a,f){return a + gramsOf(f);}, 0);
  }, 0);
  if (cookedVegTotal > 350) {
    v.push({ code: 'cooked_veg_over_limit', meal: 'day',
      msg: 'إجمالي الخضار المطبوخ ' + Math.round(cookedVegTotal) + 'جم > 350جم' });
  }
  ...
  v.push(...ownerPolicy.validate(plan));
...
function enforce(plan) {
  ...
  meals.forEach(function (meal) {
    ...
    removeFrom(meal,isBrownLentil,fixes,'brown_lentil_auto_excluded','العدس بجبة مستبعد من الوجبات التلقائية حسب طلب المالك');
    if(foodsOf(meal).some(f=>starchKind(f))) removeFrom(meal,isLentil,fixes,'lentil_starch_conflict','العدس مصدر كارب ولا يجمع مع نشويات أخرى');
    ...
    removeFrom(meal, function(f){ return !soupAllowedWithFoods(f, foodsOf(meal)); },
      fixes, 'soup_protein_pairing', 'المرقة مع بروتينها فقط، والفراخ المشوية بدون شوربة');
    ...
    // [NEW] الخضار المطبوخ والشوربة ممنوعان مع المكرونة (خبير تغذية: المكرونة + خضار مطبوخ/شوربة = هضم خاطئ وسعرات زائدة)
    if (foodsOf(meal).some(isMacarona)) {
      removeFrom(meal,function(f){return isCookedVeg(f)||isSoup(f);},fixes,'no_cookedveg_with_pasta','الخضار المطبوخ والشوربة لا يجتمعان مع المكرونة');
    }
    ...
    const forceSalad=isMainMeal(meal)&&noCooked&&!ownerPolicy.lightDinner(meal)&&String(plan&&plan._dietKey||'')!=='carnivore';
...
function capNutrient(pred, maxGrams, code, why) {
  ...
  // لو عدد الوجبات 2 → بيض/قريش لازم يكون 200جم عشان يكفي احتياج البروتين
  if (meals.length <= 2) {
    const eggMin2Meal = 200;
    meals.forEach(function(meal){
      const eggs = foodsOf(meal).filter(isEgg);
      if (eggs.length > 0) {
        const eggG = eggs.reduce(function(s,f){return s+gramsOf(f);},0);
        if (eggG > 0 && eggG < eggMin2Meal) {
          const diff = eggMin2Meal - eggG;
          const firstEgg = eggs[0];
          setGramsSafe(firstEgg, gramsOf(firstEgg) + diff);
          fixes.push({ code: 'egg_boosted_2meal', why: 'بيض لوجبتين/يوم → 200جم' });
        }
      }
      const cheeses = foodsOf(meal).filter(function(f){return /قريش|جبنة قريش/.test(nameOf(f));});
      if (cheeses.length > 0) {
        const cheeseG = cheeses.reduce(function(s,f){return s+gramsOf(f);},0);
        if (cheeseG > 0 && cheeseG < eggMin2Meal) {
          const diff = eggMin2Meal - cheeseG;
          setGramsSafe(cheeses[0], gramsOf(cheeses[0]) + diff);
          fixes.push({ code: 'cheese_boosted_2meal', why: 'قريش لوجبتين/يوم → 200جم' });
        }
      }
    });
  }
...
module.exports = {
  validate, enforce, soupAllowedWithFoods,
