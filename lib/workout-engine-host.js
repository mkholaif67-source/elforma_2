...
function buildContext() {
  ...
  code += [
    ...
    "          q=PlanValidator.scorePlan(td,{ exp:S.exp, goal:S.goal, recoveryScore:+S.recoveryScore||70, minEx:(S.exp==='advanced'?(S.gender==='female'?5:6):(S.gender==='male'?5:(S.exp==='beginner'?4:5))) });",
    "        } }catch(e){ q=null; }",
...
function computeWorkout(profile) {
  ...
  // [FEMALE-FLOOR v2 2026-09] مراجعة المالك: أي يوم أنثى ≥ 5 (أي مستوى أو جدول) —
  // [FEMALE-FLOOR v3 2026-09] والذكر: المبتدئ 5 — المتوسط/المتقدم 6،
  // إلا لو أيام تمرينه أكتر من 4/أسبوع وقتها 5 (الحجم متوزع على أيام أكتر).
  const _ruleDays = Number(p.days) || 3;
  const minDayExercises = p.gender === 'female' ? 5
    : (p.exp === 'beginner' ? 5 : (_ruleDays <= 4 ? 6 : 5));
      ...
      (plan.plan || []).forEach(function (day) {
        ...
        for (let guard = 0; day.exercises.length < minDayExercises && guard < seeds.length * 4; guard++) {
          ...
          day.exercises.push({
            ...
            sets: p.exp === 'advanced' ? 3 : 2, reps: '10-15', rest: '60-90 ث', rir: '2 RIR',
      ...
      try {
        if (c.PlanValidator && c.PlanValidator.scorePlan) {
          plan.quality = c.PlanValidator.scorePlan(plan.trainDays, {
            exp: p.exp, goal: p.goal, recoveryScore: Number(out.metrics && out.metrics.recoveryScore) || 70, minEx: minDayExercises
          });
        }
      } catch (_) {}
    });
