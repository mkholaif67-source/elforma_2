...
// All mobile surfaces resolve latest measurements and coach decisions here.
function contextForUser(profile, userId) {
  const p=Object.assign({},profile), db=require('./db');
  const row=userId?db.mobileProfile(userId):null;
  const profileAt=Date.parse(row&&row.updated_at)||0;
  const weights=userId?db.recentWeights(userId,1):[];
  // A deliberate profile edit is newer than an old weight log.
  if(weights.length && Number(weights[0].weight)>0 &&
    (Date.parse(weights[0].created_at||weights[0].logged_at||weights[0].date)||0)>=profileAt) p.weight=Number(weights[0].weight);
  const ms=userId?db.recentMeasurements(userId,1):[];
  const m=ms[0]||{}, loggedNewer=(Date.parse(m.created_at||m.logged_at||m.date)||0)>=profileAt;
  const pick=(key,logKey)=>loggedNewer&&m[logKey]!=null?m[logKey]:(Object.prototype.hasOwnProperty.call(p,key)?p[key]:m[logKey]);
  p.__extras={steps:p.steps,cardioSessions:p.cardioSessions,cardioIntensity:p.cardioIntensity,
    weeklyRate:p.weeklyRate,bodyFat:pick('bodyFat','body_fat'),waist:pick('waist','waist'),
    neck:p.neck,hips:pick('hips','hips')};
  try {
    if(userId&&require('./user-prefs').smartCoachEnabled(userId)){
      const t=db.db.prepare('SELECT target_cals,created_at FROM coach_targets WHERE user_id=? ORDER BY week DESC LIMIT 1').get(userId);
      if(t && (Date.parse(t.created_at)||0)>=profileAt) p.coachTargetCals=Number(t.target_cals);
    }
  } catch (_) { /* No coach decision yet. */ }
  return p;
}
