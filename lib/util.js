...
function serveStatic(res, rootDir, urlPath, req) {
  ...
  if (stat.isDirectory()) return serveStatic(res, rootDir, rel.replace(/\/?$/, '/'), req);
  // [ETAG] ختم للملف (حجم + لحظة تعديل): المتصفح يسأل «اتغير؟» والسيرفر يرد
  // 304 فاضي بدل ما يبعت الملف تاني — الزيارات المتكررة أسرع بكتير وأوفر باندويث.
  // المتغير الرابع req اختياري: أي استدعاء قديم من غيره يفضل شغال زي ما هو.
  const etag = 'W/"' + stat.size + '-' + Math.floor(Number(stat.mtimeMs)) + '"';
  const cacheControl = mimeFor(full).startsWith('text/html') ? 'no-cache' : 'public, max-age=3600';
