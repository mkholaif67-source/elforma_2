...
function sendJson(res, status, obj, extraHeaders) {
  ...
  const accepts = String(res && res.req && res.req.headers && res.req.headers['accept-encoding'] || '');
  // الخطط الكبيرة كانت تنتقل كنص JSON خام. ضغطها يقلل وقت الشبكة بوضوح،
  // بينما الردود الصغيرة تظل بلا ضغط لتجنب تكلفة غير مفيدة على السيرفر.
  if (body.length >= 4096 && /(?:^|[,\s])gzip(?:[,\s]|$)/i.test(accepts)) {
    const packed = zlib.gzipSync(Buffer.from(body), { level: 5 });
