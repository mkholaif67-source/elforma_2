...
function pickExercises(groups, equip, injuries, goal, time, exp, gender, dayIdx, crossDayUsed){
  ...
  // [FEMALE-FLOOR v3 2026-09] قواعد المالك: مفيش قاعدة 6 للإناث خالص —
  // أي يوم أنثى ≥ 5 تمارين مهما كان المستوى أو الجدول.
  // الذكر: المبتدئ ≥5 — المتوسط/المتقدم ≥6، إلا لو جدوله أكتر من 4 أيام
  // تمرين/أسبوع وقتها ≥5 (لأن الحجم الأسبوعي بيتوزع على أيام أكتر).
  const _maleFloor = (state.days || 3) <= 4 ? 6 : 5;
  const advancedFloor = isFemale ? 5 : _maleFloor;
  let MIN_EX = isBeginner ? 5 : advancedFloor; // ABS-FLOOR-5: لا يقل أي يوم عن 5 تمارين لأي مستوى
  ...
  if(sessionTime <= 45){
    ...
    if(!isBeginner) MIN_EX = Math.max(MIN_EX, advancedFloor);
    MIN_SETS += 1;
  ...
  if (_isLegsSession) {
    ...
    // يوم الأرجل: 6 groups أساسية (كوادز×2 + هامستينج + جلوتس + سمانة + ضامة) - max 8
    MAX_EX = 8;
    MIN_EX = Math.max(MIN_EX, isBeginner ? 5 : advancedFloor);
  }
