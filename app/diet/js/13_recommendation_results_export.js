...
function buildSmartMealPlan(totalCals, macros, weeklyMeta) {
  ...
  const mealsHTML = mealTypes.map(mType => {
    ...
    const _saladGrouped = (function(ps){
      var veg=ps.filter(function(p){return p.food&&(p.food.cat==='veggie'||p.food.cat==='salad_veg');});
      var oth=ps.filter(function(p){return !(p.food&&(p.food.cat==='veggie'||p.food.cat==='salad_veg'));});
      if(veg.length!==3) return ps;
