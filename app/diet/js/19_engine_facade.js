...
function _markProteinUsed(food) {
  ...
  // [OWNER-PAIRING] الخضار المطبوخ/الشوربة ممنوعان مع السمك والتونة فقط.
  // مسموحان مع الفراخ واللحمة والكفتة والكبدة والقوانص.
  ...
  function _egyBansCookedVeg(protein){
    ...
    return /سمك|smk|بلطي|blty|ماكريل|makryl|سردين|srdyn|بوري|bwry|سلمون|slmwn|رنجة|rnja|فسيخ|fsykh|فيليه|fylyh|بربوني|بلاميط|قاروص|دنيس|موسى|جمبري|jmbry|كابوريا|kabwrya|تونة|twna/.test(_egyText(protein));
  }
  ...
  // [OWNER-PAIRING] أولوية الكارب حسب البروتين المصري.
  function _egyCarbAffinity(carb, pk, proteinFood){
    if (!carb || carb.cat!=='carb' || pk==='other') return 0;
    const t = _egyText(carb), p = _egyText(proteinFood);
    const rice = /أرز|ارز|رز|arz|rice/.test(t);
    const pasta = /مكرون|mkrwn|باستا|basta|pasta/.test(t);
    const bread = /عيش|توست|خبز|aysh|twst|tost|bread/.test(t);
    const potato = /بطاطس|بطاطا|btats|btata/.test(t);
    const breadFriendly = /كفتة|كفته|kfta|اسكندران|إسكندران|قوانص|kwans|qwanes|wnwans/.test(p);
    if (pk==='tuna'){ if (rice) return -100; if (bread) return 45; if (pasta) return 24; return potato ? 6 : 0; }
    if (pk==='fish'){ if (pasta) return -100; if (bread) return 38; if (rice) return 32; return potato ? 4 : 0; }
    if (breadFriendly){ if (bread) return 42; if (rice) return 32; if (pasta) return 25; return potato ? 12 : 0; }
    if (pk==='meat'){ if (bread) return -100; if (rice) return 44; if (pasta) return 34; return potato ? 18 : 0; }
    return pasta ? 30 : (rice ? 26 : 0);
  }
  function _egyPickCarb(pools, slot, context, usedIds, proteinFood){
    ...
    const sc = (f)=> scoreFood(f,slot,context) + _egyCarbAffinity(f, pk, proteinFood);
  ...
  function _egyEligibleForSlot(food, slot, protein){
    ...
    // التونة: العيش أولا ثم المكرونة؛ الرز ممنوع. السمك: عيش/رز، لا مكرونة.
    if (food.cat === 'carb' && _egyProteinKind(protein) === 'tuna' && /أرز|ارز|رز|arz|rice/.test(_egyText(food))) return false;
    if (food.cat === 'carb' && _egyProteinKind(protein) === 'fish' && /مكرون|mkrwn|باستا|pasta/.test(_egyText(food))) return false;
    // اللحمة القطعيات لا يظهر معها عيش؛ الكفتة والكبدة والقوانص مستثناة.
    if (food.cat === 'carb' && _egyProteinKind(protein) === 'meat' &&
        !/كفتة|كفته|kfta|اسكندران|إسكندران|قوانص|kwans|qwanes|wnwans/.test(_egyText(protein)) &&
        /عيش|خبز|توست|aysh|bread/.test(_egyText(food))) return false;
