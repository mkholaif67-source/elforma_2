...
function calcAdaptiveDeficit(tdee) {
  const bf      = estimateBodyFat();
  const lbm     = calcLBM();
  let weeklyRate = parseFloat(document.getElementById('inp-weekly-rate')?.value) || 0.5;
  // [OWNER-RULE 2026-09] أنثى وزنها أقل من 70 كجم: ممنوع هدف الكيلو/أسبوع —
  // العجز اليومي الكبير على وزن صغير = مخاطرة هرمونية وعضلية. السقف 0.75 كجم/أسبوع.
  // 70 كجم أو أكتر: كل المعدلات (0.25/0.5/0.75/1) مسموحة عادي زي الذكور.
  if (DE.gender === 'أنثى' && Number(DE.weight) > 0 && Number(DE.weight) < 70) {
    weeklyRate = Math.min(weeklyRate, 0.75);
  }
...
function calcTargetCals() {
  ...
  if (DE.goal === 'bulk') {
    ...
    // [OWNER-RULE 2026-09] الزيادة (ضخامة أو عضلية): الاختيارات الوحيدة ربع أو نص
    // كجم/أسبوع — أي قيمة تانية بتتقص للنطاق ده — والفائض مسموح يوصل 500 سعرة/يوم.
    const _weeklyRateRaw = parseFloat(document.getElementById('inp-weekly-rate')?.value) || 0.5;
    const weeklyRateBulk = Math.min(0.5, Math.max(0.25, _weeklyRateRaw));
    const rateSurplus = Math.round(weeklyRateBulk * 7700 / 7);
    let bulkSurplus;
    if (_gainStyle === 'lean') {
      bulkSurplus = Math.min(500, Math.max(120, rateSurplus));
    } else {
      bulkSurplus = Math.min(500, Math.max(300, rateSurplus));
    }
