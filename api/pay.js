...
async function mirrorReceipt(file) {
  if (!file) return { ok: false, status: 'none' };
  const full = path.join(RECEIPTS, file);
  const result = await receiptStorage.put(full, file, 'image/' + path.extname(file).slice(1).replace('jpg', 'jpeg'));
  if (!result.ok && result.status !== 'not_configured') {
    console.error('[receipt] durable mirror failed:', result.status);
  }
  return result;
}
...
async function manual(req, res, ip) {
  ...
  // If production explicitly requires durable receipts, fail before creating a
  // pending payment when the remote mirror is unavailable. In development and
  // existing deployments the request remains backward-compatible, but the
  // returned/audited status makes the missing configuration visible.
  const receiptMirror = await mirrorReceipt(receipt);
  if (receipt && process.env.EF_REQUIRE_DURABLE_RECEIPTS === '1' && !receiptMirror.ok) {
    try { fs.rmSync(path.join(RECEIPTS, receipt), { force: true }); } catch (_) {}
    return sendJson(res, 503, { error: 'receipt_storage_unavailable', code: 'durable_receipt_required' });
  }

  const meta = { method, wallet, walletLabel, pm_id: pm ? pm.id : '', destination, sender, ref, receipt,
    receipt_backup_status: receiptMirror.status, receipt_backup_target: receiptMirror.target || null,
    coupon: price.applied, discount: price.discount };
  let payId;
  try {
