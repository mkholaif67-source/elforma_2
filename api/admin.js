...
function requireAdmin(req, res) {
  ...
  if (!user.verified || !cfg.isAdminEmail(user.email)) { sendJson(res, 403, { error: 'forbidden' }); return null; }
  return user;
}
...
async function whoami(req, res) {
  ...
  const adminEmail = !!cfg.isAdminEmail(user.email);
  return sendJson(res, 200, {
    admin: !!user.verified && adminEmail,
    adminEmail,
...
// POST /api/admin/setup/verify { token }
// Browser-only recovery for the allowlisted owner when Render Shell is not
// available and outbound email has not been configured yet. The caller must:
//   1) already be signed in; 2) own an allowlisted admin email; and
//   3) provide the long one-time value stored in EF_ADMIN_SETUP_TOKEN.
// The token is never returned by an API. Remove it from Render after success.
...
async function setupVerify(req, res, ip) {
  const user = auth.currentUser(parseCookies(req));
  if (!user) return sendJson(res, 401, { error: 'unauthenticated' });
  if (!cfg.isAdminEmail(user.email)) return sendJson(res, 403, { error: 'forbidden' });
  if (user.verified) return sendJson(res, 200, { ok: true, already: true });
  if (!rateLimit('admin-setup:' + ip, 5, 15 * 60 * 1000)) {
    return sendJson(res, 429, { error: 'too_many_attempts' });
  }
  const expected = String(process.env.EF_ADMIN_SETUP_TOKEN || '').trim();
  if (expected.length < 24) {
    return sendJson(res, 503, { error: 'setup_token_not_configured' });
  }
  const body = await readJsonBody(req);
  const supplied = String(body.token || '').trim();
  if (!sameSecret(supplied, expected)) {
    db.audit(user.id, 'admin_web_setup_failed', ip);
    return sendJson(res, 403, { error: 'invalid_setup_token' });
  }
  db.setVerified(user.id);
  db.audit(user.id, 'admin_verified_from_web_setup', ip);
  if (db.syncNow) await db.syncNow();
  const saved = db.userById(user.id);
  if (!saved || !saved.verified) return sendJson(res, 500, { error: 'verification_not_saved' });
...
async function receipt(req, res) {
  ...
  if (!full.startsWith(RECEIPTS)) return sendJson(res, 400, { error: 'bad_file' });
  if (!fs.existsSync(full)) {
    // Local Render files are disposable. If the receipt was mirrored to the
    // configured private store, stream it through the authenticated admin
    // route instead of exposing the storage URL to the browser.
    if (await receiptStorage.stream(file, res)) return;
...
async function deleteUser(req, res, ip) {
  ...
  try { require('./mobile').invalidateUserCaches(userId); } catch (_) {}
  db.audit(admin.id, 'admin_delete_user:' + u.email, ip);
  return sendJson(res, 200, { ok: true });
}

// POST /api/admin/users/bulk { userIds:[...], action:grant|extend|revoke|delete, months? }
async function bulkUsers(req, res, ip) {
  const admin = requireAdmin(req, res); if (!admin) return;
  const b = await readJsonBody(req);
  const ids = Array.from(new Set((Array.isArray(b.userIds) ? b.userIds : []).map(Number).filter(Number.isInteger))).slice(0, 100);
  const action = String(b.action || '');
  const months = Math.max(1, Math.min(60, Math.round(Number(b.months) || 1)));
  if (!ids.length) return sendJson(res, 400, { error: 'no_users_selected' });
  if (!['grant','extend','revoke','delete'].includes(action)) return sendJson(res, 400, { error: 'bad_action' });
  let done = 0; const skipped = [];
  for (const id of ids) {
    const u = db.userById(id);
    if (!u || id === admin.id || cfg.isAdminEmail(u.email)) { skipped.push(id); continue; }
    try {
      if (action === 'delete') {
        const d = db.db.prepare('DELETE FROM users WHERE id = ? RETURNING id').get(id);
        if (d) { try { require('./mobile').invalidateUserCaches(id); } catch (_) {} done++; }
      } else if (action === 'revoke') {
        commerce.adminSetSubscription(id, 'free', 'canceled', null, 'admin'); done++;
      } else {
        commerce.adminSetSubscription(id, 'pro', 'active', months, 'admin'); done++;
      }
      db.audit(admin.id, 'admin_user_' + action + ':' + id + ':' + u.email, ip);
    } catch (_) { skipped.push(id); }
  }
  db.audit(admin.id, 'admin_users_bulk:' + action + ':' + done, ip);
  return sendJson(res, 200, { ok: true, done, skipped });
}
...
function walkEngineDb(node, out, seen, source, group) {
  if (!node) return;
  if (Array.isArray(node)) {
    for (const item of node) {
      if (item && typeof item === 'object' && typeof item.n === 'string') {
        ...
        if (!key) continue;
        if(seen.has(key)){const previous=out.find(x=>x.key===key);if(previous){previous.categories=Array.from(new Set((previous.categories||[previous.category]).concat(source)));}continue;}
        seen.add(key);
        out.push({ key, name: item.n, engineVideoId: typeof item.vid === 'string' ? item.vid : '', category: String(source || 'gym'), subCategory: String(item.category || group || '') });
      } else if (item && typeof item === 'object') {
        walkEngineDb(item, out, seen, source, group);
      }
    }
    return;
  }
  if (typeof node === 'object') {
    for (const k of Object.keys(node)) walkEngineDb(node[k], out, seen, source, group || k);
  }
}
...
function videoCatalogue() {
  ...
  if (src) {
    try {
      ...
      const wrapped = src + '\n;this.__EF_VIDEO_DBS = [' +
        '(typeof GYM_DB !== "undefined" ? {source:"gym",db:GYM_DB} : null),' +
        '(typeof HOME_DB !== "undefined" ? {source:"home",db:HOME_DB} : null),' +
        '(typeof MODULE_DB !== "undefined" ? {source:"modules",db:MODULE_DB} : null)' +
        '].filter(Boolean);';
      new vm.Script(wrapped, { filename: 'engine-db.js' }).runInContext(ctx, { timeout: 5000 });
      parsed = sandbox.__EF_VIDEO_DBS;
    } catch (e) {
  ...
  if (Array.isArray(parsed)) {
    parsed.forEach(function(x){ walkEngineDb(x.db, out, seen, x.source); });
  }
  if (!out.length && src) {
  ...
  if (!out.length) throw new Error('exercise_catalogue_unavailable');
  _catalogueCache = out;
  return out;
}
...
async function videos(req, res) {
  ...
  videoCatalogue().forEach((c) => {
    ...
    items.push({
      ...
      categories: c.categories || [c.category || 'gym'],
      category: c.category || 'gym',
      subCategory: c.subCategory || '',
      updatedAt: ov ? ov.updated_at : ''
    });
  });

  // Keep admin-added links visible even if the exercise is not authored in the engine yet.
  const known = new Set(items.map(function(x){ return x.key; }));
  rows.forEach(function(ov){
    if (known.has(ov.exercise_key) || !ov.video_id) return;
    counts.added++;
    items.push({key:ov.exercise_key,name:ov.exercise_name||ov.exercise_key,engineVideoId:'',videoId:ov.video_id,
      url:'https://www.youtube.com/watch?v='+ov.video_id,thumb:'https://img.youtube.com/vi/'+ov.video_id+'/mqdefault.jpg',
      status:'added',note:ov.note||'',category:'custom',updatedAt:ov.updated_at||''});
  });
...
async function announcementsSave(req, res, ip) {
  ...
  require('../lib/announcement-events').changed();
...
async function notificationsSend(req, res, ip) {
  ...
  const phone = String(b.phone || '').replace(/[^0-9+]/g,'').slice(0,40);
  const link = String(b.link || (phone ? 'tel:' + phone : '')).trim().slice(0, 400);
  ...
  const note = {
    id: 'notif_' + require('crypto').randomUUID(),
    title, body, type, target, image, link, phone,
  ...
  if (isScheduled) {
    ...
    delivery = await push.send({
      ...
      body: body,
      image: image || null,
      data: { notifId: note.id, type: type, link: link, phone: phone, expires_at: note.expires_at },
    });
  } else {
    delivery.skipped = 'push_not_requested';
  }
  note.delivery = delivery;
  const latest = settings.getJSON('admin_notifications', []) || [];
  settings.setJSON('admin_notifications',latest.map(x=>x.id===note.id?note:x));
...
async function auditLog(req, res) {
  ...
  } catch (e) { console.error('[admin.audit]',e.message); return sendJson(res,503,{error:'audit_unavailable'}); }
...
async function uploadImage(req, res, ip) {
  ...
  await new Promise((resolve, reject) => {
    req.on('data', chunk => {uploadBytes+=chunk.length;if(uploadBytes<=6*1024*1024)chunks.push(chunk);});
    req.on('end', resolve);
    req.on('error', reject);
  });
  if(uploadBytes>6*1024*1024)return sendJson(res,413,{error:'too_large'});
  ...
  const ext = imageData.subarray(0,3).equals(Buffer.from([255,216,255])) ? 'jpg'
    : imageData.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])) ? 'png'
    : imageData.subarray(0,4).toString()==='RIFF' && imageData.subarray(8,12).toString()==='WEBP' ? 'webp' : null;
  if(!ext)return sendJson(res,400,{error:'not_an_image'});
...
module.exports = {
  whoami, setupVerify, payments, approve, reject, receipt, stats, users, setUserSubscription, deleteUser, bulkUsers,
