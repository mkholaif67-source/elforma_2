...
function walkEngineDb(node, out, seen, source) {
  if (!node) return;
  if (Array.isArray(node)) {
    for (const item of node) {
      if (item && typeof item === 'object' && typeof item.n === 'string') {
        ...
        seen.add(key);
        out.push({ key, name: item.n, engineVideoId: typeof item.vid === 'string' ? item.vid : '', category: String(source || 'gym'), subCategory: String(item.category || '') });
      } else if (item && typeof item === 'object') {
        walkEngineDb(item, out, seen, source);
      }
    }
    return;
  }
  if (typeof node === 'object') {
    for (const k of Object.keys(node)) walkEngineDb(node[k], out, seen, source);
  }
}
