...
const { nutritionCache, catalogueCache } = require('../lib/cache-redis');
const enginePool = require('../lib/engine-pool');
const activeNutritionReads = new Map();
const mealComputations = new Map();
function computeMealPlanShared(userId, profile, inputs) {
  const key = userId + ':' + require('node:crypto').createHash('sha256')
    .update(JSON.stringify({profile,inputs})).digest('hex');
  let pending = mealComputations.get(key);
  if (!pending) {
    pending = enginePool.computeMealPlanAsync(profile, inputs)
      .finally(() => mealComputations.delete(key));
    mealComputations.set(key, pending);
  }
...
const progressEvents = require('../lib/progress-events');
const workoutHistoryAnalysis = require('../lib/workout-history-analysis');
...
// [INSTANT-OPEN] كاش رد bootstrap لكل مستخدم لكل يوم. أثقل طلب في التطبيق
// (~30 استعلام متسلسل لكل فتحة) بقى بيتقدم من الذاكرة في أجزاء من الملي ثانية.
// بيتمسح من dropCache مع كل كتابة، والـ TTL (20 ثانية) بيمسك أي تغيير خارجي
// نادر مش بيعدي على dropCache (زي اعتماد دفع من لوحة الأدمن).
const bootstrapCache = require('../lib/cache').createCache({ max: 2000, ttl: 45 * 1000 }) // [PERF] يتطابق مع client-side TTL;
// مؤشرات حية للأدمن عن صحة الكاش (نسبة الإصابات، الحجم، الإخلاء).
function bootstrapCacheStats(){ try{ return bootstrapCache.stats(); }catch(_){ return null; } }
function dropCache(userId){
  for (const ticket of activeNutritionReads.get(userId) || []) ticket.valid = false;
  try{ nutritionCache.invalidatePrefix('np:' + userId + ':'); }catch(_){}
  try{ nutritionCache.invalidatePrefix('wh:' + userId + ':'); }catch(_){}
  try{ bootstrapCache.invalidatePrefix('bs:' + userId + ':'); }catch(_){}
}
...
function n(v, min, max, fallback) { if(v==null||typeof v==='boolean'||(typeof v==='string'&&!v.trim()))return fallback; const x=Number(v); return Number.isFinite(x)?Math.max(min,Math.min(max,x)):fallback; }
function date(v) { const s=String(v||'').slice(0,10); return /^\d{4}-\d{2}-\d{2}$/.test(s)?s:new Date().toISOString().slice(0,10); }
/* [FIX-DAY-TZ] اليوم الافتراضي للكتابة لازم يطابق يوم القراءة في bootstrap:
   توقيت الجهاز (هيدر X-Tz-Offset) مع افتراضي +180 — مش تاريخ UTC بتاع السيرفر.
   قبل الإصلاح: أي حفظ (تغذية/وزن/مقاسات) بين 12 و3 الفجر بتوقيت القاهرة كان
   بينزل على اليوم السابق عند السيرفر، فالـ bootstrap مابيوريهوش ضمن «النهارده». */
function requestDay(req, rawDay) {
  const explicit = String(rawDay || '').slice(0, 10);
  if (/^\d{4}-\d{2}-\d{2}$/.test(explicit)) return explicit;
  const tz = Number(req.headers['x-tz-offset']);
  return new Date(Date.now() + (Number.isFinite(tz) ? tz : 180) * 60000).toISOString().slice(0, 10);
}
...
function profile(p) {
  ...
  o.availableFoods=Array.isArray(p.availableFoods)
    ...
    :[];
  o.preferredDays=workoutSchedule.normalizePreferredDays(p.preferredDays);
...
function trainingDayFromPlan(userId,options){
  try{
    ...
    // نفس منطق أسبوع البداية/الأسبوع العادي المستخدم في شاشة التمرين.
    const scheduleOptions=Object.assign({nowMs:Date.now(),offsetMs:3*3600000},{completedSessions:db.recentWorkoutSessions(userId,30)},options||{});
    const scheduled=workoutSchedule.trainingDayForNow(data,scheduleOptions);
...
// Phase 2: normalized, auditable explanation contract. Engines remain the
// source of truth; this adapter never invents a cause.
function buildDecisionExplanation(common, diagnosis, adjustment, smartCoach) {
  const d=diagnosis&&typeof diagnosis==='object'?diagnosis:null;
  const a=adjustment&&typeof adjustment==='object'?adjustment:null;
  const primary=d&&d.primary&&typeof d.primary==='object'?d.primary:null;
  const action=a&&a.action?String(a.action):'hold';
  const observed={source:primary?'plateau-diagnostics':'diet-periodization',weightSamples:Array.isArray(common&&common.weights)?common.weights.length:0,loggedDays:Number(common&&common.loggedDays)||0,adherencePct:Number.isFinite(Number(common&&common.adherencePct))?Number(common.adherencePct):null,adherenceSource:common&&common.adherenceSource||'unknown',weeklyRateKg:d&&d.signals&&Number.isFinite(Number(d.signals.actualRateKg))?Number(d.signals.actualRateKg):null,state:d&&d.state||'unknown',confidence:d&&d.confidence||'low'};
  const decision={action,currentCals:a&&Number.isFinite(Number(a.currentCals))?Number(a.currentCals):null,suggestedCals:a&&Number.isFinite(Number(a.suggestedCals))?Number(a.suggestedCals):null,deltaCals:a&&Number.isFinite(Number(a.deltaCals))?Number(a.deltaCals):0,blockedBy:a&&a.blockedBy?String(a.blockedBy):null,smartCoach:smartCoach!==false};
  const finding=primary&&String(primary.findingAr||'').trim(), actionAr=primary&&String(primary.actionAr||'').trim(), labelAr=primary&&String(primary.labelAr||'').trim(), fallback=a&&String(a.reason||'').trim();
  const collecting=!!(d&&d.state==='collecting');
  const available=!collecting&&!!(finding||actionAr||fallback);
  return {available,code:available?'engine_explanation':'insufficient_data',observed,decision,explanation:available?{source:primary?'plateau-diagnostics':'diet-periodization',causeKey:primary&&primary.key?String(primary.key):null,labelAr:labelAr||(action==='hold'?'الخطة ثابتة حاليا':'قرار المدرب'),findingAr:finding||null,actionAr:actionAr||fallback||null,confidence:observed.confidence}:null};
}

// Phase 3: session-only readiness. It is keyed by account + active session,
// never changes a plan or progress history, and survives an app restart.
function buildReadinessDecision(input){
  input=input&&typeof input==='object'?input:{};
  const rest=input.isRest===true, acwr=input.acwr!=null&&Number.isFinite(Number(input.acwr))?Number(input.acwr):null, sleep=input.sleepHours!=null&&Number.isFinite(Number(input.sleepHours))?Number(input.sleepHours):null, stress=Number(input.stress)||1;
  let score=100;
  if(acwr!=null){if(acwr>1.5)score-=38;else if(acwr>1.3)score-=22;else if(acwr<.8)score-=8;}
  if(sleep!=null&&sleep<6)score-=20;
  if(stress>=5)score-=15;else if(stress>=3)score-=7;
  score -= (Number(input.fatigue)||0)*20;
  score -= (Number(input.pain)||0)*25;
  score -= (Number(input.readiness)||0)*20;
  if(input.pain===2 || input.readiness===2 || input.fatigue===2)score=Math.min(score,40);
  else if(input.pain===1 || input.readiness===1 || input.fatigue===1)score=Math.min(score,69);
  score=Math.max(0,Math.min(100,Math.round(score)));
  const zone=score<45?'low':score<70?'caution':'ready';
  return {score,zone,action:rest?'keep_rest':zone==='low'?'reduce_load':zone==='caution'?'reduce_load':'normal',canTrain:!rest,loadMultiplier:rest?0:zone==='low'?.7:zone==='caution'?.85:1,setMultiplier:rest?0:zone==='low'?.5:zone==='caution'?.75:1,sessionOnly:true,planMutation:false};
}
function _readinessTable(){db.db.exec(`CREATE TABLE IF NOT EXISTS session_readiness(user_id INTEGER NOT NULL,session_id INTEGER NOT NULL,day_key TEXT NOT NULL,source_json TEXT NOT NULL,decision_json TEXT NOT NULL,explanation_json TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,PRIMARY KEY(user_id,session_id),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`);}
function _readinessRow(userId,sessionId){try{_readinessTable();const r=db.db.prepare('SELECT * FROM session_readiness WHERE user_id=? AND session_id=?').get(userId,sessionId);if(!r)return null;return {sessionId:Number(r.session_id),dayKey:r.day_key,source:parse(r.source_json,{}),decision:parse(r.decision_json,{}),explanation:parse(r.explanation_json,{}),createdAt:r.created_at,updatedAt:r.updated_at};}catch(_){return null;}}
function _readinessForSession(userId,session,checkIn=null){
  if(!session||session.status!=='active')return {status:'not_active',readiness:null};
  const cached=_readinessRow(userId,session.id);if(cached&&cached.source.checkIn)return {status:'ready',readiness:cached};
  if(!checkIn)return {status:'needs_checkin',readiness:null};
  const row=db.mobileProfile(userId),p=row?parse(row.profile_json,{}):{};
  const day=_sessionDayExercises(userId,session.day_key||''), exercises=day&&Array.isArray(day.exercises)?day.exercises:[], rest=!exercises.length;
  const nowWeek=Math.floor(Date.now()/604800000), hist=[], sessions=(db.recentWorkoutSessions(userId,60)||[]).filter(x=>x.status==='completed'), by=db.workoutSetsForSessions(sessions.map(x=>x.id));
  sessions.forEach(x=>{const week=Math.floor((Date.parse(x.finished_at||x.started_at)||0)/604800000);(by.get(x.id)||[]).filter(z=>z.completed===1||z.completed===true).forEach(z=>hist.push({week,weight:Number(z.weight)||0,reps:Number(z.reps)||0,sets:1}));});
  let acwr=null;try{acwr=coachEngine.acwrFrom(hist,nowWeek);}catch(_){ }
  const decision=buildReadinessDecision({isRest:rest,acwr,...checkIn});
  const source={type:'session_readiness',checkIn,profile:{sleep:p.sleep||null,stress:p.stress||null},training:{acwr,acwrZone:coachEngine.acwrZone(acwr),completedSessions:sessions.length},day:{dayKey:session.day_key||'',isRest:rest}};
  const explanation={source:'session-readiness',titleAr:rest?'يوم راحة — مفيش تمرين اليوم':decision.zone==='low'?'جاهز بقدرة أقل النهارده':decision.zone==='caution'?'خفّف الحمل وخليك مركز':'جاهز للتمرين بالحمل المعتاد',reasonAr:rest?'الجدول الحالي يوم راحة، فمش هنحوّله لتمرين.':decision.zone==='low'?'تراكم الحمل أو الاستشفاء الحالي منخفض؛ خفّض الحمل بدل ما نغيّر خطتك.':decision.zone==='caution'?'فيه إشارة تعب متوسطة؛ خفّف الحمل في الجلسة دي فقط.':'إشارات الاستشفاء والحمل داخل النطاق المناسب للجلسة.',confidence:sessions.length>=3?'medium':'low'};
  explanation.reasonAr=checkIn.pain>0?'خفف المجهود وتجنب أي حركة تسبب لك ألم. التعديل للجلسة دي فقط.':decision.zone==='ready'?'إجاباتك مناسبة لجلسة اليوم. كمل خطتك كالمعتاد.':'هنقلل الحمل وعدد المجموعات في الجلسة دي حسب إجاباتك، وخطتك الأساسية زي ما هي.';
  explanation.confidence='self_reported';
  const result={sessionId:Number(session.id),dayKey:session.day_key||'',source,decision,explanation,createdAt:new Date().toISOString()};_readinessTable();const now=new Date().toISOString();db.db.prepare('INSERT INTO session_readiness(user_id,session_id,day_key,source_json,decision_json,explanation_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(user_id,session_id) DO UPDATE SET source_json=excluded.source_json,decision_json=excluded.decision_json,explanation_json=excluded.explanation_json,updated_at=excluded.updated_at').run(userId,session.id,result.dayKey,JSON.stringify(source),JSON.stringify(decision),JSON.stringify(explanation),now,now);return {status:'ready',readiness:_readinessRow(userId,session.id)||result};
}
async function readinessGet(req,res){const u=user(req,res);if(!u)return;const q=new URL(req.url,'http://localhost').searchParams,id=Math.round(n(q.get('sessionId'),1,1e12,0)),session=id?db.workoutSession(id,u.id):db.activeWorkoutSession(u.id);return sendJson(res,200,{ok:true,dayReset:!session,..._readinessForSession(u.id,session)});}
async function readinessSubmit(req,res){
  const u=user(req,res);if(!u)return;
  const b=await readJsonBody(req),id=Math.round(n(b.sessionId,1,1e12,0)),session=db.workoutSession(id,u.id);
  if(!session)return sendJson(res,404,{error:'session_not_found'});
  if(session.status!=='active')return sendJson(res,409,{error:'session_not_active'});
  const existed=!!_readinessRow(u.id,id)?.source?.checkIn;
  if(existed)return sendJson(res,200,{ok:true,idempotent:true,..._readinessForSession(u.id,session)});
  const c=b.checkIn;
  if(!c || typeof c.sleepHours!=='number' || !Number.isFinite(c.sleepHours) || c.sleepHours<0 || c.sleepHours>16 ||
     !['fatigue','pain','readiness'].every(k=>Number.isInteger(c[k])&&c[k]>=0&&c[k]<=2))
    return sendJson(res,422,{error:'readiness_answers_required'});
  const checkIn={sleepHours:c.sleepHours,fatigue:c.fatigue,pain:c.pain,readiness:c.readiness};
  return sendJson(res,200,{ok:true,idempotent:false,..._readinessForSession(u.id,session,checkIn)});
}
...
function activeAnnouncements(place, ctx){
  ...
  return list.filter(function(a){
    ...
    return {
      ...
      layout: ['wide','portrait','imageOnly'].includes(a.layout) ? a.layout : 'wide',
      imageAction: a.imageAction === 'phone' ? 'phone' : 'link',
...
function _enrichPlanVideoIds(planObj, options) {
  if (!planObj || !planObj.data) return planObj;
  const viewOffsetMs = options && Number.isFinite(options.offsetMs) ? options.offsetMs : 3*3600000;
  try { workoutSchedule.materializePlan(planObj.data, {nowMs:Date.now(), offsetMs:viewOffsetMs, completedSessions:options&&options.completedSessions}); } catch(_scheduleViewErr) {}
  try {
    ...
    var buckets = [planObj.data.plan, planObj.data._onboardingWeek, planObj.data._regularWeek, planObj.data.days, planObj.data.schedule, planObj.data.weeks];
    var seen = [];
      ...
      dayList.forEach(function(day) {
        ...
        [day.exercises, day.items, day.warmActivation, day.stretchCooldown].forEach(function(list) {
          ...
          list.forEach(function(ex) {
            ...
            // Resolve every saved entry, including owner-approved replacements.
            // Keep admin overrides/removals ahead of the shipped catalogue.
            var resolved = guard.resolve(id, ex.mu || ex.muscle || ex.group,
              ex.n || ex.name || ex.exerciseName || ex.key || ex.id);
            ex.vid = resolved.videoId;
            ex.videoId = resolved.videoId;
            ex.videoUrl = resolved.url;
            ex.videoThumb = resolved.thumb;
            ex.videoSource = resolved.source;
            ex.videoMissing = resolved.missing;
          });
        });
      });
    });
...
function _withUnifiedEnergy(profileObj, userId){
  ...
  try{
    const living=energy.contextForUser(profileObj,userId);
    const e=energy.unifiedFromMobile(living,living.__extras);
...
function subscriptionView(raw, req) {
...
function sendReadSnapshot(res, payload) {
  const timing=require('../lib/request-reads').stats();
  res.setHeader('Cache-Control','private, no-store');
  if(timing && process.env.EF_PERF_HEADERS==='1') {
    res.setHeader('Server-Timing', `sql;dur=${timing.sqlMs}, queries;desc="${timing.queries}", readhits;desc="${timing.hits}"`);
  }
  return sendJson(res,200,payload);
}
async function accountSnapshot(req,res) {
  return require('../lib/request-reads').withRequestReads(() => {
    const u=user(req,res); if(!u)return;
    const raw=db.getSubscription(u.id)||{plan:'free',status:'active'};
    const p=db.mobileProfile(u.id);
    return sendReadSnapshot(res,{ok:true,
      user:{id:u.id,email:u.email,phone:u.phone,name:u.name,verified:!!u.verified,
        emailVerified:!!u.verified,phoneVerified:!!u.phone_verified,created_at:u.created_at},
      subscription:subscriptionView(raw,req),profile:p?parse(p.profile_json,null):null,
      smartCoach:prefs.smartCoachEnabled(u.id),
      announcements:activeAnnouncements(null,{isPro:activeSub(raw),plan:activeSub(raw)?raw.plan:'free'}),
    });
  });
}
async function subscriptionSnapshot(req,res) {
  return require('../lib/request-reads').withRequestReads(() => {
    const u=user(req,res); if(!u)return;
    const raw=db.getSubscription(u.id)||{plan:'free',status:'active'};
    return sendReadSnapshot(res,{ok:true,subscription:subscriptionView(raw,req)});
  });
}

async function bootstrap(req,res){
  return require('../lib/request-reads').withRequestReads(() => bootstrapPayload(req,res));
}
function bootstrapPayload(req,res){
  const u=user(req,res); if(!u)return; const _urlObj=new URL(req.url,'http://localhost'); const dayParam=_urlObj.searchParams.get('day');
  const rawTzOffset=Number(req.headers['x-tz-offset']);
  const tzOffset=Number.isFinite(rawTzOffset)?Math.max(-720,Math.min(840,Math.round(rawTzOffset))):180;
  const localDay=new Date(Date.now()+tzOffset*60000).toISOString().slice(0,10);
  const today=date(dayParam||localDay);
  // [INSTANT-OPEN] fresh=1 بيتخطى الكاش عمدًا (سحبة تحديث صريحة من المستخدم).
  const _fresh=_urlObj.searchParams.get('fresh')==='1';
  const _cacheKey='bs:'+u.id+':'+today+':tz'+tzOffset;
  if(!_fresh){ const _hit=bootstrapCache.get(_cacheKey); if(_hit) return sendJson(res,200,_hit); }
  const p=db.mobileProfile(u.id), session=db.activeWorkoutSession(u.id), raw=db.getSubscription(u.id)||{plan:'free',status:'active'};
  const payload={ok:true,user:{id:u.id,email:u.email,phone:u.phone,name:u.name,verified:!!u.verified,emailVerified:!!u.verified,phoneVerified:!!u.phone_verified},
    subscription:subscriptionView(raw,req),
    smartCoach:prefs.smartCoachEnabled(u.id),
    ...
    profile:p?_withTodayTrainingFlag(_withUnifiedEnergy(parse(p.profile_json,null),u.id),u.id):null,workoutPlan:currentPlanForProfile(u.id,{offsetMs:tzOffset*60000}),activeSession:session?Object.assign({},session,{sets:db.workoutSets(session.id)}):null,recentSessions:db.recentWorkoutSessions(u.id,12),nutritionToday:nutrition(db.nutritionDay(u.id,today)),weights:db.recentWeights(u.id,30),measurements:db.recentMeasurements(u.id,30)};
  const timing=require('../lib/request-reads').stats();
  if(timing && process.env.EF_PERF_HEADERS==='1') {
    res.setHeader('Server-Timing', `sql;dur=${timing.sqlMs}, queries;desc="${timing.queries}", readhits;desc="${timing.hits}"`);
  }
  bootstrapCache.set(_cacheKey,payload);
  return sendJson(res,200,payload);
}
const WORKOUT_PROFILE_FIELDS=['gender','age','height','weight','goal','experience','equipment','trains','trainingDays','preferredDays','trainingMinutes','sleep','stress','dailyActivity','injuries','weakPoints','activeModules'];
function workoutProfileSignature(p){return JSON.stringify(WORKOUT_PROFILE_FIELDS.map(k=>[k,p&&p[k]!=null?p[k]:null]));}
function currentPlanForProfile(userId, options){
  const active=plan(db.activeWorkoutPlan(userId));
  if(active&&active.data){
    const row=db.mobileProfile(userId),p=row?parse(row.profile_json,{}):{};
    const living=energy.contextForUser(p,userId),currentEnergy=energy.unifiedFromMobile(living,living.__extras)||{};
    active.data._metrics=Object.assign({},active.data._metrics||{},currentEnergy, {recoveryScore:require('../lib/workout-recovery').score(p)});
    active.data._needsProfileRefresh=!!active.data._profileWorkoutSignature&&active.data._profileWorkoutSignature!==workoutProfileSignature(p);
  }
  const result=_enrichPlanVideoIds(active, Object.assign({},options||{},{completedSessions:db.recentWorkoutSessions(userId,30)}));
  if(result&&result.data&&!userSubActive(userId)){
    const d=result.data,days=Array.isArray(d.plan)?d.plan:[];
    const first=days.find(x=>x&&!x.isRest&&Array.isArray(x.exercises)&&x.exercises.length);
    result.data={key:d.key,name:d.name,desc:d.desc,_metrics:d._metrics,_locked:true,_needsProfileRefresh:d._needsProfileRefresh,
      trainDaysTotal:days.filter(x=>x&&!x.isRest).length,previewDay:first||null};
  }
  return result;
}
async function prepareSavedPlans(userId,p,req){
  const living=energy.contextForUser(p,userId);
  let active=db.activeWorkoutPlan(userId);
  const previous=active?parse(active.plan_json,{}):null;
  if(p.trains!==false && p.trainingDays>0 && (!previous || previous._profileWorkoutSignature!==workoutProfileSignature(p))){
    const wp=require('./workout').cleanProfile({profile:{gender:p.gender,age:p.age,height:p.height,
      weight:p.weight,goal:require('../lib/goal-vocabulary').MOBILE_TO_WORKOUT[p.goal],exp:p.experience,
      equip:p.equipment,days:p.trainingDays,time:p.trainingMinutes,daily:p.dailyActivity,
      sleep:p.sleep,stress:p.stress,injuries:p.injuries,weak:p.weakPoints,
      activeModules:p.activeModules,preferredDays:p.preferredDays}});
    const full=require('../lib/workout-plan-finisher').computeStablePlan(wp).out;
    if(!full||!Array.isArray(full.plans)||!full.plans.length)throw new Error('workout_generation_failed');
    const selected=full.plans.find(x=>previous&&x.key===previous.key)||full.plans.find(x=>x.rec)||full.plans[0];
    if(!Array.isArray(selected.plan)||!selected.plan.some(x=>Array.isArray(x.exercises)&&x.exercises.length))throw new Error('workout_empty');
    try{require('../lib/video-guard').guardPlan(full);require('../lib/warmup-activation').attachToPlan(full);require('../lib/stretch-cooldown').attachToPlan(full);}catch(_){}
    energy.unifyPlanMetrics(full,wp,living);
    selected._metrics=full.metrics;
    selected._profileWorkoutSignature=workoutProfileSignature(p);
    const offset=Number(req.headers['x-tz-offset']);
    workoutSchedule.alignPlan(selected,p.preferredDays,{nowMs:previous&&Number(previous._scheduleStartedMs)>0?Number(previous._scheduleStartedMs):Date.now(),offsetMs:Number.isFinite(offset)?offset*60000:10800000,completedSequenceCount:previous&&Number(previous._completedSequenceCount)||0,regularCompletedSequenceCount:previous&&Number(previous._regularCompletedSequenceCount)||0,firstWorkoutException:previous?previous._firstWorkoutExceptionActive===true:true});
    db.saveWorkoutPlan(userId,selected.key,JSON.stringify(selected));
    active=db.activeWorkoutPlan(userId);
    if(!active)throw new Error('workout_write_not_persisted');
  }
  const ctx=bridge.buildEngineContext(living,living.__extras);
  ctx.inputs.mealFavorites=require('../lib/meal-food-preferences').read(userId);
  ctx.profile.isTrainingDay=trainingDayFromPlan(userId)===true;
  ...
  const rawOffset=parseInt(req.headers['x-tz-offset'],10);
  const tzOffset=Number.isFinite(rawOffset)?rawOffset:cfg.tzForCountry(cfg.countryFromReq(req)).offsetMinutes;
  const cal=require('../lib/nutrition-calendar').calendar(p.nutritionStartedAt,tzOffset);
  ctx.inputs['inp-week']=cal.week;ctx.inputs.dayOfCycle=cal.day;
  ctx.inputs.isTrainingDay=ctx.profile.isTrainingDay;
  if(living.coachTargetCals>0){ctx.profile.coachTargetCals=living.coachTargetCals;ctx.inputs.coachTargetCals=living.coachTargetCals;}
  // [PHASE-0-PERF] عبر الـ worker pool (مع dedupe لكل مستخدم) بدل التنفيذ
  // المتزامن على الـ main thread — تشغيلة المحرك 631ms كانت بتوقف كل الطلبات.
  const result=await computeMealPlanShared(userId,ctx.profile,ctx.inputs);
  result._engineVersion=require('../lib/nutrition-computation-store').version;
  if(!result.plan||!Array.isArray(result.plan.meals)||!result.plan.meals.length)throw new Error('nutrition_generation_failed');
  // A generated plan is NOT a consumed-food log. Keep the two separate.
  db.db.exec(`CREATE TABLE IF NOT EXISTS prepared_nutrition_plans(user_id INTEGER PRIMARY KEY,
    profile_json TEXT NOT NULL,plan_json TEXT NOT NULL,created_at TEXT NOT NULL,
    FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`);
  db.db.prepare(`INSERT INTO prepared_nutrition_plans(user_id,profile_json,plan_json,created_at) VALUES(?,?,?,?)
    ON CONFLICT(user_id) DO UPDATE SET profile_json=excluded.profile_json,plan_json=excluded.plan_json,created_at=excluded.created_at`)
    .run(userId,JSON.stringify({profile:ctx.profile,inputs:ctx.inputs}),JSON.stringify(result),new Date().toISOString());
  dropCache(userId);
  return {workoutReady:p.trains===false||!!active,nutritionReady:true};
}
async function saveProfile(req,res){
  const u=user(req,res);if(!u)return;const b=await readJsonBody(req);
  const oldRow=db.mobileProfile(u.id),oldProfile=oldRow?parse(oldRow.profile_json,{}):{};
  const input=b.profile||b;
  if(b.patch===true&&(!input||typeof input!=='object'||Array.isArray(input)))return sendJson(res,400,{error:'بيانات التعديل غير صحيحة'});
  const merged=b.patch===true?Object.assign({},oldProfile,input):input;
  const p=profile(merged);
  p.nutritionStartedAt=oldProfile.nutritionStartedAt||(oldRow&&oldRow.updated_at)||new Date().toISOString();
  if(p.preferredDays.length && p.preferredDays.length!==p.trainingDays)return sendJson(res,400,{error:'اختر أيامًا محددة تساوي عدد أيام التدريب، أو استخدم التوزيع التلقائي'});
  // Stamp legacy plans BEFORE editing; nutrition-only changes never restart training.
  const oldPlan=db.activeWorkoutPlan(u.id);
  if(oldPlan){
    const data=parse(oldPlan.plan_json,{});
    if(!data._profileWorkoutSignature){
      data._profileWorkoutSignature=workoutProfileSignature(oldProfile);
      db.db.prepare('UPDATE workout_plans SET plan_json=? WHERE id=? AND user_id=?').run(JSON.stringify(data),oldPlan.id,u.id);
    }
  }
  ...
  let readiness=null;
  if(b.preparePlans===true && p.onboardingComplete){
    try{readiness=await prepareSavedPlans(u.id,p,req);}
    catch(err){console.error('[prepare-plans]',String(err&&err.message||err));
      return sendJson(res,503,{error:'plan_preparation_failed',profileSaved:true,profile:_withUnifiedEnergy(p,u.id),readiness});}
  }
  ...
  // خطة التغذية بتتحسب في الخلفية بعد حفظ البروفايل مباشرةً
  // No second background build: the old precompute used a different calendar,
  // ignored the active training schedule and could overwrite an exact prepared plan.
  return sendJson(res,200,{ok:true,profile:_withUnifiedEnergy(p,u.id),readiness});
}
/* [FIX-CHAT-EXIT] تجهيز الخطط له مكانه الصحيح: صفحة التحليل بعد الشات —
   مش جوه طلب حفظ البروفايل اللي المستخدم واقف مستنيه في الشات.
   المسار ده بيشغل نفس prepareSavedPlans (نفس المنطق ونفس أقفال الأمان)،
   مع dedupe لكل مستخدم: لو طلبين وصلوا في نفس اللحظة (صفحة التحليل +
   إعادة محاولة) بيتشاركوا نفس التشغيلة بدل ما المحرك يتشغل مرتين. */
const planPrepInFlight = new Map();
function prepareSavedPlansShared(userId,p,req){
  let flight=planPrepInFlight.get(userId);
  if(!flight){
    flight=Promise.resolve().then(()=>prepareSavedPlans(userId,p,req))
      .finally(()=>{ if(planPrepInFlight.get(userId)===flight) planPrepInFlight.delete(userId); });
    planPrepInFlight.set(userId,flight);
  }
  return flight;
}
async function preparePlansNow(req,res){
  const u=user(req,res);if(!u)return;
  if(!rateLimit('plans-prepare:'+u.id,6,60000))return sendJson(res,429,{error:'طلبات كتيرة — استنى شوية'});
  const row=db.mobileProfile(u.id),p=row?parse(row.profile_json,null):null;
  if(!p||!p.onboardingComplete)return sendJson(res,400,{error:'profile_incomplete'});
  try{
    const readiness=await prepareSavedPlansShared(u.id,p,req);
    return sendJson(res,200,{ok:true,readiness});
  }catch(err){
    console.error('[prepare-plans]',String(err&&err.message||err));
    return sendJson(res,503,{error:'plan_preparation_failed'});
  }
}
async function activatePlan(req,res){
  ...
  // [NEW-USER-START] preferredDays controls the permanent weekly calendar,
  // but a genuine new account always receives session 1 immediately. This is
  // a one-time exception only; after completing it, the selected weekdays are
  // authoritative on Home, Workout and nutrition day classification.
  ...
  let aligned;
  try {
    const existing=db.activeWorkoutPlan(u.id),previous=existing?parse(existing.plan_json,{}):{};
    const requestedKey=String(b.key||planData.key||'custom').slice(0,80);
    const previousStart=Number(previous._scheduleStartedMs);
    const continuation=!!(existing&&String(existing.plan_key||'')===requestedKey&&Number.isFinite(previousStart));
    const completedRows=db.recentWorkoutSessions(u.id,30).filter(x=>x&&String(x.status||'').toLowerCase()==='completed');
    const firstWorkoutException=existing
      ? previous._firstWorkoutExceptionActive===true&&completedRows.length===0
      : completedRows.length===0;
    aligned = workoutSchedule.alignPlan(planData, selectedDays, {
      nowMs: continuation?previousStart:Date.now(), offsetMs: offsetMs,
      completedSequenceCount:continuation?Number(previous._completedSequenceCount)||0:0,
      regularCompletedSequenceCount:continuation?Number(previous._regularCompletedSequenceCount)||0:0,
      firstWorkoutException
    });
    const profileRow=db.mobileProfile(u.id);
    planData._profileWorkoutSignature=workoutProfileSignature(profileRow?parse(profileRow.profile_json,{}):{});
    if(continuation)planData._continuedFromPlanId=existing.id;
  } catch (scheduleError) {
  ...
  try { var _vg=require('../lib/video-guard'); _vg.guardPlan(planData); if(Array.isArray(planData._regularWeek)) _vg.guardPlan({plan:planData._regularWeek}); if(Array.isArray(planData._onboardingWeek)) _vg.guardPlan({plan:planData._onboardingWeek}); } catch(_vgErr) {}
  ...
  return sendJson(res,200,{ok:true,id,workoutPlan:_enrichPlanVideoIds(plan(db.activeWorkoutPlan(u.id)),{offsetMs,completedSessions:db.recentWorkoutSessions(u.id,30)}),scheduleStartDate:_planStartDate,firstWorkoutDate:aligned.firstWorkoutDate});
}
...
function _sessionDayExercises(userId, dayKey){
  try {
    const pl = _enrichPlanVideoIds(plan(db.activeWorkoutPlan(userId)),{completedSessions:db.recentWorkoutSessions(userId,30)});
...
async function startSession(req,res){
  ...
  const p=db.activeWorkoutPlan(u.id);const started=db.startWorkoutSessionOnce(u.id,p&&p.id,dayKey,String(b.dayName||dayKey).slice(0,120),b.startedAt||null);const id=started.id;
  return sendJson(res,started.created?201:200,Object.assign({ok:true,resumed:!started.created,alreadyCompleted:false,session:db.workoutSession(id,u.id)}, _sessionDayExercises(u.id, dayKey)||{}));
}
...
async function finishSession(req,res){
  ...
  if(wasActive){
    db.finishWorkoutSession(id,u.id,b.finishedAt||null,Math.round(n(b.durationSec,0,86400,0)),String(b.notes||'').slice(0,1000));
    const finished=db.workoutSession(id,u.id),activePlan=db.activeWorkoutPlan(u.id);
    if(finished&&activePlan){
      const planData=parse(activePlan.plan_json,{});
      if(workoutSchedule.recordCompletion(planData,finished)){
        db.db.prepare('UPDATE workout_plans SET plan_json=?, updated_at=? WHERE id=? AND user_id=?').run(JSON.stringify(planData),new Date().toISOString(),activePlan.id,u.id);
      }
    }
  }
  dropCache(u.id);
...
async function saveNutrition(req,res){
  const u=user(req,res);if(!u)return;const b=await readJsonBody(req,2*1024*1024),d=requestDay(req,b.day),meals=Array.isArray(b.meals)?b.meals.slice(0,30):[];
...
async function saveWeight(req,res){
  ...
  db.saveWeight(u.id,requestDay(req,b.day),w,String(b.note||'').slice(0,300));dropCache(u.id);return sendJson(res,200,{ok:true,weights:db.recentWeights(u.id,30)});
}
async function saveMeasurement(req,res){
  const u=user(req,res);if(!u)return;const b=await readJsonBody(req),d=requestDay(req,b.day);
  const value={
...
async function foodSearch(req,res){
  ...
  // قبل كدا كل حرف بيكتبه أي متدرب كان بيفلتر كتالوج 2700 صنف من الأول جوّا الـ vm.
  const mealSlot=url.searchParams.get('mealSlot');
  if(mealSlot&&!['breakfast','snack','pre','lunch','dinner','main'].includes(mealSlot))return sendJson(res,400,{error:'invalid_meal_slot'});
  const _fk='fs:'+cat+':'+diet+':'+health.slice().sort().join('|')+':'+(mealSlot||'')+':'+q;
  let base=catalogueCache.get(_fk);
  if(!base){ base=nutritionHost.searchFoods({query:q,category:cat,diet,health,mealSlot}); catalogueCache.set(_fk,base); }
  const policy=require('../lib/nutrition-owner-policy');
  const foods=base.filter(food=>!mealSlot||policy.favoriteAllowed(mealSlot,food)).map(food=>{
...
async function foodPreferences(req,res){
  ...
  const grouped=require('../lib/meal-food-preferences').read(u.id);
  const mealFavorites={};for(const slot of Object.keys(grouped))mealFavorites[slot]=nutritionHost.foodsByIds(grouped[slot]);
  return sendJson(res,200,{ok:true,mealFavorites,foods,favorites:foods.filter(food=>food.favorite),recent:foods.filter(food=>food.useCount>0).sort((a,b)=>String(b.lastUsedAt||'').localeCompare(String(a.lastUsedAt||''))).slice(0,30)});
}
async function saveFoodPreference(req,res){
  ...
  if(b.mealSlot!==undefined){
    const prefs=require('../lib/meal-food-preferences');
    if(!prefs.slots.includes(b.mealSlot)||typeof b.favorite!=='boolean')return sendJson(res,400,{error:'invalid_meal_preference'});
    const selected=nutritionHost.foodsByIds([foodId])[0];
    if(!selected)return sendJson(res,400,{error:'unknown_food'});
    if(b.favorite&&!require('../lib/nutrition-owner-policy').favoriteAllowed(b.mealSlot,selected))return sendJson(res,400,{error:'الصنف غير مناسب لقواعد الوجبة المختارة'});
    try{const groups=prefs.set(u.id,b.mealSlot,foodId,b.favorite);dropCache(u.id);return sendJson(res,200,{ok:true,mealFavorites:groups});}
...
async function progressStory(req,res){
  const u=user(req,res);if(!u)return;
  if(!rateLimit('progress-story:'+u.id,30,60000))return sendJson(res,429,{error:'طلبات كثيرة في وقت قصير'});
  const url=new URL(req.url,'http://localhost');
  const requested=String(url.searchParams.get('month')||'').trim();
  const month=/^\\d{4}-\\d{2}$/.test(requested)?requested:null;
  try{return sendJson(res,200,{ok:true,...progressEvents.story(u.id,month)});}
  catch(e){return sendJson(res,500,{error:'progress_story_unavailable'});
  }
}
async function acknowledgeProgressMoments(req,res){
  const u=user(req,res);if(!u)return;
  if(!rateLimit('progress-story-ack:'+u.id,60,60000))return sendJson(res,429,{error:'طلبات كثيرة في وقت قصير'});
  const b=await readJsonBody(req);
  try{return sendJson(res,200,{ok:true,...progressEvents.markSeen(u.id,b&&b.ids)});}
  catch(e){return sendJson(res,400,{error:'invalid_progress_event_ids'});}
}
async function workoutHistory(req,res){
  ...
  // Presentation-only weekly drill-down. It reuses the completed sessions and
  // set history above; no workout-plan or Smart Coach decision is changed.
  const tzOffset=Number(req.headers['x-tz-offset']);
  const trainingWeeks=workoutHistoryAnalysis.buildTrainingWeeks(
    sessions,metadata,groupLabels,Number.isFinite(tzOffset)?tzOffset:180
  );
  const payload={ok:true,analytics:{totalSessions:sessions.length,totalSets,totalVolume,avgDuration},weeks:[...weekBuckets.values()],trainingWeeks,bestLifts,coach,sessions};
...
async function nutritionPlan(req,res){
  ...
  const _ck='np:'+u.id+':'+JSON.stringify(require('../lib/meal-food-preferences').read(u.id))+':'+(wantPlan?'1':'0')+':'+String(row.updated_at||'')+':'+_localDate+':d'+_dayOfCycle+':c'+((_coachSaved&&_coachSaved.cals)||0);
  ...
  const livingProfile=energy.contextForUser(p,u.id);
  const ctx=bridge.buildEngineContext(livingProfile,livingProfile.__extras);
  ctx.inputs.mealFavorites=require('../lib/meal-food-preferences').read(u.id);

  // [EGY-v70] احقن يوم الدورة + رقم الأسبوع في مدخلات المحرك (تنويع يومي دورة 4 أيام + إزاحة أسبوعية).
  const _startedAt=p.nutritionStartedAt||row.updated_at||null;
  ...
  const _trFromPlan=trainingDayFromPlan(u.id,{nowMs:Date.now()+_safeOffset*86400000,offsetMs:_tz.offsetMinutes*60000});
  ...
  if(livingProfile.coachTargetCals>0){
    ctx.profile.coachTargetCals=livingProfile.coachTargetCals;
    if(ctx.inputs)ctx.inputs.coachTargetCals=livingProfile.coachTargetCals;
  }

  let computed;
  const computationStore=require('../lib/nutrition-computation-store');
  const computationHash=computationStore.digest(ctx.profile,ctx.inputs);
  try{
    let saved=wantPlan?computationStore.read(u.id,_localDate,computationHash):null;
    if(saved)res.setHeader('X-Plan-Cache','STORED');
    // Exact context match only: never reuse a different day/account/profile/pantry.
    if(wantPlan&&!saved)try{
      const row=db.db.prepare('SELECT profile_json,plan_json FROM prepared_nutrition_plans WHERE user_id=?').get(u.id);
      if(row&&row.profile_json===JSON.stringify({profile:ctx.profile,inputs:ctx.inputs})){const candidate=parse(row.plan_json,null);if(candidate&&candidate._engineVersion===computationStore.version)saved=candidate;}
    }catch(_){}
    if (wantPlan && !saved) {
      const ticket = { valid: true };
      let tickets = activeNutritionReads.get(u.id);
      if (!tickets) { tickets = new Set(); activeNutritionReads.set(u.id, tickets); }
      tickets.add(ticket);
      try {
        computed = await computeMealPlanShared(u.id, ctx.profile, ctx.inputs);
        // Reads can now overlap writes. Never restore a response invalidated
        // while its worker was running; auth/subscription gating stays live.
        const currentUser = auth.currentUser(parseCookies(req));
        if (!currentUser || currentUser.id !== u.id) return sendJson(res,401,{error:'unauthenticated'});
        const currentProfile = db.mobileProfile(u.id);
        if (!ticket.valid || !currentProfile || currentProfile.profile_json !== row.profile_json) {
          return sendJson(res,409,{error:'request_superseded',message:'بياناتك اتحدثت، افتح الخطة مرة أخرى'});
        }
        if (db.getFeatureFlag('ai_nutritionist') === '0') return sendJson(res,503,{error:'feature_disabled'});
        computationStore.write(u.id,_localDate,computationHash,computed);
        res.setHeader('X-Plan-Cache','GENERATED');
      } finally {
        tickets.delete(ticket);
        if (!tickets.size) activeNutritionReads.delete(u.id);
      }
    } else {
      computed = wantPlan ? saved : {targets:nutritionHost.computeTargets(ctx.profile,ctx.inputs),plan:null};
    }
  }catch(e){
    ...
    if (['engine_busy','engine_timeout','worker_crashed','worker_exited'].includes(code)) {
      res.setHeader('Retry-After','3');
      return sendJson(res,503,{error:'engine_busy',message:'الخطط قيد التجهيز، حاول بعد لحظات'});
    }
    const GUIDE={
  ...
  if(computed.plan)computed.plan._contextRevision=computationHash;
  ...
  const _planDue=!!(staleness&&staleness.needsRecalc) || _engineWeek > 1;
  ...
  try{
    ...
    // The periodizer was advisory only, yet the UI displayed its alternate
    // number as if the actual meal plan had been rebuilt to it. Keep phase/day
    // context, but expose one canonical target until a day-specific target is
    // genuinely applied to the foods and their macros.
    if(today&&typeof today==='object'){
      today.periodizedSuggestionCals=today.targetCals;
      today.targetCals=targets.targetCals||0;
      today.macros=Object.assign({},targets.macros||{});
    }
    ...
    periodization={week:week,phase:phase,today:today,adjustment:finalAdjust,diagnosis:diagnosis,_explanationContext:{common,diagnosis,adjustment:finalAdjust}};
  ...
  let decisionExplanation=null;try{const ec=periodization&&periodization._explanationContext;if(ec)decisionExplanation=buildDecisionExplanation(ec.common,periodization.diagnosis,periodization.adjustment,_smartOn);}catch(_){}

  // Reading analysis must be idempotent. Suggestions remain advisory until
  // explicitly applied; do not change another screen's target on a GET.
  if(periodization&&_coachSaved&&livingProfile.coachTargetCals>0){
    periodization.activeCoachTarget={targetCals:targets.targetCals,week:_coachSaved.week,since:_coachSaved.at};
  }
  ...
  return sendJson(res,200,gateNutritionPayload(u.id,nutritionCache.set(_ck,{   // [FIX C1]
    ...
    staleness,
    smartCoach:_smartOn,
    periodization:(function(){if(periodization&&periodization._explanationContext){const c=Object.assign({},periodization);delete c._explanationContext;return c;}return periodization;})(),
    decisionExplanation,
    timezone:_tz.iana,
...
async function modules(req,res){
  ...
  // غير المشترك بيشوف الوحدات وعدد تمارين كل وحدة (عشان يعرف بيشترك إيه)
...
async function startTrial(req,res,ip){
  ...
  let deviceId='';
  try{const b=await readJsonBody(req,4096);deviceId=String((b&&b.deviceId)||'').trim();}catch(_){}
  if(!/^[a-f0-9]{32}$/i.test(deviceId)) return sendJson(res,400,{error:'تعذر تأمين التجربة على الجهاز — حدّث التطبيق'});
  const usedVia=prefs.trialUsedBy({userId:u.id,email:u.email,phone:u.phone,deviceId});
  if(usedVia) return sendJson(res,409,{error:'سبق استخدام التجربة المجانية لهذا الحساب أو الجهاز'});
...
async function getNotifications(req,res){
  const u=user(req,res); if(!u)return;
  const now=Date.now();
  const isPro=proContext(u.id).isPro;
  const all=(settings.getJSON('admin_notifications',[])||[]);
  const items=all.filter(function(n){
    if(!n||!n.title)return false;
    const target=String(n.target||'all');
    if(target==='pro'&&!isPro)return false;
    if(target==='free'&&isPro)return false;
    if(target!=='all'&&target!=='pro'&&target!=='free'&&target!==String(u.id))return false;
    if(n.scheduled_at&&Date.parse(n.scheduled_at)>now)return false;
...
// Long polling sends changes immediately on this instance; timeout also
// observes changes from a redeployed/other instance without a new paid service.
async function watchAnnouncements(req,res){
  const u=user(req,res);if(!u)return;
  const cursor=new URL(req.url,'http://localhost').searchParams.get('cursor');
  const snapshot=()=>activeAnnouncements(null,proContext(u.id));
  const hash=items=>require('node:crypto').createHash('sha256').update(JSON.stringify(items)).digest('hex').slice(0,24);
  let items=snapshot(),revision=hash(items);
  if(cursor===revision){
    await require('../lib/announcement-events').wait(res);
    if(res.destroyed)return;
    if(!auth.currentUser(parseCookies(req)))return sendJson(res,401,{error:'unauthenticated'});
    items=snapshot();revision=hash(items);
  }
  res.setHeader('Cache-Control','no-store');
  return sendJson(res,200,{ok:true,announcements:items,cursor:revision});
}
...
module.exports={accountSnapshot,subscriptionSnapshot,bootstrap,history,watchAnnouncements,getNotifications,registerDevice,unregisterDevice,mobileAppConfig,activeAnnouncementsPublic:activeAnnouncements,saveProfile,activatePlan,startSession,saveSet,finishSession,saveNutrition,saveWeight,saveMeasurement,foodSearch,foodPreferences,saveFoodPreference,exerciseHistory,exerciseAlternatives,workoutHistory,progressStory,acknowledgeProgressMoments,nutritionPlan,modules,appVersion,clientEvent,reportVideo,smartCoachGet,smartCoachSet,startTrial,checkinSubmit,saveMealOverride,getMealOverrides,preparePlansNow,bootstrapCacheStats,invalidateUserCaches:dropCache,readinessGet,readinessSubmit,buildDecisionExplanation,buildReadinessDecision};
