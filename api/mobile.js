...
async function prepareSavedPlans(userId,p,req){
  ...
  if(p.trains!==false && p.trainingDays>0 && (!previous || previous._profileWorkoutSignature!==workoutProfileSignature(p))){
    ...
    workoutSchedule.alignPlan(selected,p.preferredDays,{nowMs:previous&&Number(previous._scheduleStartedMs)>0?Number(previous._scheduleStartedMs):Date.now(),offsetMs:Number.isFinite(offset)?offset*60000:10800000,firstWorkoutException:!previous||previous._firstWorkoutExceptionActive===true});
    if(previous&&previous._firstWorkoutCompleted===true)selected._firstWorkoutCompleted=true;
...
async function activatePlan(req,res){
  ...
  try {
    ...
    // Recompute the present plan at the ORIGINAL journey anchor, never reset progress.
    const previousStart=Number(previous._scheduleStartedMs);
    const continuation=existing&&Number.isFinite(previousStart);
    aligned = workoutSchedule.alignPlan(planData, selectedDays, {
      nowMs: continuation?previousStart:Date.now(), offsetMs: offsetMs,
      firstWorkoutException:!continuation||previous._firstWorkoutExceptionActive===true
    });
    if(continuation&&previous._firstWorkoutCompleted===true)planData._firstWorkoutCompleted=true;
...
async function finishSession(req,res){
  ...
  if(wasActive){
    ...
    if(finished&&activePlan){
      const planData=parse(activePlan.plan_json,{}),firstKey=String(planData._firstWorkoutDayKey||'').trim(),finishedKey=String(finished.day_key||'').trim();
      if(planData._firstWorkoutExceptionActive===true&&firstKey&&firstKey===finishedKey){
        planData._firstWorkoutCompleted=true;
