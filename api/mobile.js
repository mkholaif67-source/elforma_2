...
function trainingDayFromPlan(userId,options){
  try{
    ...
    const scheduled=workoutSchedule.trainingDayForNow(data,options||{nowMs:Date.now(),offsetMs:3*3600000});
...
function _enrichPlanVideoIds(planObj) {
  ...
  try { workoutSchedule.materializePlan(planObj.data, {nowMs:Date.now(), offsetMs:3*3600000}); } catch(_scheduleViewErr) {}
  try {
...
function bootstrapPayload(req,res){
  ...
  const localDay=new Date(Date.now()+(Number.isFinite(tzOffset)?tzOffset:180)*60000).toISOString().slice(0,10);
...
function currentPlanForProfile(userId){
  ...
  const result=_enrichPlanVideoIds(active);
...
async function prepareSavedPlans(userId,p,req){
  ...
  if(p.trains!==false && p.trainingDays>0 && (!previous || previous._profileWorkoutSignature!==workoutProfileSignature(p))){
    ...
    workoutSchedule.alignPlan(selected,p.preferredDays,{nowMs:previous&&Number(previous._scheduleStartedMs)>0?Number(previous._scheduleStartedMs):Date.now(),offsetMs:Number.isFinite(offset)?offset*60000:10800000});
...
module.exports={accountSnapshot,subscriptionSnapshot,bootstrap,history,watchAnnouncements,getNotifications,registerDevice,unregisterDevice,mobileAppConfig,activeAnnouncementsPublic:activeAnnouncements,saveProfile,activatePlan,startSession,saveSet,finishSession,saveNutrition,saveWeight,saveMeasurement,foodSearch,foodPreferences,saveFoodPreference,exerciseHistory,exerciseAlternatives,workoutHistory,nutritionPlan,modules,appVersion,clientEvent,reportVideo,smartCoachGet,smartCoachSet,startTrial,checkinSubmit,saveMealOverride,getMealOverrides,preparePlansNow,bootstrapCacheStats};
