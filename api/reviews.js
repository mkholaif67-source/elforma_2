...
async function submit(req, res, ip) {
  const u = requireUser(req, res); if (!u) return;
  // One account can edit its review, but cannot turn that endpoint into a
  // write/amplification loop.  The IP limit also covers compromised accounts.
  if (!rateLimit('review:submit:user:' + u.id, 3, 60 * 60 * 1000) ||
      !rateLimit('review:submit:ip:' + String(ip || 'unknown'), 12, 60 * 60 * 1000)) {
    return sendJson(res, 429, { error: 'محاولات تقييم كثيرة — حاول بعد ساعة' });
  }
