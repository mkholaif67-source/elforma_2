...
async function googleAuth(req, res, ip) {
  ...
  // [SEC-S2] Audience check is mandatory – GOOGLE_CLIENT_ID must be set.
  const wantAud = process.env.GOOGLE_CLIENT_ID || '';
  if (!wantAud) {
    return sendJson(res, 503, { error: 'google_auth_not_configured',
      message: "GOOGLE_CLIENT_ID is not set - add it to your environment variables to enable Google login" });
  }
  if (String((info && info.aud) || '') !== wantAud) {
...
async function logout(req, res) {
  ...
  if (user) {
    ...
    try { db.audit(user.id, 'logout', ''); } catch (_) {}
  }
