...
async function exportData(req, res) {
  ...
  const payload = {
    ...
    state,
    community: require('../lib/community').exportUser(user.id),
    friend_challenges: require('../lib/friend-challenges').exportUser(user.id),
    mobile: {
...
async function deleteAccount(req, res, ip) {
  ...
  try { require('./mobile').invalidateUserCaches(user.id); } catch (_) {}
