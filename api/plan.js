...
async function compute(req, res, ip) {
  ...
  // A guest can still see the teaser, but must not receive the same expensive
  // compute budget as a paying account.  Keep the guest key per IP so one
  // anonymous caller cannot consume the budget for every other visitor.
  const guestIp = String(ip || (req.socket && req.socket.remoteAddress) || 'unknown');
  const computeKey = user ? 'u' + user.id : 'ip' + guestIp;
  const computeBudget = user ? 12 : 4;
  if (tooMany(res, 'plan:compute:' + computeKey, computeBudget, 60000)) return;
  ...
  let full;
  try { full = await sharedCompute('plan', computeKey, profile, inputs, () => enginePool.computeMealPlanAsync(profile, inputs)); }
  catch (e) {
    const m = (e && e.message) || '';
...
async function targets(req, res) {
  ...
  const targetKey = tUser ? 'u' + tUser.id : 'ip' + String((req.socket && req.socket.remoteAddress) || 'unknown');
  if (tooMany(res, 'plan:targets:' + targetKey, tUser ? 30 : 10, 60000)) return;
  const b = await readJsonBody(req);
  const profile = cleanProfile(b);
  const inputs = cleanInputs(b);
  const bodyProblem = bodyBounds.validateBody(profile);
  if (bodyProblem) return sendJson(res, 422, { error: 'profile_incomplete', code: bodyProblem.code, message: engineGuide.GUIDE[bodyProblem.code] });
  let t;
  try { t = await sharedCompute('targets', targetKey, profile, inputs, () => enginePool.computeTargetsAsync(profile, inputs)); }
  catch (e) {
    const m = (e && e.message) || '';
    if (/engine_busy|engine_timeout|engine_pool_closed/.test(m)) {
      return sendJson(res, 503, { error: 'الخادم مشغول حاليًا — حاول بعد دقيقة' });
