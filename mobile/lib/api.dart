...
class Api {
  ...
  Future<bool> bindAccount(Object? userId, {String? createdAt}) async {
    ...
    final changed = _accountId != id || _accountScope != scope || previousScope != scope;
  ...
  Future<ApiResult?> _cachedRead(String path) async {
    ...
    try {
      ...
      data['fromCache'] = true;
      data['stale'] = DateTime.now().difference(savedAt) > const Duration(days: 1);
  ...
  Future<ApiResult> logout() async {
    ...
    _sessionEpoch++; _cookie=null; _accountId=null; _accountScope=null;
    invalidateBootstrap();invalidateNutritionPlans();
    accountChanges.value=_sessionEpoch;
    try { await _persistCookie(); } catch (_) {}
    try {
      final sp=await SharedPreferences.getInstance();
      await sp.remove(_accountIdKey);await sp.remove('ef_profile_cache_v1');
    } catch (_) {}
  ...
  void invalidateBootstrap() {
    ...
    _cachePurge = _cachePurge.then((_) async {
      final sp = await SharedPreferences.getInstance();
      final affected = sp.getKeys().where((k) =>
          k.startsWith(_readCachePrefix) &&
          (k.contains('/api/mobile/bootstrap') || k.contains('/api/mobile/nutrition-plan'))).toList();
      for (final key in affected) { await sp.remove(key); }
    }).catchError((Object _) {});
  ...
  Future<ApiResult> challengeDetail(String id) => _send('GET', '/api/mobile/community/detail?id=${Uri.encodeQueryComponent(id)}');
  ...
  Future<ApiResult> getSessionReadiness({int? sessionId}) => _send(
      'GET', '/api/mobile/session/readiness${sessionId == null ? '' : '?sessionId=$sessionId'}');
  Future<ApiResult> submitSessionReadiness(int sessionId, {required Map<String, dynamic> checkIn}) =>
      _send('POST', '/api/mobile/session/readiness', {'sessionId': sessionId, 'checkIn': checkIn});
  ...
  Future<ApiResult> progressStory({String? month}) => _send(
        'GET', '/api/mobile/progress-story${month == null ? '' : '?month=${Uri.encodeQueryComponent(month)}'}');
    ...
    if (!force && hit == null) {
      ...
      if (persisted != null) {
        ...
        final bgFuture = _send('GET', path).then((r) {
          ...
          return r;
        }).whenComplete(() { if (bgEpoch == _epoch) _nutritionInFlight.remove(path); });
...
const String kAppVersionName = '1.0.31';
