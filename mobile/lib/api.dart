...
class Api {
  ...
  Future<bool> bindAccount(Object? userId) async {
    ...
    if (previous != null && previous.isNotEmpty && previous != id) {
      // الكاش والطابور مربوطان بمعرف الحساب داخل المفتاح؛ نحتفظ ببيانات الحساب
      // السابق ولا نسمح بإرسالها للحساب الجديد. نمسح فقط نسخة البروفايل العامة.
  ...
  String _cacheKeyFor(String path) =>
      '$_readCachePrefix${_accountId ?? 'anon'}:${path.replaceAll('?fresh=1', '')}';
