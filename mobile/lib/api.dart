...
class Api {
  ...
  Future<ApiResult> _refreshBootstrapNetwork() {
    final active = _bootstrapInFlight;
    if (active != null) return active;
    final future = _send('GET', '/api/mobile/bootstrap').then((r) {
      if (r.ok) { _bootstrapCached = r; _bootstrapCachedAt = DateTime.now(); }
      return r;
  ...
  Future<ApiResult> mobileBootstrap({String? day, bool force = false}) async {
    ...
    if (force) return _refreshBootstrapNetwork();
    final hit = _bootstrapCached, at = _bootstrapCachedAt;
    if (hit != null && at != null && DateTime.now().difference(at) < _bootstrapTtl) return hit;
    final active = _bootstrapInFlight;
    if (active != null) return active;
    final disk = await _cachedRead('/api/mobile/bootstrap');
    if (disk != null) {
      _bootstrapCached = disk; _bootstrapCachedAt = DateTime.now();
      unawaited(_refreshBootstrapNetwork());
      return disk;
    }
    return _refreshBootstrapNetwork();
  }
  ...
  Future<ApiResult> saveMobileProfile(Map<String, dynamic> profile) async {
    final r = await _send('PUT', '/api/mobile/profile', {'profile': profile});
    if (r.ok) { invalidateBootstrap(); invalidateNutritionPlans(); }
    return r;
  }
  ...
  Future<ApiResult> nutritionPlan({bool withMeals = true, int dayOffset = 0, bool force = false}) async {
    ...
    if (!force) {
      ...
      if (hit != null && at != null && DateTime.now().difference(at) < _nutritionTtl) return hit;
      final active = _nutritionInFlight[path];
      if (active != null) return active;
      final disk = await _cachedRead(path);
      if (disk != null) {
        _nutritionMemo[path] = disk; _nutritionMemoAt[path] = DateTime.now();
        unawaited(nutritionPlan(withMeals: withMeals, dayOffset: dayOffset, force: true));
        return disk;
      }
    }
    final future = _send('GET', path).then((r) {
      if (r.ok) { _nutritionMemo[path] = r; _nutritionMemoAt[path] = DateTime.now(); }
      return r;
    }).whenComplete(() => _nutritionInFlight.remove(path));
  ...
  Future<ApiResult> saveFoodPreference(String foodId, {bool? favorite, bool used = false}) async {
    final r = await _send('PUT', '/api/mobile/food-preference', {
      'foodId': foodId,
      if (favorite != null) 'favorite': favorite,
      if (used) 'used': true,
    }, true);
    if (r.ok) invalidateNutritionPlans();
    return r;
  }
