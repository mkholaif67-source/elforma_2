...
class ApiResult {
  ...
  bool get isProfileIncomplete =>
      status == 422 && error == 'profile_incomplete';
  ...
  String friendlyError(String fallback) {
    ...
    bool readable(String v) =>
        RegExp(r'[؀-ۿ]').hasMatch(v) &&
        !RegExp(r'Exception|Error:|Stack|SELECT|INSERT|https?://|[a-z]+_[a-z]+')
            .hasMatch(v);
...
class Api {
  ...
  Api.forTesting(http.Client client) : _client = client {
    _initialized = true;
    _accountId = "test";
  }
  ...
  Future<void> _persistCookie() {
    ...
    final write = _credentialWrites.catchError((Object _) {}).then((_) async {
      if (cookie == null || !remember) {
        await _secure.delete(key: _cookieKey);
      } else {
        await _secure.write(key: _cookieKey, value: cookie);
      }
    });
  ...
  static const _accountScopeKey = 'ef_account_scope_v2';
  static const _installMarkerKey = 'ef_install_marker_v1';
  String? _accountId;
  ...
  /// Stable identity for local caches; creation time prevents id-reuse leaks.
  String? get accountScope => _accountScope;

  String _scopeFor(String id, String? createdAt) =>
      '$id|${createdAt == null || createdAt.isEmpty ? 'unknown' : createdAt}';
  ...
  Future<bool> bindAccount(Object? userId, {String? createdAt}) async {
    final startingSession = _sessionEpoch;
    final id = (userId == null) ? '' : '$userId'.trim();
    if (id.isEmpty) return false;
    final scope = _scopeFor(id, createdAt);
    final sp = await SharedPreferences.getInstance();
    if (startingSession != _sessionEpoch) return false;
    final previous = sp.getString(_accountIdKey);
    final previousScope = sp.getString(_accountScopeKey);
    final changed =
        _accountId != id || _accountScope != scope || previousScope != scope;
    if (changed) _sessionEpoch++;
    _accountId = id;
    _accountScope = scope;
    if (changed || previous != id || previousScope != scope) {
      invalidateBootstrap();
    ...
    await sp.setString(_accountScopeKey, scope);
    return changed;
  }
  ...
  Future<void> _initOnce() async {
    ...
    _accountScope = sp.getString(_accountScopeKey) ?? _accountId;

    // iOS Keychain can survive uninstall while SharedPreferences does not.
    // A secure cookie with no matching local account identity therefore belongs
    // to an older installation and must not silently resurrect its user data.
    final knownInstall = sp.getBool(_installMarkerKey) == true;
    if (!knownInstall &&
        _cookie != null &&
        _cookie!.isNotEmpty &&
    ...
    await sp.setBool(_installMarkerKey, true);
    ...
    final staleKeys =
        sp.getKeys().where((k) => k.startsWith(anonPrefix)).toList();
    ...
    if (_cookie == null || _cookie!.isEmpty) {
      // No authenticated owner means no user-scoped identity may survive into
      // the next login. The next bindAccount starts from a clean namespace.
      _accountId = null;
      _accountScope = null;
      await sp.remove(_accountIdKey);
      await sp.remove(_accountScopeKey);
      await sp.remove('ef_profile_cache_v1');
      final orphanedReads = sp
          .getKeys()
          .where((key) => key.startsWith(_readCachePrefix))
          .toList();
      for (final key in orphanedReads) {
        await sp.remove(key);
      }
    }
    _initialized = true;
  ...
  Future<int> flushOfflineQueue() async {
    if (_flushing || _cookie == null || _cookie!.isEmpty)
    ...
    try {
      ...
      List<dynamic> queue;
      try {
        queue = jsonDecode(sp.getString(queueKey) ?? '[]') as List<dynamic>;
      } catch (_) {
        queue = <dynamic>[];
      }
      ...
      while (queue.isNotEmpty && guard < 200 && sessionEpoch == _sessionEpoch) {
        ...
        if (raw is! Map || raw['ownerId'] != owner) {
          queue.removeAt(0);
          continue;
        }
        try {
          ...
          final path = '${raw['path']}';
          final body = raw['body'] is Map
              ? Map<String, dynamic>.from(raw['body'] as Map)
              : <String, dynamic>{};
          ...
          if (method == 'POST') {
            response = await _client
                .post(uri, headers: _headers(json: true), body: encoded)
                .timeout(const Duration(seconds: 25));
          } else {
            response = await _client
                .put(uri, headers: _headers(json: true), body: encoded)
                .timeout(const Duration(seconds: 25));
          }
  ...
  static const _cacheableGets = <String>[
    ...
    '/api/mobile/modules',
    '/api/mobile/friend-challenges',
    '/api/mobile/friend-challenges/detail',
  ];
  ...
  // أي تحديث صريح ناجح يحدث نفس النسخة المحفوظة اللي بنعرضها فورا بعد كده.
  String _cacheKeyFor(String path) =>
      '$_readCachePrefix${_accountScope ?? _accountId ?? 'anon'}:${path.replaceAll('?fresh=1', '')}';

  Future<void> _cacheRead(
      String path, Map<String, dynamic> data, int epoch) async {
    try {
      final sp = await SharedPreferences.getInstance();
  ...
  Future<ApiResult?> _cachedRead(String path) async {
    ...
    try {
      ...
      final savedAt = DateTime.tryParse('${decoded['at'] ?? ''}');
      if (savedAt == null ||
  ...
  bool _changesPlans(String path) => const [
        '/api/mobile/profile',
        '/api/mobile/workout-plan',
        '/api/mobile/session/',
        '/api/mobile/nutrition',
        '/api/mobile/weight',
        '/api/mobile/measurement',
        '/api/mobile/food-preference',
        '/api/mobile/checkin',
        '/api/mobile/trial/',
        '/api/mobile/smart-coach',
        '/api/mobile/friend-challenges',
        '/api/state',
        '/api/pay/',
        '/api/account/',
  ...
  Future<ApiResult> _send(String method, String path,
    ...
    try {
      ...
      if (sessionEpoch != _sessionEpoch)
        return ApiResult(409, {'error': 'account_changed'});
      requestEpoch = _epoch;
      final uri = Uri.parse('$baseUrl$path');
      final headers = _headers(json: body != null);
      final b = body != null ? jsonEncode(body) : null;
      // [PHASE-0-PERF] GET كان 60 ثانية — طويل جدا لو الطلب علق في الشبكة.
      ...
      if (requestEpoch != _epoch && method == 'GET') {
        if (retryStale)
          return await _send(method, path, body, queueOnFailure, false);
      ...
      if (queueOnFailure && body != null) {
        ...
        return ApiResult(202, {
          'ok': true,
          'queued': true,
          'message':
              'تم الحفظ على الهاتف وسيتم الإرسال تلقائيا عند عودة الإنترنت'
        });
      }
      ...
      return ApiResult(failure is TimeoutException ? 408 : 0, {
        'error': failure is TimeoutException
            ? 'request_timeout'
            : 'connection_failed'
      });
    }
  ...
  Future<ApiResult> submitCheckin(
          {required String scope, required String answer}) =>
      _send('POST', '/api/mobile/checkin', {'scope': scope, 'answer': answer},
          true);
  ...
  Future<void> _bindFrom(ApiResult r) async {
    ...
    if (u is Map && u['id'] != null) {
      await bindAccount(u['id'], createdAt: '${u['created_at'] ?? ''}');
    }
  }
  ...
  /// يبدأ بدري كل ما الإقلاع البارد يخلص أسرع. صامت تماما: مفيش خطأ بيرمى.
  Future<void> warmup() async {
    try {
      await _client.get(Uri.parse('$baseUrl/api/health'), headers: const {
  ...
  Future<ApiResult> changePassword(String current, String next) => _send(
      'POST', '/api/account/password', {'current': current, 'next': next});
  Future<ApiResult> sendVerificationEmail() =>
      _send('POST', '/api/auth/verify/send', const {});
  Future<ApiResult> login(String identifier, String password,
      {bool remember = true}) async {
    _rememberSession = remember;
    final r = await _send('POST', '/api/auth/login',
  ...
  Future<ApiResult> googleAuth(String idToken,
      {String? name, String? email, bool remember = true}) async {
    _rememberSession = remember;
  ...
  // [FIX-LOGOUT-NAV] الخروج بقى فوري ومحلي أولا. المشكلة الأصلية كانت إن
  // الانتظار الشبكي (لحد 70 ثانية لو السيرفر نايم) كان بيعلق الزرار، وبعدها
  // إشعار accountChanges بينفذ قبل التنقل → الـ shell بيدمر شاشة الحساب
  ...
  // صراحة بالطلب عشان يوصل معرفا بالجلسة حتى بعد المسح المحلي الفوري)،
  // والتنظيف المحلي بيحصل في الحال ومتسامح مع أي فشل، والشاشة بتتنقل فورا.
  ...
  Future<ApiResult> couponCheck(String code, String plan) => _send('POST',
      '/api/coupon/check', {'code': code, 'plan': plan, 'currency': 'EGP'});
  ...
  Future<ApiResult> sendClientEvent(
  ...
  /// Fires whenever the nutrition plan in the background SWR refresh
  /// completes. meal_plan_screen listens and reloads without a spinner.
  final ValueNotifier<int> nutritionRevision = ValueNotifier<int>(0);
  ...
  void invalidateBootstrap() {
    ...
    // Preserve the last account-scoped disk snapshot. Mutations invalidate the
    // memory copy; the next successful GET replaces the durable snapshot.
  ...
  Future<ApiResult> _refreshBootstrapNetwork({bool fresh = false}) {
    ...
    final future = _send('GET',
            fresh ? '/api/mobile/bootstrap?fresh=1' : '/api/mobile/bootstrap')
        .then((r) {
      if (epoch == _epoch && r.ok && r.data['fromCache'] != true) {
        _bootstrapCached = _copyResult(r);
        _bootstrapCachedAt = DateTime.now();
      }
      return r;
    }).whenComplete(() {
      if (epoch == _epoch) _bootstrapInFlight = null;
    });
  ...
  Future<ApiResult> mobileBootstrap({String? day, bool force = false}) async {
    ...
    // على الجهاز من آخر جلسة ناجحة النهارده: اعرضها في اللحظة وحدث من الشبكة
    // في الخفاء. دي طريقة التطبيقات اللي «بتتحرك في لحظات» — المحتوى أولا،
    // والتحديث بيحصل من غير ما المستخدم يحس بأي انتظار.
    if (!force && hit == null) {
      final persisted = await _cachedRead('/api/mobile/bootstrap');
      if (persisted != null) {
        // Keep every consumer on the same snapshot while the fresh request is
        // in flight. Without this, Home could receive the persisted plan while
        // Workout received the network plan a moment later, producing two
        // different "today" days on the same screen transition.
        _bootstrapCached = _copyResult(persisted);
        _bootstrapCachedAt = DateTime.now();
        unawaited(_refreshBootstrapNetwork(fresh: true).then((r) {
          if (r.ok && r.data['fromCache'] != true) bootstrapRevision.value++;
        }));
        return _copyResult(persisted);
      }
    }
  ...
  Future<ApiResult> _lightSnapshot(String path, {bool force = false}) async {
    ...
    final future = _send('GET', path).then((r) async {
      ...
      if (epoch == _epoch &&
          ...
          r.ok &&
          r.data['fromCache'] != true) {
        _lightCached[path] = _copyResult(r);
        _lightCachedAt[path] = DateTime.now();
      }
      return r;
    }).whenComplete(() {
      if (epoch == _epoch) _lightInFlight.remove(path);
    });
  ...
  Future<ApiResult> startTrial([String phone = '']) async => _send(
      'POST',
      '/api/mobile/trial/start',
      {'phone': phone, 'deviceId': await deviceId()});

  /// كتالوج الوحدات المساعدة بتمارينها الحقيقية من المحرك — مش مجرد مفاتيح تشغيل.
  Future<ApiResult> community() => _send('GET', '/api/mobile/community');
  Future<ApiResult> challengeDetail(String id) => _send(
      'GET', '/api/mobile/community/detail?id=${Uri.encodeQueryComponent(id)}');
  Future<ApiResult> communityAction(String action, Map<String, dynamic> data) =>
      _send('POST', '/api/mobile/community/$action', data);

  Future<ApiResult> friendChallenges() =>
      _send('GET', '/api/mobile/friend-challenges');
  Future<ApiResult> friendChallengeDetail(String id) => _send('GET',
      '/api/mobile/friend-challenges/detail?id=${Uri.encodeQueryComponent(id)}');
  Future<ApiResult> friendChallengePreview(String code) => _send('GET',
      '/api/mobile/friend-challenges/preview?code=${Uri.encodeQueryComponent(code)}');
  Future<ApiResult> createFriendChallenge(Map<String, dynamic> data) =>
      _send('POST', '/api/mobile/friend-challenges/create', data);
  Future<ApiResult> friendChallengeAction(
          String action, Map<String, dynamic> data,
          {bool queueOnFailure = false}) =>
      _send('POST', '/api/mobile/friend-challenges/$action', data,
          queueOnFailure);
  ...
  Future<ApiResult> saveMobileProfile(Map<String, dynamic> profile,
      {bool patch = false, bool preparePlans = false}) async {
    // Keep the last complete bootstrap snapshot before _send invalidates it.
    // A successful profile mutation must replace the profile inside that
    // snapshot immediately; otherwise the next reader can receive yesterday's
    // persisted/memory value while the fresh bootstrap request is still running.
    final previousBootstrap =
        _bootstrapCached == null ? null : _copyResult(_bootstrapCached!);
    final r = await _send('PUT', '/api/mobile/profile', {
      'profile': profile,
      if (patch) 'patch': true,
      if (preparePlans) 'preparePlans': true
    });
    if (r.ok && r.data['queued'] != true && r.data['profile'] is Map) {
      await _commitProfileToBootstrap(
        Map<String, dynamic>.from(r.data['profile'] as Map),
        previousBootstrap,
      );
    }
    return r;
  ...
  Future<void> _commitProfileToBootstrap(
      Map<String, dynamic> savedProfile, ApiResult? previous) async {
    final owner = _accountScope ?? _accountId;
    if (previous == null) {
      // No complete snapshot existed to patch. Removing the durable copy is
      // safer than briefly reviving stale data; the next bootstrap goes to the
      // network and the epoch guard supersedes any read started before the PUT.
      if (owner != null) {
        try {
          final sp = await SharedPreferences.getInstance();
          await sp.remove(_cacheKeyFor('/api/mobile/bootstrap'));
        } catch (_) {}
      }
      return;
    }
    final data = Map<String, dynamic>.from(previous.data)
      ...
      ..remove('stale')
      ..remove('cachedAt');
    final mergedProfile = data['profile'] is Map
        ? Map<String, dynamic>.from(data['profile'] as Map)
        : <String, dynamic>{};
    mergedProfile.addAll(savedProfile);
    data['profile'] = mergedProfile;
    data['ok'] = true;
    _bootstrapCached = ApiResult(200, Map<String, dynamic>.from(data));
    _bootstrapCachedAt = DateTime.now();
    if (owner == null) return;
    try {
      final sp = await SharedPreferences.getInstance();
      if (owner != (_accountScope ?? _accountId)) return;
      await sp.setString(
          _cacheKeyFor('/api/mobile/bootstrap'),
          jsonEncode({
            'at': DateTime.now().toIso8601String(),
            'data': data,
          }));
  ...
  Future<void> _patchCachedWorkoutPlan(Map<String, dynamic> workoutPlan) async {
    final owner = _accountScope ?? _accountId;
    if (owner == null) return;
    try {
      final sp = await SharedPreferences.getInstance();
      final key = _cacheKeyFor('/api/mobile/bootstrap');
      final raw = sp.getString(key);
  ...
  Future<ApiResult> activateWorkoutPlan(String key, Map<String, dynamic> plan,
      {List<int> selectedDays = const <int>[]}) async {
    final revision = _sessionEpoch;
    final result = await _send('PUT', '/api/mobile/workout-plan',
        {'key': key, 'plan': plan, 'selectedDays': selectedDays}, true);
    if (revision == _sessionEpoch &&
        result.ok &&
        result.data['queued'] != true &&
        result.data['workoutPlan'] is Map) {
      await _patchCachedWorkoutPlan(
          Map<String, dynamic>.from(result.data['workoutPlan'] as Map));
    }
    return result;
  }

  Future<ApiResult> startWorkoutSession(String dayKey, String dayName) => _send(
      'POST',
      '/api/mobile/session/start',
      {'dayKey': dayKey, 'dayName': dayName});
  Future<ApiResult> getSessionReadiness({int? sessionId}) => _send('GET',
      '/api/mobile/session/readiness${sessionId == null ? '' : '?sessionId=$sessionId'}');
  Future<ApiResult> submitSessionReadiness(int sessionId,
          {required Map<String, dynamic> checkIn}) =>
      _send('POST', '/api/mobile/session/readiness',
          {'sessionId': sessionId, 'checkIn': checkIn});
  ...
  Future<ApiResult> exerciseAlternatives(String current,
    ...
    return _send('GET',
        '/api/mobile/exercise-alternatives?${Uri(queryParameters: params).query}');
  }

  Future<ApiResult> workoutHistory() =>
      _send('GET', '/api/mobile/workout-history');
  Future<ApiResult> progressStory({String? month}) => _send('GET',
      '/api/mobile/progress-story${month == null ? '' : '?month=${Uri.encodeQueryComponent(month)}'}');
  Future<ApiResult> acknowledgeProgressMoments(List<dynamic> ids) =>
      _send('POST', '/api/mobile/progress-story/ack', {'ids': ids}, true);

  final Map<String, ApiResult> _nutritionMemo = <String, ApiResult>{};
  final Map<String, DateTime> _nutritionMemoAt = <String, DateTime>{};
  final Map<String, Future<ApiResult>> _nutritionInFlight =
      <String, Future<ApiResult>>{};
  ...
  Future<ApiResult> nutritionPlan(
      {bool withMeals = true, int dayOffset = 0, bool force = false}) async {
    final params = <String>[];
    if (!withMeals) params.add('plan=0');
    if (dayOffset != 0) params.add('offset=$dayOffset');
    final path = '/api/mobile/nutrition-plan' +
    ...
    if (active != null) return active.then(_copyResult);
    // [INSTANT-OPEN] SWR: لو الذاكرة فاضية (فتحة جديدة للتطبيق) بس فيه نسخة على الجهاز:
    // ارجعها فورا وابدأ التحديث في الخلف. لما التحديث يكتمل nutritionRevision.value++ يخبرنا
    // ونعيد التحميل بدون spinner.
    if (!force && hit == null) {
      final persisted = await _cachedRead(path);
      if (persisted != null) {
        final bgEpoch = _epoch;
        final bgFuture = _send('GET', path).then((r) {
          if (bgEpoch == _epoch && r.ok && r.data['fromCache'] != true) {
            _nutritionMemo[path] = _copyResult(r);
            _nutritionMemoAt[path] = DateTime.now();
            nutritionRevision.value++;
          }
          return r;
        }).whenComplete(() {
          if (bgEpoch == _epoch) _nutritionInFlight.remove(path);
        });
        _nutritionInFlight[path] = bgFuture;
        unawaited(bgFuture);
        return persisted;
      }
    }
    ...
    final future = _send('GET', path).then((r) {
      ...
      if (epoch == _epoch) _nutritionInFlight.remove(path);
    });
  ...
  Future<ApiResult> finishWorkoutSession(
    ...
    if (revision == sessionRevision &&
        r.ok &&
        r.data['queued'] != true &&
        r.data['session'] is Map) {
      PlanStore.I.sessionCompleted(
          owner, Map<String, dynamic>.from(r.data['session'] as Map));
    }
    return r;
  }
  ...
  Future<ApiResult> searchFoods(String query,
    ...
    return _send(
        'GET', '/api/mobile/foods?${Uri(queryParameters: params).query}');
  }

  Future<ApiResult> foodPreferences() =>
      _send('GET', '/api/mobile/food-preferences');
  Future<ApiResult> saveFoodPreference(String foodId,
      {bool? favorite, bool used = false, String? mealSlot}) async {
    final r = await _send(
        'PUT',
        '/api/mobile/food-preference',
        {
  ...
  Future<ApiResult> registerDevice(
          String token, String platform, int appBuild) =>
  ...
  Future<ApiResult> watchAnnouncements(String cursor) => _send('GET',
      '/api/mobile/announcements/watch?cursor=${Uri.encodeQueryComponent(cursor)}');
...
const String kAppVersionName = '1.0.35';
