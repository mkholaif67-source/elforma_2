...
class NotificationService {
  ...
  Future<void> initialize() {
    ...
    return future.whenComplete(() => _initializeInFlight = null);
  }
    ...
    try {
      ...
      await plugin.initialize(settings,
        onDidReceiveNotificationResponse: (response) {
          _openPayload(response.actionId?.startsWith('tel:') == true ? response.actionId : response.payload);
        },
      );
      _ready = true;
      ...
      if (launch?.didNotificationLaunchApp == true) {
        ...
        await _openPayload(response?.actionId?.startsWith('tel:') == true ? response!.actionId : response?.payload);
      }
    } catch (_) {
      _ready = false;
    }
  ...
  Future<void> _openPayload(String? raw) async {
    final value = (raw ?? '').trim();
    if (value.isEmpty) return;
    if (CommunityLinks.accept(value)) return;
    if (value.startsWith('tel:')) {
      final phone = Uri.tryParse(value);
      if (phone != null) { try { await launchUrl(phone, mode: LaunchMode.externalApplication); } catch (_) {} }
      return;
    }
    final uri = Uri.tryParse(value.startsWith('https://') ? value : 'https://$value');
    if (uri == null || uri.scheme != 'https' || uri.host.isEmpty) return;
    final context = CommunityLinks.navigatorKey.currentContext;
    ...
    final approved = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.of(dialogContext).pop(true), child: const Text('فتح')),
        ],
      ),
    );
  ...
  Future<bool> showAnnouncement({required String title, required String body, int id = 950, String? link, String? image, String? phone}) async {
    final session=Api.I.sessionRevision;
    await initialize();
    if (!_ready) return false;
    final android = plugin.resolvePlatformSpecificImplementation<AndroidFlutterLocalNotificationsPlugin>();
    if (await android?.areNotificationsEnabled() == false) return false;
    StyleInformation style = BigTextStyleInformation(body, contentTitle: title);
    String? picturePath;
    final raw = (image ?? '').trim();
    if (raw.isNotEmpty) {
      final client = http.Client();
      try {
        final uri = Uri.parse(Api.baseUrl).resolve(raw);
        if (uri.scheme == 'https' || uri.host == '127.0.0.1') {
          final response = await client.send(http.Request('GET', uri)).timeout(const Duration(seconds: 6));
          if (response.statusCode == 200 && (response.headers['content-type'] ?? '').startsWith('image/') && (response.contentLength ?? 0) <= 5 * 1024 * 1024) {
            final bytes = <int>[];
            await for (final chunk in response.stream.timeout(const Duration(seconds: 6))) {
              bytes.addAll(chunk);
              if (bytes.length > 5 * 1024 * 1024) throw const FormatException('Image size limit');
            }
            final dir = await getTemporaryDirectory();
            picturePath = '${dir.path}/notification_$id.img';
            await File(picturePath).writeAsBytes(bytes);
            style = BigPictureStyleInformation(FilePathAndroidBitmap(picturePath), contentTitle: title, summaryText: body);
          }
        }
      } catch (_) {} finally { client.close(); }
    }
    if(session!=Api.I.sessionRevision){if(picturePath!=null){try{await File(picturePath).delete();}catch(_){}}return false;}
    final number = (phone ?? '').replaceAll(RegExp(r'[^0-9+]'), '');
    try {
      await plugin.show(id, title, body, NotificationDetails(
        android: AndroidNotificationDetails('admin_updates_v4', 'أخبار وعروض الفورمة',
          channelDescription: 'إشعارات الإدارة بدون تكرار', importance: Importance.high,
          priority: Priority.high, onlyAlertOnce: true, category: AndroidNotificationCategory.promo,
          styleInformation: style, actions: [
            if (number.isNotEmpty) AndroidNotificationAction('tel:$number', 'اتصل بنا', showsUserInterface: true),
          ]),
        iOS: const DarwinNotificationDetails(presentAlert: true, presentSound: true),
      ), payload: link);
      return true;
    } finally {
      if (picturePath != null) { try { await File(picturePath).delete(); } catch (_) {} }
    }
  }
