...
class NotificationService {
  ...
  Future<void> showAnnouncement({required String title, required String body, int id = 950, String? link}) async {
    await initialize();
    if (!_ready) return;
    await plugin.show(id, title, body,
      _details('promo_announcements', 'إشعارات العروض', sound: 'promo_alert'),
      payload: (link ?? '').trim().isEmpty ? null : link!.trim());
  }
