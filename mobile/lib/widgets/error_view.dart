...
String efErrorMessage(int status, [dynamic data]) {
  ...
  if (status == 408) {
    return 'الاتصال استغرق وقتا أطول من المعتاد. جرب مرة تانية بعد لحظات';
  }
  if (status == 401 || code == 'unauthenticated') {
    return 'انتهت جلسة الدخول. سجل دخولك مرة تانية';
  }
  if (status == 403) {
    if (code == 'subscription_required' || code == 'forbidden') {
      return 'الجزء ده محتاج اشتراك فعال';
    }
    return 'مش متاح الوصول للجزء ده حاليا';
  }
  if (status == 429) {
    return 'في محاولات كتير ورا بعض. استنى دقيقة وجرب مرة تانية';
  }
  if (status >= 500) {
    return 'حصلت مشكلة مؤقتة أثناء التحميل. جرب مرة تانية';
  }
  if (status == 404) return 'المحتوى المطلوب مش متاح حاليا';
  return 'حصلت مشكلة غير متوقعة. جرب مرة تانية';
}
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: FormaIcon(icon, size: 38, color: const Color(0xFF0A6A3B)),
                        ),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: FormaIcon(icon, size: 29, color: const Color(0xFF0A6A3B)),
            ),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            FormaIcon(icon, size: 44, color: AppColors.muted),
            const SizedBox(height: 14),
            Text(
              message,
              textAlign: TextAlign.center,
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          const FormaLoader(color: AppColors.nu),
          if (label != null) ...[
            const SizedBox(height: 14),
            Text(
              label!,
