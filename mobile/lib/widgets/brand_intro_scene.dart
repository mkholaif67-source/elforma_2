            ...
            children: [
              ...
              Positioned(
                ...
                child: BrandIntroConnection(
                    progress: ease(3.1, 4.3), waiting: waiting),
              ),
              for (var i = 0; i < 3; i++) ...[
                Positioned(
                  left: 84 + i * 132.0 - 27,
...
/// Only the connecting line moves while startup finishes; the intro stays put.
...
class _BrandIntroConnectionState extends State<BrandIntroConnection>
  ...
  @override
  void didUpdateWidget(covariant BrandIntroConnection old) {
  ...
  Widget build(BuildContext context) => ExcludeSemantics(
        ...
        child: AnimatedBuilder(
            ...
            builder: (_, child) => CustomPaint(
                  ...
                  painter: _PulsePainter(widget.progress,
                      flow: widget.waiting &&
...
class _PulsePainter extends CustomPainter {
  const _PulsePainter(this.progress, {this.flow});
  final double progress;
  final double? flow;
  @override
  void paint(Canvas c, Size s) {
    ...
    if (flow != null) {
      final center = metric.length * flow!;
      final opacity = math.sin(math.pi * flow!).clamp(0.0, 1.0);
      final segment = metric.extractPath(
          math.max(0, center - 24), math.min(metric.length, center + 24));
      c.drawPath(
          segment,
          Paint()
            ..color = const Color(0xFF8CB94C).withValues(alpha: opacity * .25)
            ..style = PaintingStyle.stroke
            ..strokeWidth = 7
      ...
      c.drawPath(
          segment,
          Paint()
            ..color = brandInk.withValues(alpha: opacity)
            ..style = PaintingStyle.stroke
            ..strokeCap = StrokeCap.round
            ..strokeWidth = 2.6);
    }
