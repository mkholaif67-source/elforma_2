...
class _AnnouncementCardState extends State<AnnouncementCard> {
  ...
  Future<void> _open(String raw) async {
    ...
    if (CommunityLinks.accept(raw)) return;
    var url = raw.trim();
  ...
  ({Color tint, IconData icon}) get _look {
    switch (_s('style')) {
      ...
      case 'warn':
        return (tint: const Color(0xFF9A5C16), icon: Icons.warning_amber_rounded);
      case 'promo':
        return (tint: const Color(0xFF7855A4), icon: Icons.local_offer_rounded);
      default:
        return (tint: const Color(0xFF226B93), icon: Icons.campaign_rounded);
    }
  }
  ...
  Widget _image(String raw, {bool full = false, bool portrait = false}) {
    ...
    final picture = Image.network(url, width: double.infinity,
      ...
      errorBuilder: (_, __, ___) => const Padding(padding: EdgeInsets.all(24), child: FormaIcon(Icons.image_not_supported_outlined, color: AppColors.muted)),
    );
    return Semantics(button: true, label: _s('imageAction') == 'phone' ? 'اتصل بنا' : 'افتح الإعلان',
      child: InkWell(onTap: () => _s('imageAction') == 'phone' ? _call(_s('phone')) : _open(_s('link')),
  ...
  Widget _close() => IconButton(tooltip: 'إخفاء الإعلان', onPressed: _dismiss,
      ...
      icon: const FormaIcon(Icons.close_rounded, size: 18, color: AppColors.text));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: FormaDecoration(borderRadius: BorderRadius.circular(22),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (!imageOnly) Padding(padding: const EdgeInsets.all(18), child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              FormaIcon(look.icon, color: look.tint, size: 16),
            ...
            if (title.isNotEmpty) ...[
              ...
              Text(title, style: const TextStyle(color: AppColors.text, fontSize: 19, height: 1.4, fontWeight: FontWeight.w900)),
            ],
            if (body.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(body, style: const TextStyle(color: Color(0xFF5B7065), fontSize: 14, height: 1.75)),
            ],
            if (link.isNotEmpty || phone.isNotEmpty) ...[
              ...
              Wrap(spacing: 10, runSpacing: 10, children: [
                if (link.isNotEmpty) FilledButton.icon(
                  ...
                  onPressed: () => _open(link), icon: const FormaIcon(Icons.arrow_outward_rounded, size: 18),
                  label: Text(cta, style: const TextStyle(fontWeight: FontWeight.w900))),
                if (phone.isNotEmpty) OutlinedButton.icon(
                  style: OutlinedButton.styleFrom(foregroundColor: const Color(0xFF267346),
                    side: const BorderSide(color: Color(0xFF277D70)),
                    minimumSize: const Size(140, 48),
                    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                  onPressed: () => _call(phone), icon: const FormaIcon(Icons.call_outlined, size: 18),
