...
import 'package:elforma/screens/workout_week_detail_screen.dart';

class WorkoutHistoryScreen extends StatefulWidget {
  const WorkoutHistoryScreen({super.key, this.mode = WorkoutHistoryMode.history});
  final WorkoutHistoryMode mode;
...
class _WorkoutHistoryScreenState extends State<WorkoutHistoryScreen> {
  ...
  List<Map<String, dynamic>> trainingWeeks = [];
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      if (response.ok) {
        ...
        trainingWeeks = _maps(response.data['trainingWeeks']);
        bestLifts = _maps(response.data['bestLifts']);
        sessions = _maps(response.data['sessions']);
      } else {
        error = response.friendlyError('تعذر تحميل سجل التدريب');
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.bg,
        title: Text(widget.mode == WorkoutHistoryMode.history ? 'سجل التدريب' : 'تحليل الأداء',
            style: const TextStyle(fontWeight: FontWeight.w900)),
        centerTitle: true,
      ),
      body: loading
          ? const Center(child: FormaLoader(color: AppColors.wo))
          : error != null
  ...
  Widget _content() => ListView(
        ...
        children: widget.mode == WorkoutHistoryMode.history ? [
          const Text('الأسابيع المسجلة', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          const Text('اختار أسبوعا ثم يوم التدريب لعرض التمارين والمجموعات',
              style: TextStyle(color: AppColors.textSoft, fontSize: 12.5, height: 1.5)),
          const SizedBox(height: 18),
          _trainingWeeks(),
        ] : [
          _summary(), const SizedBox(height: 14), _coachCard(),
          const SizedBox(height: 14), _weeklyChart(),
          const SizedBox(height: 14), _loadCard(),
          if (bestLifts.isNotEmpty) ...[
            const SizedBox(height: 18),
            const Text('أفضل أرقامك', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
            const SizedBox(height: 9), ...bestLifts.take(5).map(_bestLift),
          ],
        ],
      );
  ...
  Widget _loadCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const FormaIcon(Icons.monitor_heart_rounded, color: AppColors.wo, size: 19),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (delta != null) ...[
                const SizedBox(height: 5),
                Row(children: [
                  FormaIcon(_n(delta) >= 0 ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded,
  ...
  Widget _coachCard() {
    ...
    final color = status == 'deload' ? AppColors.wo2 : warning ? const Color(0xFF996214) : AppColors.nu;
    final muscles = _maps(coach['muscleVolume']);
    final stalls = _maps(coach['stalls']);
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: FormaDecoration(color: color.withValues(alpha: .08), borderRadius: BorderRadius.circular(18),
          border: Border.all(color: color.withValues(alpha: .28))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 40, height: 40, decoration: FormaDecoration(color: color.withValues(alpha: .15), borderRadius: BorderRadius.circular(12)),
              child: FormaIcon(status == 'deload' ? Icons.battery_alert_rounded : Icons.psychology_alt_rounded, color: color)),
        ...
        if (coach['deloadScheduled'] == true) ...[
          const SizedBox(height: 11),
          _note(Icons.battery_saver_rounded, const Color(0xFF996214),
        ...
        if (_names(coach['belowMev']).isNotEmpty) ...[
          ...
          _note(Icons.trending_down_rounded, const Color(0xFF996214),
              ...
              Row(children: [
                ...
                if (grade != 'unknown') ...[
                  Container(
                    ...
                    decoration: FormaDecoration(color: gradeColor.withValues(alpha: .15), borderRadius: BorderRadius.circular(6)),
  ...
  Color _gradeColor(String g, Color fallback) {
    switch (g) {
      case 'below_mev': return const Color(0xFF996214);
  ...
  Widget _note(IconData icon, Color color, String text) => Container(
        ...
        decoration: FormaDecoration(color: color.withValues(alpha: .10), borderRadius: BorderRadius.circular(11),
            border: Border.all(color: color.withValues(alpha: .25))),
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          FormaIcon(icon, color: color, size: 16),
  ...
  Widget _coachChip(String text, Color color) => Container(
        ...
        decoration: FormaDecoration(color: color.withValues(alpha: .12), borderRadius: BorderRadius.circular(9)),
  ...
  Widget _summary() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Row(children: [
            FormaIcon(Icons.query_stats_rounded, color: Colors.white),
            SizedBox(width: 8),
  ...
  Widget _trainingWeeks() {
    if (trainingWeeks.isEmpty) return _empty();
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('أسابيع التدريب', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
      const SizedBox(height: 4),
      const Text('كل أسبوع بيعرض الأيام اللي اتمرنت وسجلتها فعلا فقط',
          style: TextStyle(color: AppColors.textSoft, fontSize: 11.5)),
      const SizedBox(height: 12),
      SizedBox(
        height: 154,
        child: ListView.separated(
          ...
          itemCount: trainingWeeks.length,
          separatorBuilder: (_, __) => const SizedBox(width: 10),
          itemBuilder: (context, index) {
            final week = trainingWeeks[index];
            final current = week['isCurrent'] == true;
            final comparison = week['comparison'] is Map
                ? Map<String, dynamic>.from(week['comparison'] as Map)
                : null;
            final delta = comparison?['volumeDeltaPct'];
            return SizedBox(
              width: 224,
              child: Material(
                color: current ? AppColors.nu : AppColors.card,
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(20),
                  side: BorderSide(color: current ? AppColors.nu : AppColors.line),
                ),
                clipBehavior: Clip.antiAlias,
                child: InkWell(
                  onTap: () => Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) => WorkoutWeekDetailScreen(week: week, latest: current),
                  )),
                  child: Padding(
                    padding: const EdgeInsets.all(15),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Expanded(child: Text(current ? 'الأسبوع الحالي' : 'أسبوع ${_date(week['start'])}',
                            maxLines: 1, overflow: TextOverflow.ellipsis,
                            style: TextStyle(color: current ? Colors.white : AppColors.text,
                                fontWeight: FontWeight.w900, fontSize: 14))),
                        FormaIcon(Icons.chevron_left_rounded,
                            color: current ? Colors.white : AppColors.muted, size: 20),
                      ]),
                      const SizedBox(height: 6),
                      Text('${_date(week['start'])} — ${_date(week['end'])}',
                          maxLines: 1, overflow: TextOverflow.ellipsis,
                          style: TextStyle(color: current ? Colors.white.withValues(alpha: .74) : AppColors.textSoft,
                              fontSize: 10.5)),
                      const Spacer(),
                      Row(children: [
                        _weekMetric('${_n(week['sessionCount']).toInt()}', 'أيام', current),
                        _weekMetric('${_n(week['setCount']).toInt()}', 'مجموعة', current),
                        _weekMetric(_volume(week['volume']), 'حجم', current),
                      ]),
                      if (delta != null) ...[
                        const SizedBox(height: 10),
                        Text('${_n(delta) >= 0 ? '+' : ''}${_n(delta).toStringAsFixed(1)}% عن الأسبوع السابق',
                            style: TextStyle(color: current ? Colors.white.withValues(alpha: .82) : AppColors.muted,
  ...
  Widget _weekMetric(String value, String label, bool inverted) => Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(value, maxLines: 1, overflow: TextOverflow.ellipsis,
              style: TextStyle(color: inverted ? Colors.white : AppColors.text,
                  fontWeight: FontWeight.w900, fontSize: 12.5)),
          const SizedBox(height: 2),
          Text(label, style: TextStyle(color: inverted ? Colors.white.withValues(alpha: .68) : AppColors.muted,
              fontSize: 9)),
        ]),
      );
  ...
  Widget _bestLift(Map<String, dynamic> lift) => Container(
        ...
        decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.line)),
        child: Row(children: [
          Container(width: 39, height: 39, decoration: FormaDecoration(color: AppColors.nu.withValues(alpha: .12), borderRadius: BorderRadius.circular(11)),
              child: const FormaIcon(Icons.emoji_events_outlined, color: AppColors.nu)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _empty() => Container(
        ...
        child: const Column(children: [
          FormaIcon(Icons.history_toggle_off_rounded, color: AppColors.muted, size: 42),
          SizedBox(height: 9),
...
enum WorkoutHistoryMode { history, analysis }
