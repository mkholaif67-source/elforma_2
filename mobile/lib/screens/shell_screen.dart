...
class _ShellScreenState extends State<ShellScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      CommunityLinks.flush();
      unawaited(CommunityLinks.readNativeLink());
    });
  ...
  Future<void> _checkMonthlyWrapUp() async {
    try {
      final accountId = Api.I.accountId;
      if (accountId == null) return;
      final month = _previousMonthKey();
      final prefs = await SharedPreferences.getInstance();
      final seenKey = 'seen_monthly_wrap_up:$accountId:$month';
      if (prefs.getBool(seenKey) == true) return;
      final response = await Api.I.progressStory(month: month);
      if (!mounted || !response.ok || response.data['empty'] == true) return;
      await showMonthlyWrapUp(context, Map<String, dynamic>.from(response.data));
      await prefs.setBool(seenKey, true);
    } catch (_) {
      // A recap is celebratory, never a reason to block opening the app.
    }
  }
