...
class _AnalysisScreenState extends State<AnalysisScreen>
  ...
  Future<void> _run() async {
    ...
    if (!widget.viewOnly) {
      ...
      // تجهيز التمرين عملية ثقيلة (compute + activate + bootstrap) ولا تعتمد
      // عليها دوائر السعرات والماكروز. نشغلها في الخلفية بدل حبس صفحة التحليل
      // وإظهار «حاول تاني» بسبب جزء غير متعلق بنتائج التغذية.
      unawaited(_prepareWorkoutPlan().then((workoutError) {
        if (workoutError != null) {
          debugPrint('[analysis] workout preparation deferred: $workoutError');
        }
      }));
    }
    await _fetch();
