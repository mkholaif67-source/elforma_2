...
// Analysis saves the profile once and awaits nutrition targets only.
// Workout generation belongs to the workout screen; history is optional.
import 'dart:async';
...
class AnalysisScreen extends StatefulWidget {
  ...
  final bool viewOnly;
  final bool profileAlreadySaved;
  const AnalysisScreen({super.key, required this.payload, this.viewOnly = false, this.profileAlreadySaved = false});
...
class _AnalysisScreenState extends State<AnalysisScreen>
  ...
  String _phaseText = 'جار حفظ بياناتك';
  bool _profileSaved = false;

  late final AnimationController _spin;
  Timer? _phaseTimer;
  Timer? _analysisCopyTimer;
  int _analysisCopyIndex = 0;
  static const _analysisCopy = [
    'بنراجع بيانات جسمك ونشاطك',
    'بنوازن احتياجك اليومي مع هدفك',
    'بنرتب تغذيتك مع أيام تمرينك',
    'بنجهز ملخصك وخطتك المناسبة ليك',
  ];
  void _startAnalysisCopy() {
    _analysisCopyTimer?.cancel();
    _analysisCopyIndex = 0;
    _phaseText = _analysisCopy.first;
    _analysisCopyTimer = Timer.periodic(const Duration(milliseconds: 1800), (timer) {
      if (!mounted || !_analyzing || _dataError != null) { timer.cancel(); return; }
      if (_analysisCopyIndex >= _analysisCopy.length - 1) { timer.cancel(); return; }
      setState(() => _phaseText = _analysisCopy[++_analysisCopyIndex]);
    });
  }
  ...
  void initState() {
    ...
    if (widget.viewOnly) _phaseText = 'جار تحميل تحليل بياناتك الحالية';
  ...
  void didChangeDependencies() {
    ...
    if (MediaQuery.disableAnimationsOf(context)) {
      _spin.stop();
      _spin.value = 1;
    } else if (_analyzing && !_spin.isAnimating) {
      _spin.repeat();
    }
  }
  ...
  Future<void> _run() async {
    ...
    _running = true;
    try {
      if (!widget.viewOnly && !widget.profileAlreadySaved && !_profileSaved) {

        final save = await Api.I.saveMobileProfile(widget.payload, preparePlans: true);
        if (!save.ok || save.data['queued'] == true || save.data['profile'] is! Map) {
          if(mounted)setState(() {
            _saveError=save.friendlyError('تعذر تأكيد حفظ البيانات. حاول مرة أخرى');
            _dataError=_saveError; _dataOffline=efIsOffline(save.status);
          });
          return;
        }
        _profileSaved=true;
        await ProfileStore.I.apply(Map<String,dynamic>.from(save.data['profile'] as Map));
      }

      await _fetch();
    } catch (_) {
      if(mounted)setState(()=>_dataError='تعذر تحميل التحليل. بياناتك المحفوظة لم تحذف؛ حاول مرة أخرى');
    } finally {
      _running=false;
      _maybeReveal();
    }
  ...
  Future<void> _fetch() async {
    // [FIX-CHAT-EXIT] الحساب التقيل (توليد خطتي التمرين والتغذية) بيحصل هنا —
    // في صفحة التحليل ذات واجهة الانتظار المصممة — مش جوه الشات. بيشتغل
    // بالتوازي مع جلب الأهداف، فالداشبورد يتكشف أول ما السعرات تجهز والخطط
    // بتكون جاهزة لما المستخدم يوصل للتابات.
    if (!widget.viewOnly) {
      unawaited(Api.I.preparePlansNow().then((r) {
        // فشل التجهيز المسبق مش قاتل: تابات التمرين والتغذية بتحسب عند الطلب.
        if (!r.ok) unawaited(Api.I.sendClientEvent('plan_prepare', 'prepare_failed', 'status=${r.status}'));
      }).catchError((Object _) {}));
    }
    ...
    final targetsFuture = Api.I.nutritionPlan(withMeals: false, force: true);
    ...
    setState(() {
      if (res.ok && res.data['targets'] is Map) {
        ...
        if (res.data['decisionExplanation'] is Map) _decisionExplanation = Map<String, dynamic>.from(res.data['decisionExplanation'] as Map);
        ...
        _dataError = res.ok ? 'وصل رد غير مكتمل. حاول تحميل التحليل مرة أخرى' : res.friendlyError('تعذر تحميل التحليل. حاول مرة أخرى');
      }
    });
    ...
    unawaited(historyFuture.then((history) {
      if (!mounted) return;
      setState(() {
        if(history.ok && history.data['load'] is Map) _loadStats=Map<String,dynamic>.from(history.data['load'] as Map);
        if(history.ok && history.data['coach'] is Map) _coach=Map<String,dynamic>.from(history.data['coach'] as Map);
        if(history.ok && history.data['bestLifts'] is List) _bestLifts=history.data['bestLifts'] as List;
      });
    }).catchError((Object _) {}));
  }
  ...
  Future<void> _retryData() async {
    ...
    setState(() { _dataError = null; _saveError = null; _analyzing=true; });
    _startAnalysisCopy();
    await _run();
  }
  ...
  void _revealDashboard() {
    if (!mounted || !_analyzing) return;
    // The 1400ms spinner used to keep repeating for the entire lifetime of the
    // screen. After the reveal it is not even painted any more, yet it still
    // requested a vsync callback on every single frame and re-ran
    // _SpinnerPainter. Stopping it here ends that permanent 60fps wakeup.
    _spin.stop();
      ...
      child: Column(
        ...
        children: [
            ...
            child: AnimatedBuilder(
              ...
              builder: (_, __) {
                return CustomPaint(
                  ...
                  child: const Center(
                    child: FormaIcon(Icons.insights_rounded,
          ...
          AnimatedSwitcher(
            ...
            child: Text(_phaseText,key:ValueKey(_phaseText),textAlign:TextAlign.center,
              style:const TextStyle(fontSize:15,color:AppColors.nu2,fontWeight:FontWeight.w700)),
          ),
  ...
  Widget _dashboardView() {
    if(_targets==null){
      return ErrorView(message:_dataError??'تعذر تحميل التحليل. حاول مرة أخرى',offline:_dataOffline,onRetry:_retryData);
    }
    final bmi = _bmi;
    return ListView(
      ...
      children: [
        Row(
          children: [
            Container(
              ...
              child: const FormaIcon(Icons.verified_rounded, color: Color(0xFFFFFFFF), size: 30),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(widget.viewOnly ? 'تحليل بياناتك' : 'تحليل بياناتك جاهز',
                      style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 3),
                  Text(widget.viewOnly ? 'ملخص كامل مبني على أرقامك إنت' : 'النتائج مبنية على بياناتك الحالية',
        ...
        _caloriesCard(),
        // [FIX-HIDE-INTERNALS] تفاصيل الحساب الداخلية (الوزن اللي اتبني عليه
        // الحساب وسعرات الحفاظ) بتفضل موجودة ومحفوظة جوا _targets['calculation']
        // لأي استخدام نظامي، لكنها معلومة داخلية ملهاش معنى للمستخدم —
        // اتشال سطر النص اللي كان ظاهر تحت كارت السعرات من غير أي تأثير على البيانات.
    ...
    return _card(
      child: Column(
        ...
        children: [
          Row(
            children: [
              FormaIcon(Icons.straighten_rounded, color: color, size: 20),
    ...
    return _card(
      child: Column(
        ...
        children: [
          Row(
            children: [
              const FormaIcon(Icons.timeline_rounded, color: AppColors.nu, size: 20),
              const SizedBox(width: 8),
  ...
  // Render the normalized Phase 2 decision from the existing nutrition engine.
  // This keeps the server decision visible and prevents fetching it as unused
  // decoration; no nutrition rule is duplicated on the device.
  Widget? _decisionCard() {
    final x = _decisionExplanation;
    if (x == null || x['available'] != true) return null;
    ...
    final d = x['decision'] is Map
        ...
        : const <String, dynamic>{};
    final title = '${e['labelAr'] ?? ''}'.trim();
    final finding = '${e['findingAr'] ?? ''}'.trim();
    final actionText = '${e['actionAr'] ?? ''}'.trim();
    final action = '${d['action'] ?? ''}'.trim();
    final blockedBy = '${d['blockedBy'] ?? ''}'.trim();
    final delta = _num(d['deltaCals']).round();
    final deltaText = delta == 0
        ? 'لا يوجد تغيير في السعرات حاليا'
        : '${delta > 0 ? 'زيادة' : 'خفض'} ${delta.abs()} سعرة';
    return _statCard(
      icon: Icons.psychology_rounded,
      color: AppColors.nu2,
      title: title.isEmpty ? 'قرار المدرب الغذائي' : title,
      extra: [
        const SizedBox(height: 8),
        Wrap(spacing: 8, runSpacing: 8, children: [
          if (action.isNotEmpty) _pill(action, AppColors.nu2),
          _pill(deltaText, AppColors.nu),
          if (blockedBy.isNotEmpty)
            _pill('التعديل متوقف بسبب $blockedBy', const Color(0xFF996214)),
        ]),
      ],
      note: actionText.isNotEmpty
          ? actionText
          : (finding.isNotEmpty ? finding : 'القرار مبني على بيانات الخطة الحالية'),
    );
  }
  ...
  Widget? _loadCard() {
    ...
    if (zone == 'high' || zone == 'danger') {
      zoneColor = const Color(0xFFB34343);
    } else if (zone == 'low') {
      zoneColor = const Color(0xFF996214);
    }
    ...
    return _card(
      child: Column(
        ...
        children: [
          Row(
            children: [
              const FormaIcon(Icons.fitness_center_rounded,
  ...
  Widget? _adaptiveCard() {
    ...
    if (status != 'ready' || observed <= 0) {
      ...
      return _statCard(
        ...
        note: logged > 0
            ? 'سجلت $logged يوم أكل حتى دلوقتي. محتاجين 14 يوم أكل مسجل و 4 قياسات وزن على الأقل ونقدر نقول صرفك الحقيقي بدل المعادلة'
  ...
  /* الالتزام: مافيش خطة بتفشل وهي متطابقة. قبل أي تعديل في الأرقام
  ...
  Widget? _plateauCard() {
    ...
    final title = primary != null ? '${primary['labelAr'] ?? primary['titleAr'] ?? primary['title'] ?? ''}' : '';
    ...
    return _statCard(
      ...
      extra: [
        ...
        if (blockedBy.isNotEmpty) ...[
          const SizedBox(height: 8),
          _pill('منعنا تقليل السعرات', const Color(0xFFB34343)),
        ],
      ],
      note: action.isEmpty
  ...
  Widget? _recoveryCard() {
    ...
    final color = deload ? const Color(0xFFB34343) : AppColors.nu;
    return _statCard(
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          ...
          if (avgRir != null)
            _pill('متوسط RIR ${_num(avgRir).toStringAsFixed(1)}',
                const Color(0xFF226B93)),
          if (deload) _pill('أسبوع تخفيف مستحق', const Color(0xFFB34343)),
        ]),
      ],
    ...
    return _statCard(
      ...
      extra: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          ...
          if (water > 0) _pill('مية $water مل يومي', AppColors.nu2),
          if (fiber > 0) _pill('ألياف $fiber جم يومي', const Color(0xFF226B93)),
        ]),
      ],
      note: perKg > 0
          ? (perKg < 1.6
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _macroRing('بروتين', p, share(pk), const Color(0xFF3468B6)),
  ...
  Widget _bmiCard(double bmi) {
    ...
    Color col;
    if (bmi < 18.5) {
      cat = 'تحت الطبيعي'; col = const Color(0xFF3468B6);
    } else if (bmi < 25) {
      cat = 'طبيعي'; col = AppColors.nu;
    } else if (bmi < 30) {
      cat = 'زيادة بسيطة'; col = const Color(0xFF996214);
    } else {
                  ...
                  Container(
                    ...
                    decoration: FormaDecoration(
                      ...
                      gradient: const LinearGradient(colors: [
                        Color(0xFF3468B6), AppColors.nu, Color(0xFF996214), AppColors.wo,
                      ]),
                    ),
                  ),
                  Positioned(
  ...
  Widget _infoTile(IconData icon, String label, String value, Color color) {
    return Container(
      ...
      child: Row(
        children: [
          FormaIcon(icon, color: color, size: 22),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
      ...
      children: [
        SizedBox(
          ...
          child: FilledButton(
            ...
            child: Text(_confirmed ? 'متابعة إلى خطتي' : 'في انتظار اكتمال التحليل',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
          ),
        ),
        const SizedBox(height: 10),
        Text(_confirmed && _dataError==null ? 'سننتقل إلى الخطة خلال $_left ثانية' : 'أعد المحاولة لإكمال التحليل',
            style: const TextStyle(fontSize: 12.5, color: AppColors.muted)),
      ],
    );
  }
