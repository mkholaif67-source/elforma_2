...
import 'package:flutter/services.dart'; // [UI-COMPLETION] HapticFeedback
...
import 'package:elforma/models/meal_display.dart';
...
import 'package:elforma/screens/food_picker_screen.dart';
import 'package:elforma/screens/meal_favorites_screen.dart';
...
class _MealPlanScreenState extends State<MealPlanScreen> {
  ...
  String _planContextRevision = '';
  String? _portionNotice;
  num _planCalories = 0;
  num _planProtein = 0;
  num _planCarbs = 0;
  num _planFat = 0;
  ...
  Map<String, dynamic>? _decisionExplanation;
  ...
  void initState() {
    ...
    Api.I.nutritionRevision.addListener(_onNutritionFresh); // [INSTANT-OPEN] SWR
  ...
  /// [SWR] Called when background nutrition refresh completes.
  void _onNutritionFresh() { if (mounted && _isToday) _load(); } // [PERF] SWR بس لو شايف النهارده

  void _onProfileChanged(){
    if(!mounted)return;
    final p=ProfileStore.I.profile;
    final signature=p?.toString();
    if(signature==_profileSignature)return;
    _profileSignature=signature;
    if(ProfileStore.I.loaded)_load();
  }
  void _onSubChanged() {
    if (!mounted) return;
  ...
  Future<void> _load({bool recovering = false, bool force = false}) async {
    ...
    if(ProfileStore.I.loadError!=null && ProfileStore.I.profile==null){
      setState((){_busy=false;_error=ProfileStore.I.loadError;_needsSetup=false;});
      return;
    }
    ...
    setState(() {
      _busy = _meals.isEmpty || _loadedDayOffset!=_dayOffset;
      _error = null;
      _needsSetup = false;
    });
    ...
    final responses = await Future.wait<ApiResult>([
      Api.I.nutritionPlan(withMeals: true, dayOffset: _dayOffset, force: force),
    ...
    if (!response.ok || response.data['plan'] is! Map) {
      ...
      setState(() {
        _busy = false;
        if (needsProfile && !ProfileStore.I.isComplete) {
          _needsSetup = true;
        } else {
          _error = response.friendlyError('تعذر تحميل خطة التغذية. حاول مرة أخرى');
        }
      });
      return;
    }
    ...
    final planRevision = '${plan['_contextRevision'] ?? ''}';
    final savedRevision = savedMeals.isNotEmpty && savedMeals.first is Map
        ? '${(savedMeals.first as Map)['_planContextRevision'] ?? ''}' : '';
    final sameContext = planRevision.isNotEmpty && planRevision == savedRevision;
    // Completed entries are historical records, not automatic recommendations.
    // Preserve them; only replace unconsumed suggestions from an obsolete plan.
    final recordedMeals = <String, NutritionMeal>{};
    for (var i = 0; i < editableMeals.length && i < savedMeals.length; i++) {
      if (savedMeals[i] is Map && (savedMeals[i] as Map)['completed'] == true) {
        recordedMeals[editableMeals[i].key] = editableMeals[i];
      }
    }
    final useSavedMeals = sameContext && editableMeals.isNotEmpty && serverTarget > 0 &&
    ...
    setState(() {
      ...
      _decisionExplanation = response.data['decisionExplanation'] is Map ? Map<String, dynamic>.from(response.data['decisionExplanation'] as Map) : null;
      _adjustment = adjustment;
      _checkin = checkin;
      _meals = (useSavedMeals ? editableMeals : meals.map((m) => recordedMeals[m.key] ?? m).toList())
      ...
      final newDay = _loadedDayOffset != _dayOffset;
      if (newDay) _expanded.clear();
      _expanded.removeWhere((key) => !_meals.any((m) => m.key == key));
      if (_expanded.length > 1) { final keep = _expanded.first; _expanded..clear()..add(keep); }
      if ((newDay || _expanded.isEmpty) && _meals.isNotEmpty) {
        _expanded..clear()..add(_meals.first.key);
      }
      _loadedDayOffset=_dayOffset;
      _targetCalories = serverTarget;
      _planContextRevision = planRevision;
      _portionNotice=plan['portionLimited']==true?'الأصناف المتاحة لا تغطي هدفك بالكامل جرب زيادة عدد الوجبات أو تنويع الأصناف':null;
      ...
      if (useSavedMeals || recordedMeals.isNotEmpty) _recalculate();
      _busy = false;
    });
    ...
    } catch (_) { /* Reminders cannot invalidate a successfully loaded meal plan. */ }
    } catch (_) {
      if(mounted&&generation==_loadGeneration)setState(()=>_error='تعذر عرض خطة التغذية. حاول مرة أخرى؛ بياناتك محفوظة');
    } finally {
      if(mounted&&generation==_loadGeneration)setState(()=>_busy=false);
    }
  }
  ...
  Future<void> _saveDay() async {
    ...
    if (mealPayloads.isNotEmpty) {
      ...
      mealPayloads.first['_planContextRevision'] = _planContextRevision;
    }
    ...
    if (!response.ok) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.friendlyError('تعذر حفظ يومك'))));
    }
  }
    ...
    final value = await showDialog<num>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            if (foodPhoto(name) != null) ...[
              Center(child: FoodPhoto(name: name, size: 96)),
  ...
  Future<void> _toggleCompleted(NutritionMeal meal) async {
    final wasCompleted = _dayCompleted; // [UI-COMPLETION] قبل التغيير
    setState(() => _completed.contains(meal.key)
        ? _completed.remove(meal.key)
        : _completed.add(meal.key));
    // [UI-COMPLETION] vibration: ثقيل لما تكمل كل وجبات اليوم، خفيف للوجبة الواحدة
    if (!wasCompleted && _dayCompleted) {
      ...
      final mealsData = _meals.map((m) {
        ...
        final foods = shown.map((f) {
          ...
          return <String, dynamic>{
            ...
            'grams': f.grams,
            'cals': f.calories,
            'pro': f.protein, 'carb': f.carbs, 'fat': f.fat,
          };
        }).toList();
        return <String, dynamic>{
          'name': m.name,
  ...
  void dispose() {
    ...
    Api.I.nutritionRevision.removeListener(_onNutritionFresh); // [INSTANT-OPEN]
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.bg,
        title: const Text('نظامك الغذائي', style: TextStyle(fontWeight: FontWeight.w900)),
        centerTitle: true,
        actions: [
          if (_canExport)
            IconButton(
  ...
  Widget _errorView() => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const FormaIcon(Icons.error_outline, color: AppColors.wo2, size: 44),
            const SizedBox(height: 14),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const FormaIcon(Icons.restaurant_menu_rounded,
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_decisionExplanation?['available'] == true && (_diagnosis?['stalled'] != true)) ...[
            _decisionExplanationCard(),
            const SizedBox(height: 12),
          ],
          ...
          const SizedBox(height: 10),
          // [UI-COMPLETION] AnimatedSwitcher: بتظهر/تتخفى hero card بـ fade+slide
          AnimatedSwitcher(
            ...
            child: (_isToday && _dayCompleted)
                ? Column(key: const ValueKey('nutr_hero'), children: [
                    _nutritionCompletedHero(),
                    const SizedBox(height: 14),
                  ])
                : const SizedBox.shrink(key: ValueKey('nutr_empty')),
          ),
  ...
  Future<void> _answerCheckin(dynamic raw) async {
    ...
    if (!result.ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(result.friendlyError('تعذر حفظ ردك'))));
    }
  }
  ...
  Widget _daySelector(){
    Widget dayButton(int offset) {
      final selected = offset == _dayOffset;
      return Semantics(
        selected: selected,
        child: OutlinedButton(
          style: OutlinedButton.styleFrom(
            foregroundColor: selected ? const Color(0xFFFFFFFF) : AppColors.text,
            backgroundColor: selected ? AppColors.nu : AppColors.card,
            side: BorderSide(color: selected ? AppColors.nu : AppColors.line),
          ...
          onPressed: () {
            ...
            _daySelectionTimer = Timer(const Duration(milliseconds: 150), () { if (mounted) _load(); });
          },
          child: Text(_dayLabel(offset), textAlign: TextAlign.center),
        ),
      );
    }
    final offsets = <int>[0, 1, 2, 3, 4, 5, 6, if (_hasYesterdayData) -1];
    Widget dayRow(int start) => Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [for (int col = 0; col < 4; col++) ...[
        if (col > 0) const SizedBox(width: 6),
        Expanded(child: start + col < offsets.length
          ? dayButton(offsets[start + col]) : const SizedBox.shrink()),
      ]],
    );
    return Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      // زرا تجهيز مبكر + إضافة مفضلة في نفس الصف — متناسقين
      Row(children:[
        Expanded(child:OutlinedButton.icon(
          style:OutlinedButton.styleFrom(
            foregroundColor:AppColors.nu2,
            side:BorderSide(color:AppColors.nu.withValues(alpha:.5)),
            shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(14)),
            padding:const EdgeInsets.symmetric(vertical:14),
          ),
          onPressed:(){setState((){_showFullWeek=!_showFullWeek;if(!_showFullWeek)_dayOffset=0;});if(!_showFullWeek)_load();},
          icon:FormaIcon(_showFullWeek?Icons.expand_less:Icons.calendar_month_outlined,size:18),
          label:const Text('تجهيز مبكر',style:TextStyle(fontWeight:FontWeight.w800,fontSize:14)))),
        ...
        Expanded(child:OutlinedButton.icon(
          ...
          onPressed:()async{
            await Navigator.of(context).push<bool>(MaterialPageRoute(builder:(_)=>const MealFavoritesScreen()));
            if(mounted){Api.I.invalidateNutritionPlans();await _load(force:true);}
          },
          icon:const FormaIcon(Icons.favorite_border,size:18),
          label:const Text('أطعمة مفضلة',style:TextStyle(fontWeight:FontWeight.w800,fontSize:14)))),
      ]),
      // Four equal columns, exactly two horizontal rows, including yesterday.
      if (_showFullWeek) Padding(
        padding: const EdgeInsets.only(top: 12),
        child: Column(children: [dayRow(0), const SizedBox(height: 8), dayRow(4)]),
      ),
    ]);
  }
  ...
  Widget _previewBanner() {
    return Container(
      ...
      child: Row(children: [
        const FormaIcon(Icons.visibility_outlined, color: AppColors.nu2, size: 18),
        const SizedBox(width: 8),
        Expanded(
          child: Text('معاينة يوم ${_dayLabel(_dayOffset)} · التسجيل والتحليل خاصين بيوم النهارده',
  ...
  Widget _waterTracker() {
    ...
    return Container(
      padding: const EdgeInsets.all(15),
      decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(17),
          border: Border.all(color: const Color(0xFF226B93).withValues(alpha: .35))),
      child: Column(children: [
        Row(children: [
          const FormaIcon(Icons.water_drop_rounded, color: Color(0xFF226B93)),
          const SizedBox(width: 8),
          ...
          Text('${(_waterMl / 1000).toStringAsFixed(1)} / 2.5 لتر',
              style: const TextStyle(color: Color(0xFF226B93), fontWeight: FontWeight.w900)),
        ]),
        const SizedBox(height: 10),
        LinearProgressIndicator(value: progress, minHeight: 7, borderRadius: BorderRadius.circular(8),
            color: const Color(0xFF226B93), backgroundColor: AppColors.line),
        const SizedBox(height: 8),
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          TextButton(onPressed: _saving ? null : () => _addWater(-250), child: const Text('- كوب')),
          FilledButton(
            style: FilledButton.styleFrom(backgroundColor: const Color(0xFF226B93)),
  ...
  Widget _decisionExplanationCard() {
    final x=_decisionExplanation??const <String,dynamic>{};final e=x['explanation'] is Map?Map<String,dynamic>.from(x['explanation'] as Map):const <String,dynamic>{};final d=x['decision'] is Map?Map<String,dynamic>.from(x['decision'] as Map):const <String,dynamic>{};
    return Container(padding:const EdgeInsets.all(15),decoration:FormaDecoration(color:AppColors.card,borderRadius:BorderRadius.circular(18),border:Border.all(color:AppColors.nu.withValues(alpha:.35))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[const FormaIcon(Icons.auto_awesome_rounded,color:AppColors.nu,size:18),const SizedBox(width:7),Expanded(child:Text('${e['labelAr']??'قرار الخطة'}',style:const TextStyle(fontWeight:FontWeight.w900)))]),if('${e['findingAr']??''}'.isNotEmpty)...[const SizedBox(height:8),Text('${e['findingAr']}',style:const TextStyle(fontSize:12.5,height:1.5))],if('${e['actionAr']??''}'.isNotEmpty)...[const SizedBox(height:7),Text('${e['actionAr']}',style:const TextStyle(fontSize:12.5,height:1.5))],const SizedBox(height:7),Text('القرار: ${d['action']=='hold'?'تثبيت الخطة':d['action']=='increase'?'زيادة تدريجية':'تقليل تدريجي'}',style:const TextStyle(color:AppColors.nu,fontWeight:FontWeight.w800,fontSize:12))]));
  }
  ...
  Widget? _diagnosisCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const FormaIcon(Icons.troubleshoot_rounded,
        ...
        if (action.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const FormaIcon(Icons.arrow_forward_rounded,
        ...
        if (blocked) ...[
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const FormaIcon(Icons.shield_rounded,
  ...
  Widget _dailySummary() {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Text('${calories.round()}',
            style: const TextStyle(color: Color(0xFFFFFFFF), fontSize: 42, fontWeight: FontWeight.w900)),
        Text(
            target <= 0
                ? 'سعرة حرارية'
                : over
            ...
            style: const TextStyle(color: Color(0xFFEAF2E4))),
  ...
  Widget _summaryMacro(String label, num value) => Expanded(
        child: Column(children: [
          Text('${value.round()} جم',
              style: const TextStyle(color: Color(0xFFFFFFFF), fontWeight: FontWeight.w900)),
          Text(label, style: const TextStyle(color: Color(0xFFEAF2E4), fontSize: 11)),
        ]),
      );
      ...
      child: Column(children: [
        InkWell(
          ...
          child: Padding(
            padding: const EdgeInsets.all(15),
            child: Row(children: [
              MealArt(names: meal.foods.where((f) => !f.isAddon).map((f) => f.name).toList()),
              ...
              FormaIcon(isOpen ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded,
                  color: AppColors.nu),
            ]),
          ),
        ),
        if (isOpen) ...[
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              _macroChip('C', meal.carbs, const Color(0xFF226B93)),
              ...
              if (!(_isToday && _dayCompleted)) ...[
                ...
                Expanded(child: FilledButton(
                  style: FilledButton.styleFrom(
                    ...
                    foregroundColor: done ? const Color(0xFFFFFFFF) : AppColors.text,
                ...
                const Expanded(
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    FormaIcon(Icons.check_circle_rounded, color: AppColors.nu, size: 18),
                    SizedBox(width: 6),
                    Text('تمت الوجبة',
        ...
        Row(children: [
          Container(
            ...
            decoration: FormaDecoration(
                ...
                shape: BoxShape.circle),
            child: const FormaIcon(Icons.restaurant_rounded,
  ...
  Widget _macroChip(String label, num value, Color color) => Expanded(
        child: Container(
          ...
          decoration: FormaDecoration(color: color.withValues(alpha: .10), borderRadius: BorderRadius.circular(9)),
  ...
  Widget _foodRow(NutritionMeal meal, int index, NutritionFoodItem item) {
    ...
    return Padding(
        ...
        child: Row(children: [
          if (!addon) FoodPhoto(name: item.name, size: 38)
          else const FormaIcon(Icons.add_rounded, size: 12, color: AppColors.muted),
          ...
          if (index >= 0)
            SizedBox(
              ...
              child: PopupMenuButton<String>(
                ...
                icon: const FormaIcon(Icons.more_vert, color: AppColors.muted, size: 20),
                onSelected: (value) {
  ...
  Widget _lockedCard(int remaining) => Container(
        ...
        child: Column(children: [
          const FormaIcon(Icons.lock_outline, color: AppColors.wo2, size: 36),
          const SizedBox(height: 10),
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              const FormaIcon(Icons.eco_rounded, color: AppColors.nu, size: 22),
              const SizedBox(width: 8),
              const Text('إضافة عنصر للسلطة',
                  style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
              const Spacer(),
              IconButton(
                icon: const FormaIcon(Icons.close),
