...
class _MealPlanScreenState extends State<MealPlanScreen> {
  ...
  void initState() {
    super.initState();
    ProfileStore.I
        .addListener(_onProfileChanged); // refresh when profile updates
    SubscriptionStore.I
        .addListener(_onSubChanged); // refresh on subscription change
    Api.I.nutritionRevision
        .addListener(_onNutritionFresh); // [INSTANT-OPEN] SWR
  ...
  void _onNutritionFresh() {
    if (mounted && _isToday) _load();
  } // [PERF] SWR بس لو شايف النهارده
  ...
  void _onSubChanged() {
    ...
    if (np != _lastSubPlan || na != _lastSubActive) {
      // [FIX-10]
      _lastSubPlan = np;
      _lastSubActive = na;
      unawaited(_load());
  ...
  Future<void> _load({bool recovering = false, bool force = false}) async {
    ...
    try {
      ...
      if (ProfileStore.I.loadError != null && ProfileStore.I.profile == null) {
        setState(() {
          _busy = false;
          _error = ProfileStore.I.loadError;
          _needsSetup = false;
        });
        return;
      }
      ...
      final responses = await Future.wait<ApiResult>([
        Api.I.nutritionPlan(
            withMeals: true, dayOffset: _dayOffset, force: force),
      ...
      if (!response.ok || response.data['plan'] is! Map) {
        ...
        setState(() {
          ...
          if (needsProfile && !ProfileStore.I.isComplete) {
            ...
            _error =
                response.friendlyError('تعذر تحميل خطة التغذية. حاول مرة أخرى');
          }
        });
        return;
      }
      ...
      for (var i = 0; i < rawMeals.length; i++) {
        if (rawMeals[i] is Map)
          meals.add(NutritionMeal.fromJson(rawMeals[i] as Map, i));
      }
      final totals = plan['totals'] is Map ? plan['totals'] as Map : const {};
      final targets = response.data['targets'] is Map
          ? response.data['targets'] as Map
          : const {};
      final saved = (isToday &&
              bootstrap != null &&
              bootstrap.data['nutritionToday'] is Map)
          ? bootstrap.data['nutritionToday'] as Map
          : const {};
      final savedMeals =
          saved['meals'] is List ? saved['meals'] as List : const [];
      ...
      final comparableSavedTarget =
          savedTarget > 0 ? savedTarget : savedCalories;
      ...
      final savedRevision = savedMeals.isNotEmpty && savedMeals.first is Map
          ...
          : '';
      final sameContext =
          planRevision.isNotEmpty && planRevision == savedRevision;
      ...
      for (var i = 0; i < editableMeals.length && i < savedMeals.length; i++) {
        if (savedMeals[i] is Map &&
            (savedMeals[i] as Map)['completed'] == true) {
          recordedMeals[editableMeals[i].key] = editableMeals[i];
        }
      }
      final useSavedMeals = sameContext &&
          editableMeals.isNotEmpty &&
          serverTarget > 0 &&
      ...
      setState(() {
        ...
        _decisionExplanation = response.data['decisionExplanation'] is Map
            ? Map<String, dynamic>.from(
                response.data['decisionExplanation'] as Map)
            : null;
        _adjustment = adjustment;
        _checkin = checkin;
        _meals = (useSavedMeals
                ? editableMeals
                : meals.map((m) => recordedMeals[m.key] ?? m).toList())
        ...
        if (_expanded.length > 1) {
          final keep = _expanded.first;
          _expanded
            ..clear()
            ..add(keep);
        }
        if ((newDay || _expanded.isEmpty) && _meals.isNotEmpty) {
          _expanded
            ..clear()
            ..add(_meals.first.key);
        }
        _loadedDayOffset = _dayOffset;
        _targetCalories = serverTarget;
        _planContextRevision = planRevision;
        _portionNotice = plan['portionLimited'] == true
            ? 'الأصناف المتاحة لا تغطي هدفك بالكامل جرب زيادة عدد الوجبات أو تنويع الأصناف'
            : null;
      ...
      try {
        ...
        if (isToday)
          await _syncMealReminders(_meals.isNotEmpty ? _meals : meals);
      } catch (_) {
        /* Reminders cannot invalidate a successfully loaded meal plan. */
      }
    } catch (_) {
      if (mounted && generation == _loadGeneration)
        setState(() =>
            _error = 'تعذر عرض خطة التغذية. حاول مرة أخرى؛ بياناتك محفوظة');
    } finally {
      if (mounted && generation == _loadGeneration)
  ...
  Future<void> _saveDay() async {
    ...
    if (!response.ok) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(response.friendlyError('تعذر حفظ يومك'))));
    }
  }

  Future<void> _pickFood(NutritionMeal meal, {int? replaceIndex}) async {
    final food =
        await Navigator.of(context).push<NutritionFoodItem>(MaterialPageRoute(
      builder: (_) => FoodPickerScreen(
        diet: (_profile['selectedDiet'] ?? 'balanced').toString(),
        health: _profile['healthConditions'] is List
            ? (_profile['healthConditions'] as List)
                .whereType<String>()
                .toList()
            : const [],
        replacementTarget: replaceIndex != null &&
                replaceIndex >= 0 &&
                replaceIndex < meal.foods.length
    ...
    if (overflow && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'ضفناه بالكامل وهيعدي هدفك ب ${(_planCalories - _targetCalories).round()} سعرة - شوف لو تحب تقلل صنف تاني'),
      ));
    }
    if (trimmed && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Future<void> _editGrams(
    ...
    final grams = await _askGrams(item.name, item.grams, isAddon: item.isAddon);
  ...
  Future<void> _downloadPlan() async {
    if (_meals.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('احفظ خطة تغذيتك الأول قبل ما تحملها')));
      return;
    }
    try {
  ...
  Widget _errorView() => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const FormaIcon(Icons.error_outline,
                color: AppColors.wo2, size: 44),
            const SizedBox(height: 14),
            Text(_error!,
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_decisionExplanation?['available'] == true &&
              (_diagnosis?['stalled'] != true)) ...[
        ...
        Row(children: [
          for (var i = 0; i < options.length; i++) ...[
            ...
            Expanded(
              ...
              child: Text('${(options[i] as Map)['label'] ?? ''}',
                  textAlign: TextAlign.center),
            )),
          ],
        ]),
      ...
      return Semantics(
        ...
        child: OutlinedButton(
          style: OutlinedButton.styleFrom(
            foregroundColor:
                selected ? const Color(0xFFFFFFFF) : AppColors.text,
          ...
          onPressed: () {
            ...
            _daySelectionTimer = Timer(const Duration(milliseconds: 150), () {
              if (mounted) _load();
            });
          },
    ...
    Widget dayRow(int start) => Row(
          ...
          children: [
            for (int col = 0; col < 4; col++) ...[
              ...
              Expanded(
                  child: start + col < offsets.length
                      ? dayButton(offsets[start + col])
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            child: OutlinedButton.icon(
                ...
                onPressed: () {
                  setState(() {
                    ...
                    if (!_showFullWeek) _dayOffset = 0;
                  });
                  if (!_showFullWeek) _load();
                },
                icon: FormaIcon(
                    _showFullWeek
                        ? Icons.expand_less
            ...
            child: OutlinedButton.icon(
                ...
                onPressed: () async {
                  await Navigator.of(context).push<bool>(MaterialPageRoute(
                      builder: (_) => const MealFavoritesScreen()));
                  if (mounted) {
                    Api.I.invalidateNutritionPlans();
                    await _load(force: true);
                  }
                },
                icon: const FormaIcon(Icons.favorite_border, size: 18),
                label: const Text('أطعمة مفضلة',
                    style:
                        TextStyle(fontWeight: FontWeight.w800, fontSize: 14)))),
      ]),
      ...
      if (_showFullWeek)
        Padding(
          ...
          child: Column(
              children: [dayRow(0), const SizedBox(height: 8), dayRow(4)]),
        ),
    ]);
  }
  ...
  Widget _previewBanner() {
    return Container(
      ...
      child: Row(children: [
        const FormaIcon(Icons.visibility_outlined,
            color: AppColors.nu2, size: 18),
        const SizedBox(width: 8),
        Expanded(
          child: Text(
              'معاينة يوم ${_dayLabel(_dayOffset)} · التسجيل والتحليل خاصين بيوم النهارده',
              style: const TextStyle(
                  color: AppColors.nu2, fontSize: 12, height: 1.4)),
        ),
      ]),
    );
  }
  ...
  Widget _waterTracker() {
    ...
    return Container(
      ...
      decoration: FormaDecoration(
          ...
          border: Border.all(
              color: const Color(0xFF226B93).withValues(alpha: .35))),
      child: Column(children: [
        Row(children: [
          const FormaIcon(Icons.water_drop_rounded, color: Color(0xFF226B93)),
          const SizedBox(width: 8),
          const Expanded(
              child: Text('مياه اليوم',
                  style: TextStyle(fontWeight: FontWeight.w900))),
          Text('${(_waterMl / 1000).toStringAsFixed(1)} / 2.5 لتر',
              style: const TextStyle(
                  color: Color(0xFF226B93), fontWeight: FontWeight.w900)),
        ]),
        const SizedBox(height: 10),
        LinearProgressIndicator(
            ...
            color: const Color(0xFF226B93),
            backgroundColor: AppColors.line),
        const SizedBox(height: 8),
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          TextButton(
              onPressed: _saving ? null : () => _addWater(-250),
              child: const Text('- كوب')),
          FilledButton(
            style: FilledButton.styleFrom(
                backgroundColor: const Color(0xFF226B93)),
            onPressed: _saving ? null : () => _addWater(250),
  ...
  Widget _decisionExplanationCard() {
    final x = _decisionExplanation ?? const <String, dynamic>{};
    final e = x['explanation'] is Map
        ? Map<String, dynamic>.from(x['explanation'] as Map)
        : const <String, dynamic>{};
    final d = x['decision'] is Map
        ? Map<String, dynamic>.from(x['decision'] as Map)
        : const <String, dynamic>{};
    return Container(
        padding: const EdgeInsets.all(15),
        decoration: FormaDecoration(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const FormaIcon(Icons.auto_awesome_rounded,
                color: AppColors.nu, size: 18),
            const SizedBox(width: 7),
            Expanded(
                child: Text('${e['labelAr'] ?? 'قرار الخطة'}',
                    style: const TextStyle(fontWeight: FontWeight.w900)))
          ]),
          if ('${e['findingAr'] ?? ''}'.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text('${e['findingAr']}',
                style: const TextStyle(fontSize: 12.5, height: 1.5))
          ],
          if ('${e['actionAr'] ?? ''}'.isNotEmpty) ...[
          ...
          const SizedBox(height: 7),
          Text(
              'القرار: ${d['action'] == 'hold' ? 'تثبيت الخطة' : d['action'] == 'increase' ? 'زيادة تدريجية' : 'تقليل تدريجي'}',
              style: const TextStyle(
                  color: AppColors.nu,
  ...
  Widget? _diagnosisCard() {
    ...
    final blocked =
        _adjustment != null && '${_adjustment!['blockedBy'] ?? ''}'.isNotEmpty;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (finding.isNotEmpty) ...[
          ...
          Text(finding, style: const TextStyle(fontSize: 12.5, height: 1.6)),
        ],
        ...
        if (lowConfidence) ...[
          const SizedBox(height: 9),
          const Text('ملحوظة: البيانات لسة قليلة فده ترجيح مش تأكيد',
  ...
  Widget _dailySummary() {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Text(
            target > 0
                ? 'سعرات خطتك من أصل ${target.round()} سعرة'
                : 'إجمالي يومك',
            style: const TextStyle(
  ...
  Widget _summaryMacro(String label, num value) => Expanded(
        child: Column(children: [
          ...
          Text(label,
              style: const TextStyle(color: Color(0xFFEAF2E4), fontSize: 11)),
        ]),
      );
  ...
  Widget _mealCard(NutritionMeal meal) {
    ...
    return AnimatedContainer(
      ...
      decoration: FormaDecoration(
        ...
        border: Border.all(
            color:
                isOpen ? AppColors.nu.withValues(alpha: .55) : AppColors.line),
      ),
      child: Column(children: [
        InkWell(
          borderRadius: BorderRadius.circular(19),
            ...
            child: Row(children: [
              ...
              Expanded(
                  child: Column(
                    ...
                    const SizedBox(height: 4),
                    Text(
                        '${displayFoods(meal.foods).length} أصناف · ${meal.calories.round()} سعرة',
                        style: const TextStyle(
              ...
              if (!(_isToday && _dayCompleted)) ...[
                ...
                Expanded(
                  ...
                  style: FilledButton.styleFrom(
                    ...
                    foregroundColor:
                        done ? const Color(0xFFFFFFFF) : AppColors.text,
                    side:
                        BorderSide(color: done ? AppColors.nu : AppColors.line),
                  ),
                ...
                const Expanded(
                  child: Row(
                      ...
                      children: [
                        FormaIcon(Icons.check_circle_rounded,
                            color: AppColors.nu, size: 18),
                        SizedBox(width: 6),
                        Text('تمت الوجبة',
                            style: TextStyle(
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text(
          ...
          style:
              TextStyle(color: AppColors.muted, fontSize: 12.5, height: 1.55),
        ),
      ]),
    );
  }
  ...
  Widget _foodRow(NutritionMeal meal, int index, NutritionFoodItem item) {
    ...
    return Padding(
      ...
      child: Row(children: [
        if (!addon)
          FoodPhoto(name: item.name, size: 38)
        else
          const FormaIcon(Icons.add_rounded, size: 12, color: AppColors.muted),
        ...
        if (index >= 0)
          SizedBox(
            ...
            child: PopupMenuButton<String>(
              ...
              icon: const FormaIcon(Icons.more_vert,
              ...
              itemBuilder: (_) => [
                if (!addon)
                  const PopupMenuItem(
                      value: 'replace', child: Text('استبدال الصنف')),
                const PopupMenuItem(
                    value: 'grams', child: Text('تعديل الكمية')),
          ...
          SizedBox(
            ...
            child: PopupMenuButton<String>(
              ...
              itemBuilder: (_) => const [
                PopupMenuItem(
                    value: 'saladGrams', child: Text('تعديل كمية السلطة')),
                PopupMenuItem(
                    value: 'saladAdd', child: Text('إضافة عنصر للسلطة')),
                PopupMenuItem(
                    value: 'saladDropOne', child: Text('حذف مكون من السلطة')),
                PopupMenuItem(
                    value: 'saladRemove',
                    child: Text('حذف طبق السلطة بالكامل')),
              ],
            ),
          )
        else
  ...
  Widget _lockedCard(int remaining) => Container(
        ...
        decoration: FormaDecoration(
          ...
          gradient: LinearGradient(
              colors: [AppColors.wo.withValues(alpha: .10), AppColors.card]),
        ),
        child: Column(children: [
          ...
          SizedBox(
            ...
            child: FilledButton(
              ...
              child: const Text('افتح الخطة الكاملة',
                  style: TextStyle(fontWeight: FontWeight.w900)),
            ),
          ),
        ]),
...
class _SaladPickerSheetState extends State<_SaladPickerSheet> {
  ...
  Future<void> _searchFromDb() async {
    final picked = await Navigator.of(context).push<NutritionFoodItem>(
      MaterialPageRoute(
        builder: (_) =>
            FoodPickerScreen(diet: widget.diet, health: widget.health),
      ),
    );
            ...
            SizedBox(
              ...
              child: FilledButton(
                ...
                onPressed: _totalPicked == 0
                    ? null
                    : () =>
                        Navigator.pop(context, [..._selected, ..._customFoods]),
                child: Text(
                  _totalPicked == 0
