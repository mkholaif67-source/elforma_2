...
class _ChangePasswordScreenState extends State<ChangePasswordScreen> {
  ...
  Future<void> _submit() async {
    if (_new1.text.length < 8) {
      setState(() => _error = 'كلمة المرور الجديدة لازم تكون 8 أحرف على الأقل');
      return;
    }
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: IconButton(
          icon: FormaIcon(hide ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: AppColors.muted),
          onPressed: toggle,
        ),
      ),
    );
