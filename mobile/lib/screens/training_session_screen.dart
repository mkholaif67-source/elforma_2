...
class _TrainingSessionScreenState extends State<TrainingSessionScreen> {
  ...
  int _sessionSets(WorkoutExercise exercise) {
    final adjusted = (exercise.sets * _readinessFactor('setMultiplier')).ceil().clamp(1, exercise.sets);
    // Never hide sets already logged during this session.
    var count = adjusted;
    for (final key in completed.where((key) => key.startsWith('${exercise.key}:'))) {
  ...
  Future<void> _enterWorkout() async {
    ...
    if (!_readinessLoaded) {
      final response = await _api.getSessionReadiness(sessionId: sessionId);
      if (!mounted) return;
      if (!response.ok) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعذر تحميل الجلسة. جرب تاني')));
        return;
      }
      _readinessLoaded = true;
      if (response.data['readiness'] is Map) readiness = Map<String, dynamic>.from(response.data['readiness']);
    }
    setState(() => phase = _hasCheckIn(readiness) ? 'workout' : 'readiness');
  }

  Future<bool> _submitReadiness(Map<String, dynamic> answers) async {
    final id = sessionId;
    if (id == null) return false;
    final response = await _api.submitSessionReadiness(id, checkIn: answers);
    if (!mounted || !response.ok || response.data['readiness'] is! Map) return false;
    final result = Map<String, dynamic>.from(response.data['readiness']);
    if (!_hasCheckIn(result)) return false;
    setState(() { readiness = result; phase = 'workout'; });
    return true;
  }
  ...
  Future<void> _start() async {
    ...
    if (response.ok && id != null) {
      ...
      if (!mounted) return;
      _readinessLoaded = rr.ok;
      if (rr.ok && rr.data['readiness'] is Map) readiness = Map<String, dynamic>.from(rr.data['readiness']);
    }
    setState(() {
      loading = false;
      if (response.ok && session is Map) {
        ...
        if (_hasCheckIn(readiness)) {
          phase = 'workout';
        } else if (oldSets.isNotEmpty && _readinessLoaded) {
          phase = 'readiness';
        }
  ...
  Future<void> _toggleSet(WorkoutExercise exercise, int setNo) async {
    ...
    if (response.ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.friendlyError('تعذر حفظ المجموعة'))));
    }
  }
      ...
      if (restRemaining <= 1) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          ...
          content: Text('خلصت الراحة جاهز للمجموعة التالية',
              style: TextStyle(color: Color(0xFFFFFFFF), fontWeight: FontWeight.w900)),
        ));
      } else {
  ...
  Future<void> _finish() async {
    ...
    if (response.data['queued'] == true) {
      setState(() => error = 'إنهاء الجلسة محفوظ على الهاتف وفي انتظار الاتصال لتأكيده');
      return;
    }
    if (response.ok) {
      ...
      if (PlanStore.I.completedToday(Api.I.accountId) == null) PlanStore.I.markChanged();
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return const Scaffold(body: Center(child: FormaLoader(color: AppColors.wo)));
    }
    ...
    if (phase == 'readiness') return SessionReadinessCheckin(onSubmit: _submitReadiness);
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: ListView(
          ...
          children: [
            if(readiness!=null)...[_readinessCard(),const SizedBox(height:14)],
            _videoCard(exercise),
            ...
            if (exercise.progression.isNotEmpty && _readinessFactor('loadMultiplier') == 1) ...[
              const SizedBox(height: 14),
              _guidance(Icons.trending_up_rounded, 'التطور', exercise.progression, AppColors.nu),
            ],
            const SizedBox(height: 10),
            FormCoachEntryCard(exercise: exercise, totalSets: _sessionSets(exercise)),
            const SizedBox(height: 12),
            _coachCard(exercise),
            ...
            const SizedBox(height: 10),
            ...List.generate(_sessionSets(exercise), (index) => _setRow(exercise, index + 1)),
          ],
        )),
  ...
  Widget _readinessCard() {
    final r=readiness??const <String,dynamic>{};final d=r['decision'] is Map?Map<String,dynamic>.from(r['decision'] as Map):const <String,dynamic>{};final e=r['explanation'] is Map?Map<String,dynamic>.from(r['explanation'] as Map):const <String,dynamic>{};final low='${d['zone']??''}'=='low';final color=low?const Color(0xFFB34343):AppColors.wo;
    return Container(padding:const EdgeInsets.all(14),decoration:FormaDecoration(color:AppColors.card,borderRadius:BorderRadius.circular(16),border:Border.all(color:color.withValues(alpha:.35))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Icon(Icons.bolt_rounded,color:color,size:19),const SizedBox(width:7),Expanded(child:Text('${e['titleAr']??'جاهزية الجلسة'}',style:const TextStyle(fontWeight:FontWeight.w900)))]),const SizedBox(height:7),Text('${e['reasonAr']??''}',style:const TextStyle(fontSize:12.5,height:1.5)),const SizedBox(height:5),Text('قرار هذه الجلسة فقط: ${d['action']=='normal'?'الحمل المعتاد':d['action']=='keep_rest'?'يوم راحة':'خفّف الحمل'}',style:TextStyle(color:color,fontWeight:FontWeight.w800,fontSize:12))]));
  }
        ...
        child: Stack(alignment: Alignment.center, children: [
          Image.network(
            ...
            errorBuilder: (_, __, ___) => Container(
              height: 180,
              color: AppColors.card,
              child: const FormaIcon(Icons.play_circle_fill, color: AppColors.wo2, size: 62),
            ),
          ),
          ...
          Container(
            width: 62,
            height: 62,
            decoration: FormaDecoration(color: AppColors.wo.withValues(alpha: .94), shape: BoxShape.circle),
            child: const FormaIcon(Icons.play_arrow_rounded, color: Colors.white, size: 39),
          ),
          const Positioned(
            right: 12,
            bottom: 10,
  ...
  Widget _guidance(IconData icon, String label, String value, Color color) => Container(
        ...
        decoration: FormaDecoration(color: color.withValues(alpha: .08), borderRadius: BorderRadius.circular(13),
  ...
  Widget _coachCard(WorkoutExercise exercise) {
    ...
    final weight = originalWeight is num ? originalWeight * _readinessFactor('loadMultiplier') : originalWeight;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          FormaIcon(deload ? Icons.spa_rounded : Icons.psychology_alt_rounded, color: accent, size: 19),
  ...
  Widget _historyCard(WorkoutExercise exercise) {
    ...
    if (sets.isEmpty) {
      return Container(
        padding: const EdgeInsets.all(12),
        decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(13),
            border: Border.all(color: AppColors.line)),
    ...
    return Container(
      ...
      decoration: FormaDecoration(color: AppColors.nu.withValues(alpha: .07), borderRadius: BorderRadius.circular(14),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        IconButton(
          ...
          icon: FormaIcon(done ? Icons.check_circle : Icons.radio_button_unchecked,
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                if (id.isNotEmpty)
                  const FormaIcon(Icons.play_circle_outline, color: AppColors.wo2, size: 22),
              ]),
            ),
          ),
        );
  ...
  Widget _warmupScreen(List<WorkoutExercise> list) {
    ...
    return _phaseScaffold(
      ...
      secondaryAction: TextButton(onPressed: _enterWorkout, child: const Text('تخطي الإحماء')),
    );
  }
      ...
      List<Widget> extra = const []}) => Scaffold(
        ...
        body: ListView(padding: const EdgeInsets.fromLTRB(18, 12, 18, 30), children: [
          ...
          ...steps.asMap().entries.map((entry) => Container(
            margin: const EdgeInsets.only(bottom: 10),
            padding: const EdgeInsets.all(14),
            decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(15),
                border: Border.all(color: AppColors.line)),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(width: 30, height: 30, alignment: Alignment.center,
                decoration: FormaDecoration(color: color.withValues(alpha: .14), shape: BoxShape.circle),
  ...
  Widget _chip(String text) => Container(
        ...
        decoration: FormaDecoration(color: AppColors.wo.withValues(alpha: .12), borderRadius: BorderRadius.circular(9)),
        child: Text(text,
