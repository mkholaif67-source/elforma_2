...
import 'package:elforma/screens/exercise_picker_screen.dart';
import 'package:elforma/features/form_coach/ui/form_coach_entry_card.dart';

class TrainingSessionScreen extends StatefulWidget {
  final Map<String, dynamic> day;
  final Api? api;
  const TrainingSessionScreen({super.key, required this.day, this.api});
...
class _TrainingSessionScreenState extends State<TrainingSessionScreen> {
  ...
  bool _hasCheckIn(Map<String, dynamic>? value) =>
      value?['source'] is Map && (value!['source'] as Map)['checkIn'] is Map;

  double _readinessFactor(String field) {
    final decision = readiness?['decision'];
    final value = decision is Map ? decision[field] : null;
    return value is num ? value.toDouble().clamp(0.0, 1.0) : 1;
  }

  int _sessionSets(WorkoutExercise exercise) {
    final adjusted = (exercise.sets * _readinessFactor('setMultiplier'))
        .ceil()
        .clamp(1, exercise.sets);
    // Never hide sets already logged during this session.
    var count = adjusted;
    for (final key
        in completed.where((key) => key.startsWith('${exercise.key}:'))) {
      final number = int.tryParse(key.split(':').last) ?? 0;
      if (number > count) count = number;
    }
    return count;
  }

  Future<void> _enterWorkout() async {
    await _startFuture;
    if (!mounted || sessionId == null) return;
    // Readiness is no longer a prerequisite. Never fabricate check-in answers.
    setState(() => phase = 'workout');
  }

  List<WorkoutExercise> _initialExercises() {
    final raw = widget.day['exercises'] is List
        ? widget.day['exercises'] as List
        : const [];
  ...
  void initState() {
    ...
    sessionTicker = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted)
        setState(() => elapsed = DateTime.now().difference(started).inSeconds);
    });
    _loadRestPref();
    _startFuture = _start();
  }
  ...
  Future<void> _loadRestPref() async {
    ...
    if (mounted)
      setState(() =>
          _restBackground = sp.getBool('rest_background_enabled') ?? true);
  }

  Future<void> _saveRestPref(bool v) async {
    final sp = await SharedPreferences.getInstance();
    await sp.setBool('rest_background_enabled', v);
    if (mounted) setState(() => _restBackground = v);
    if (!v) {
      await NotificationService.I.cancelRestProgress();
    } else if (restRemaining > 0) {
      await NotificationService.I.showRestProgress(
          remaining: restRemaining, total: restTotal, enabled: true);
    }
  }
  ...
  Future<void> _start() async {
    final key =
        (widget.day['key'] ?? widget.day['name'] ?? 'workout').toString();
    final name = (widget.day['name'] ?? 'جلسة تمرين').toString();
    final response = await _api.startWorkoutSession(key, name);
    if (!mounted) return;
    final session = response.data['session'];
    final id = session is Map ? (session['id'] as num?)?.toInt() : null;
    if (response.ok && id != null) {
      final rr = await _api.getSessionReadiness(sessionId: id);
      if (!mounted) return;
      if (rr.ok && rr.data['readiness'] is Map)
        readiness = Map<String, dynamic>.from(rr.data['readiness']);
    }
    setState(() {
      loading = false;
      if (response.ok && session is Map) {
        ...
        final oldSets =
            session['sets'] is List ? session['sets'] as List : const [];
        if (_hasCheckIn(readiness) || oldSets.isNotEmpty) {
          phase = 'workout';
        }
        for (final raw in oldSets.whereType<Map>()) {
          ...
          if (raw['completed'] == 1 || raw['completed'] == true)
            completed.add(key);
        }
      } else {
        error = response.friendlyError('تعذر بدء الجلسة');
      }
        ...
        for (var i = 0; i < sessionExercises.length; i++) {
          ...
          for (final raw in serverExercises.whereType<Map>()) {
            ...
            if (id.isNotEmpty && id != ex.videoId) {
              sessionExercises[i] =
                  ex.replaceWith(Map<String, dynamic>.from(raw));
            }
            break;
          }
        }
    ...
    if (response.ok && exercises.isNotEmpty) {
      unawaited(_loadHistory(exercises.first));
    }
  }
  ...
  Future<void> _loadHistory(WorkoutExercise exercise) async {
    if (history.containsKey(exercise.key) ||
        historyLoading.contains(exercise.key)) return;
    setState(() => historyLoading.add(exercise.key));
    final response = await _api.exerciseHistory(exercise.key);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _toggleSet(WorkoutExercise exercise, int setNo) async {
    ...
    if (response.ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(response.friendlyError('تعذر حفظ المجموعة'))));
    }
  }
  ...
  void _startRest(int seconds) {
    ...
    NotificationService.I.showRestProgress(
        remaining: restRemaining, total: restTotal, enabled: _restBackground);
    _runRestTimer();
  }
  ...
  Future<void> _replaceExercise(WorkoutExercise exercise) async {
    final selected = await Navigator.of(context).push<Map<String, dynamic>>(
      MaterialPageRoute(
          builder: (_) => ExercisePickerScreen(currentName: exercise.name)),
    );
  ...
  Future<void> _finish() async {
    ...
    if (response.data['queued'] == true) {
      setState(() =>
          error = 'إنهاء الجلسة محفوظ على الهاتف وفي انتظار الاتصال لتأكيده');
      return;
    }
    if (response.ok) {
      ...
      if (PlanStore.I.completedToday(Api.I.accountId) == null)
      ...
      await showDialog<void>(
        ...
        builder: (_) => AlertDialog(
          ...
          content:
              Text('الجلسة اتحفظت بنجاح. أنجزت ${completed.length} مجموعة'),
          actions: [
            TextButton(
      ...
      setState(() => error = response.friendlyError('تعذر إنهاء الجلسة'));
    }
  }
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return const Scaffold(
          body: Center(child: FormaLoader(color: AppColors.wo)));
    }
    ...
    if (allExercises.isEmpty) {
      return Scaffold(
          appBar: AppBar(),
          body: const Center(child: Text('مفيش تمارين في اليوم ده')));
    }
        ...
        Expanded(
          ...
          children: [
            ...
            Wrap(
                ...
                runSpacing: 7,
                children: [
                  _chip('${_sessionSets(exercise)} مجموعات'),
            ...
            if (exercise.progression.isNotEmpty &&
                _readinessFactor('loadMultiplier') == 1) ...[
              const SizedBox(height: 14),
              _guidance(Icons.trending_up_rounded, 'التطور',
                  exercise.progression, AppColors.nu),
            ],
            const SizedBox(height: 10),
            FormCoachEntryCard(
                exercise: exercise, totalSets: _sessionSets(exercise)),
            const SizedBox(height: 12),
            _coachCard(exercise),
            _historyCard(exercise),
            if (exercise.alternative.isNotEmpty) ...[
              const SizedBox(height: 9),
              _guidance(Icons.swap_horiz_rounded, 'البديل',
                  exercise.alternative, AppColors.wo2),
            ],
            const SizedBox(height: 9),
            OutlinedButton(
            ...
            const SizedBox(height: 10),
            ...List.generate(_sessionSets(exercise),
                (index) => _setRow(exercise, index + 1)),
          ],
        )),
  ...
  Widget _readinessCard() {
    final r = readiness ?? const <String, dynamic>{};
    final d = r['decision'] is Map
        ? Map<String, dynamic>.from(r['decision'] as Map)
        : const <String, dynamic>{};
    final e = r['explanation'] is Map
        ? Map<String, dynamic>.from(r['explanation'] as Map)
        : const <String, dynamic>{};
    final low = '${d['zone'] ?? ''}' == 'low';
    final color = low ? const Color(0xFFB34343) : AppColors.wo;
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(Icons.bolt_rounded, color: color, size: 19),
            const SizedBox(width: 7),
            Expanded(
                child: Text('${e['titleAr'] ?? 'جاهزية الجلسة'}',
          ...
          Text(
              'قرار هذه الجلسة فقط: ${d['action'] == 'normal' ? 'الحمل المعتاد' : d['action'] == 'keep_rest' ? 'يوم راحة' : 'خفّف الحمل'}',
              style: TextStyle(
    ...
    if (exercise.videoId.isEmpty) {
      return Container(
        ...
        child: const Column(mainAxisSize: MainAxisSize.min, children: [
          FormaIcon(Icons.fitness_center, color: AppColors.wo2, size: 48),
          SizedBox(height: 8),
          Text('لا يوجد فيديو لهذا التمرين',
        ...
        child: Stack(alignment: Alignment.center, children: [
          Image.network(
            ...
            errorBuilder: (_, __, ___) => Container(
              height: 180,
              color: AppColors.card,
              child: const FormaIcon(Icons.play_circle_fill,
                  color: AppColors.wo2, size: 62),
            ),
          ),
          ...
          Container(
            ...
            height: 62,
            decoration: FormaDecoration(
                color: AppColors.wo.withValues(alpha: .94),
                shape: BoxShape.circle),
            child: const FormaIcon(Icons.play_arrow_rounded,
  ...
  Future<void> _reportBrokenVideo(WorkoutExercise exercise) async {
    ...
    final wa =
        (r.data['whatsapp'] ?? '').toString().replaceAll(RegExp(r'[^0-9]'), '');
  ...
  Widget _guidance(IconData icon, String label, String value, Color color) =>
      Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          FormaIcon(icon, color: color, size: 20),
  ...
  Widget _coachCard(WorkoutExercise exercise) {
    ...
    final originalWeight = coach['suggestedWeight'];
    final weight = originalWeight is num
        ? originalWeight * _readinessFactor('loadMultiplier')
        : originalWeight;
    ...
    final trend = coach['trend'] is Map
        ? (coach['trend'] as Map).cast<String, dynamic>()
        : null;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          FormaIcon(deload ? Icons.spa_rounded : Icons.psychology_alt_rounded,
              color: accent, size: 19),
          const SizedBox(width: 7),
          Text(deload ? 'مدربك الذكي · أسبوع تخفيف' : 'مدربك الذكي',
              style: TextStyle(
                  color: accent, fontWeight: FontWeight.w900, fontSize: 13.5)),
        ]),
        const SizedBox(height: 10),
        if (hasBase) ...[
          ...
          Text(
            _readinessFactor('loadMultiplier') < 1
                ? 'الحمل ده أخف للجلسة دي فقط. خليك في مجهود مريح'
                : deload
          ...
          Text(
              '${coach['text'] ?? 'سجل أول وزن عشان مدربك الذكي يبدأ يطور حملك'}',
              style: const TextStyle(
        ...
        if (hasBase) ...[
          ...
          Wrap(spacing: 7, runSpacing: 7, children: [
            if (bestE1rm != null)
              _coachStat(
                  'أقوى 1RM تقديري', '${fmt(bestE1rm)} كجم', AppColors.nu),
            if (prs > 0) _coachStat('أرقام قياسية', '$prs', AppColors.nu2),
            if (progressions > 0)
              _coachStat('مرات تطورت', '$progressions', accent),
            if ((coach['stall'] as num?) != null &&
                (coach['stall'] as num).toInt() > 0)
              _coachStat('جلسات ثبات', '${coach['stall']}', AppColors.wo),
          ]),
        ],
        if (trend != null &&
            (trend['pct'] as num?) != null &&
            (trend['pct'] as num) != 0) ...[
          const SizedBox(height: 10),
          Row(children: [
            FormaIcon(
            ...
            Expanded(
              child: Text(
                ...
                style: const TextStyle(
                    color: AppColors.muted, fontSize: 11.5, height: 1.45),
              ),
            ),
          ]),
        ],
  ...
  Widget _historyCard(WorkoutExercise exercise) {
    if (_readinessFactor('loadMultiplier') < 1) return const SizedBox.shrink();
    if (historyLoading.contains(exercise.key)) {
      return const Center(
        ...
        child: SizedBox(
            width: 18,
            height: 18,
            child: FormaLoader(strokeWidth: 2, color: AppColors.nu)),
      ));
    }
    final data = history[exercise.key];
    ...
    if (sets.isEmpty) {
      return Container(
        ...
        child: const Row(children: [
          FormaIcon(Icons.auto_awesome_outlined, color: AppColors.nu, size: 19),
          SizedBox(width: 9),
          Expanded(
              child: Text('أول مرة لهذا التمرين سجل أداءك لنقترح الحمل القادم',
                  style: TextStyle(color: AppColors.muted, fontSize: 12))),
        ]),
      );
    }
    final completedSets =
        sets.whereType<Map>().where((set) => set['completed'] == true).toList();
    final source = completedSets.isNotEmpty
        ? completedSets
        : sets.whereType<Map>().toList();
    final best = source.reduce((a, b) {
      final av =
          (a['weight'] as num? ?? 0) * (1 + (a['reps'] as num? ?? 0) / 30);
      final bv =
          (b['weight'] as num? ?? 0) * (1 + (b['reps'] as num? ?? 0) / 30);
      return bv > av ? b : a;
    });
    ...
    final canIncrease =
        weight > 0 && repsDone >= high && (rirDone == null || rirDone >= 1.5);
    final suggestion =
        canIncrease ? ((weight * 1.025) * 2).round() / 2 : weight;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          FormaIcon(Icons.history_rounded, color: AppColors.nu, size: 19),
          SizedBox(width: 7),
          Text('أداؤك السابق', style: TextStyle(fontWeight: FontWeight.w900)),
        ]),
        const SizedBox(height: 7),
        Text(
            '${weight.toStringAsFixed(1)} كجم × $repsDone تكرار${rirDone == null ? '' : ' · RIR ${rirDone.toStringAsFixed(1)}'}',
            style: const TextStyle(
                color: AppColors.text, fontWeight: FontWeight.w800)),
        const SizedBox(height: 5),
        Text(message,
            style: const TextStyle(
                color: AppColors.nu, fontSize: 12, height: 1.45)),
      ]),
    );
  }
  ...
  Widget _restPanel() {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          const FormaIcon(Icons.timer_outlined, color: AppColors.nu),
          const SizedBox(width: 8),
          const Expanded(
              child: Text('وقت الراحة',
        ...
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          ...
          TextButton(
              onPressed: () => _adjustRest(15), child: const Text('+15 ثانية')),
          TextButton(
              onPressed: () => _adjustRest(-restRemaining),
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          decoration: const InputDecoration(
              labelText: 'الوزن', suffixText: 'كجم', isDense: true),
        )),
        const SizedBox(width: 8),
        Expanded(
            child: TextField(
          ...
          textAlign: TextAlign.center,
          decoration:
              const InputDecoration(labelText: 'التكرار', isDense: true),
        )),
        const SizedBox(width: 6),
        SizedBox(
            width: 58,
        ...
        IconButton(
          tooltip: done
              ? 'إلغاء تأكيد المجموعة $setNo'
              : 'تأكيد إتمام المجموعة $setNo',
        ...
        child: Row(children: [
          if (current > 0) ...[
            IconButton.filledTonal(
                ...
                icon: const FormaIcon(Icons.arrow_forward_rounded)),
            const SizedBox(width: 8),
          ],
          Expanded(
              child: SizedBox(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                  ...
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(14))),
              child: finishing
                  ? const FormaLoader(color: Colors.white, strokeWidth: 2)
                  : Text(
                      current < count - 1
                          ? 'التمرين التالي'
                          : 'الإطالة ثم الإنهاء',
  ...
  Future<void> _openVideoId(String id) async {
    ...
    if (!opened)
      opened = await _launchVideoAttempt(uri, LaunchMode.externalApplication);
    if (!opened && mounted) {
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                if (id.isNotEmpty)
                  const FormaIcon(Icons.play_circle_outline,
  ...
  Widget _warmupScreen(List<WorkoutExercise> list) {
    final muscles =
        list.map((e) => e.muscle).where((e) => e.isNotEmpty).take(3).join('، ');
    final steps = <MapEntry<String, String>>[
      const MapEntry(
          'رفع حرارة الجسم', '3 5 دقائق مشي سريع أو دراجة بمجهود خفيف'),
      const MapEntry(
          'حركة المفاصل', 'دوائر للكتف والحوض والكاحل × 8 لكل اتجاه'),
      MapEntry(
          'تنشيط عضلات اليوم',
          muscles.isEmpty
              ? 'حركات خفيفة للعضلات المستهدفة'
              : 'حركات خفيفة ل $muscles'),
      const MapEntry('مجموعات تمهيدية',
          'نفذ مجموعتين من أول تمرين: 50% ثم 70% من حمل العمل بدون إجهاد'),
    ];
    return _phaseScaffold(
      ...
      button: 'خلصت الإحماء',
      onPressed: _enterWorkout,
      secondaryAction: TextButton(
          onPressed: _enterWorkout, child: const Text('تخطي الإحماء')),
    );
  }
  ...
  List<Widget> _stretchCards() {
    ...
    return [
      Padding(
        ...
        child: Row(children: [
          const FormaIcon(Icons.spa_rounded, color: AppColors.nu, size: 18),
          const SizedBox(width: 7),
          const Expanded(
          ...
          children: [
            for (var i = 0; i < shown.length; i++) ...[
              ...
              Padding(
                ...
                child: Row(children: [
                  const FormaIcon(Icons.check_circle_outline_rounded,
  ...
  Widget _cooldownScreen() => _phaseScaffold(
        ...
        steps: const [
          ...
          MapEntry('إطالة خفيفة',
              'إطالة عضلات اليوم 20 30 ثانية بدون ألم أو ارتداد'),
  ...
  Widget _phaseScaffold(
          {required String title,
          required IconData icon,
          required Color color,
          required String intro,
          required List<MapEntry<String, String>> steps,
      ...
      Scaffold(
        ...
        appBar: AppBar(
            backgroundColor: AppColors.bg,
            title: Text(
                WorkoutExercise.splitDayName(
                    widget.day['name']?.toString() ?? '')[0],
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontWeight: FontWeight.w900))),
        body: ListView(
            ...
            children: [
              Container(
                ...
                child: Column(children: [
                  FormaIcon(icon, color: Colors.white, size: 46),
                  const SizedBox(height: 10),
                  Text(title,
                      style: const TextStyle(
                          color: Colors.white,
