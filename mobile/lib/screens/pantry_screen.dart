...
class _PantryScreenState extends State<PantryScreen> {
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: const FormaIcon(Icons.search, color: AppColors.muted, size: 20),
  ...
  Widget _list() {
    if (loading) {
      return const Center(
        child: FormaLoader(color: AppColors.nu, strokeWidth: 2),
      );
    }
    if (foods.isEmpty) {
      return Center(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final cal = food['cal'];
        final pro = food['pro'];
        final foodName = '${food['nameAr'] ?? food['nameEn'] ?? ''}';
        ...
        return Padding(
          ...
          child: Semantics(
            button: true,
            selected: active,
            label: '$foodName، $cal سعرة، بروتين $pro جرام لكل 100 جرام',
            child: Material(
              color: Colors.transparent,
              borderRadius: radius,
              child: InkWell(
                ...
                child: Ink(
                  ...
                  decoration: FormaDecoration(
                    color: active
                        ? AppColors.nu.withValues(alpha: .10)
                        : AppColors.card,
                    borderRadius: radius,
                    border: Border.all(
                      color: active
                          ? AppColors.nu.withValues(alpha: .55)
                          : AppColors.line,
                    ),
                  ),
                  child: Row(
                    children: [
                      ...
                      ExcludeSemantics(
                        child: FoodPhoto(name: foodName, size: 44),
                      ),
