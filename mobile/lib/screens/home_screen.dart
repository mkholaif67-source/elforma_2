...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  List get sessions => data['recentSessions'] is List
      ? data['recentSessions'] as List
      : const [];
  List get weights =>
      data['weights'] is List ? data['weights'] as List : const [];
  ...
  void _onSubChanged() {
    if (mounted) _load();
  } // shared bootstrap, no forced duplicate
  ...
  Future<void> _profileSetup() async {
    final changed = await Navigator.of(context).push<bool>(MaterialPageRoute(
      builder: (_) => ProfileStore.I.isComplete
          ...
          : ProfileSetupScreen(initial: profile),
    ));
  ...
  Map<String, dynamic>? _activeWorkoutDay() {
    ...
    for (final day in candidates) {
      ...
      if (key == wanted ||
          day['name']?.toString() == session['day_name']?.toString()) {
        return day;
      }
    }
    return null;
  ...
  List<Widget> _announcementCards() {
    final list = AnnouncementStore.I.loaded
        ? AnnouncementStore.I.items
    ...
    final rows = list
        ...
        .where((a) =>
            (a['mode'] ?? 'card').toString() == 'card' ||
            (a['mode'] ?? '').toString() == 'both')
        .toList();
    ...
    final cards = <Widget>[
      ...
      Padding(
        ...
        child: Row(children: [
          Expanded(
              child: Divider(
                  color: AppColors.line.withValues(alpha: .5), thickness: 1)),
          Padding(
        ...
        child: Container(
          ...
          decoration: FormaDecoration(
            gradient: LinearGradient(
                colors: [AppColors.nu.withValues(alpha: .18), AppColors.card]),
            borderRadius: BorderRadius.circular(19),
            border: Border.all(color: AppColors.nu.withValues(alpha: .35)),
          ),
          child: const Row(children: [
            FormaIcon(Icons.auto_awesome_rounded,
                color: AppColors.nu, size: 29),
            SizedBox(width: 12),
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text('جهز خطتك الشخصية',
                      style:
  ...
  Widget _trialBanner() {
    ...
    if (sub.currentPeriodEnd != null) {
      try {
        ...
        if (rem > 0)
          label = 'تجربتك المجانية تنتهي خلال $rem يوم';
        else
          label = 'تجربتك انتهت!';
      } catch (_) {}
    }
    return InkWell(
      ...
      child: Container(
        ...
        decoration: FormaDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border:
              Border.all(color: const Color(0xFF267346).withValues(alpha: .35)),
        ),
        child: Row(children: [
          ...
          child: Row(children: [
            ...
            _giftBusy
                ? const SizedBox(
                    width: 22,
                    height: 22,
                    child: FormaLoader(strokeWidth: 2.4, color: AppColors.nu))
                : Container(
                    padding:
                    ...
                    decoration: FormaDecoration(
                      ...
                      border: Border.all(
                          color: AppColors.nu.withValues(alpha: .35)),
                    ),
  ...
  Widget _activeWorkout() {
    ...
    final done = (session['sets'] is List)
        ? (session['sets'] as List)
            .where((s) =>
                s is Map && (s['completed'] == 1 || s['completed'] == true))
            .length
        : 0;
    return _actionCard(
      color: AppColors.wo,
      ...
      subtitle: done > 0
          ? 'أنجزت $done مجموعات كمل من مكانك'
          : 'الجلسة محفوظة ومستعدة للاستكال',
    ...
    try {
      ...
      if (!finished.ok || finished.data['queued'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(finished.data['queued'] == true
                ? 'إنهاء الجلسة محفوظ على الهاتف وفي انتظار تأكيد الاتصال'
                : finished.friendlyError('تعذر إنهاء الجلسة حاول تاني'))));
        return;
      }
      final now = DateTime.now();
      _completedLocalDay =
      ...
      setState(() {});
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم تسجيل تمرين النهارده ')));
      await _load(force: true);
    } finally {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text(
          ...
          style:
              TextStyle(color: Color(0xFFD7E9DE), fontSize: 12.5, height: 1.55),
        ),
        const SizedBox(height: 14),
        SizedBox(
  ...
  List<Map<String, dynamic>> _planDays() {
    ...
    if (planData['_locked'] == true && planData['previewDay'] is Map)
      return [Map<String, dynamic>.from(planData['previewDay'] as Map)];
  ...
  Map<String, dynamic>? _todayPlanDay() {
    ...
    if (createdAtStr != null && createdAtStr.isNotEmpty) {
      try {
        ...
        final activationMidnight = DateTime(
            activationDate.year, activationDate.month, activationDate.day);
        final daysSince =
            todayMidnight.difference(activationMidnight).inDays.clamp(0, 10000);
  ...
  bool _isRestDay(Map<String, dynamic> day) {
    ...
    final name = '${day['name'] ?? ''}';
    if (name.contains('راحة') || name.toLowerCase().contains('rest'))
      return true;
  ...
  Widget _nextWorkout() {
    ...
    if (day == null) {
      return _actionCard(
          ...
          eyebrow: 'تمرين اليوم',
          title: 'تعذر عرض حصة اليوم',
          subtitle: 'افتح جدولك للمحاولة مرة أخرى',
          button: 'افتح الجدول',
          onTap: () => widget.onGo(1));
    }
    if (_isRestDay(day)) {
      return _actionCard(
        ...
        subtitle: 'الراحة بتبني قوتك: حركة خفيفة، مية كفاية، ونوم هادئ',
  ...
  Future<void> _quickComplete(Map<String, dynamic> day) async {
    ...
    if (_isTodayCompleted())
      return; // ضغطة تانية بعد الاكتمال = مفيش تأثير زيادة
    setState(() => _quickBusy = true);
    try {
      final dayKey =
      ...
      if (!start.ok) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('حصل خطأ، جرب تاني')));
        return;
      }
      ...
      if (start.data['alreadyCompleted'] == true) {
        ...
        setState(() {});
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تمرين النهاردة متسجل خلاص ')));
        await _load();
        return;
      }
  ...
  Widget _coachCard() {
    ...
    final adherence = _weekTarget == 0
        ? 0
        : ((_weekDone / _weekTarget) * 100).clamp(0, 100).round();
      ...
      ClipRRect(
        ...
        child: Stack(
          children: [
            Positioned.fill(
              child: Image.asset(
                'assets/workout/workout_plan_hero.webp',
                fit: BoxFit.cover,
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            FormaIcon(icon, color: Colors.white, size: 15),
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            style: OutlinedButton.styleFrom(
                              ...
                              backgroundColor: const Color(0x66071E16),
                              side: BorderSide(
  ...
  Widget _nutritionCard() {
    ...
    final meals =
        nutrition?['meals'] is List ? nutrition!['meals'] as List : const [];
  ...
  Widget _waterCard() {
    ...
    return HomeMetricCard(
      ...
      progress: (water / 2500).clamp(0, 1).toDouble(),
      onTap: () => widget.onGo(2),
    );
  }
  ...
  Widget _weeklyProgress() {
    ...
    final target = profile?['trainingDays'] is num
        ? (profile!['trainingDays'] as num).toInt()
        : 3;
    return HomeWeeklyProgress(count: count, target: target);
  }

  Widget _weightCard() {
    final latest = weights.isNotEmpty && weights.first is Map
