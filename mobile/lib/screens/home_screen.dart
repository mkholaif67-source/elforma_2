import 'package:elforma/widgets/community_home.dart';
...
import 'package:elforma/widgets/offline_banner.dart';
...
import 'package:elforma/screens/progress_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  Timer? _dayTimer;
  DateTime _visibleDay = DateUtils.dateOnly(DateTime.now());

  void _scheduleDayRefresh() {
    _dayTimer?.cancel();
    final now = DateTime.now();
    final next = DateTime(now.year, now.month, now.day + 1);
    _dayTimer = Timer(next.difference(now) + const Duration(seconds: 1), () {
      if (!mounted) return;
      _visibleDay = DateUtils.dateOnly(DateTime.now());
      _load(force: true);
      _scheduleDayRefresh();
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (today != _visibleDay) { _visibleDay = today; _load(force: true); }
    _scheduleDayRefresh();
  }
  bool loading = true;
  String? error;
  ...
  void initState() {
    ...
    AnnouncementStore.I.addListener(_onCoachToggle);
    Api.I.bootstrapRevision.addListener(_onBootstrapFresh); // [INSTANT-OPEN]
    _load();
  }
  ...
  // [INSTANT-OPEN] وصلت نسخة طازة بعد عرض المحفوظة — التحميل ده من الكاش فوري.
  void _onBootstrapFresh() { if (mounted) _load(); }

  void _onDataChanged() {
    // Called when ProfileStore or PlanStore notifies (workout/meal plan changed)
    if (mounted) {
      setState(() => data = PlanStore.I.mergeCompletion(data, Api.I.accountId));
      _load(force: true);
    }
  }
  ...
  void dispose() {
    ...
    AnnouncementStore.I.removeListener(_onCoachToggle);
  ...
  Future<void> _load({bool force = false}) async {
    ...
    setState(() {
      loading = false;
      if (response.ok) {
        data = PlanStore.I.mergeCompletion(response.data, Api.I.accountId);
        error = null;
      } else {
        // أول تحميل فشل: اعرض رسالة الخطأ
        error = response.friendlyError('تعذر تحميل يومك');
      }
  ...
  Future<void> _profileSetup() async {
    final changed = await Navigator.of(context).push<bool>(MaterialPageRoute(
      builder: (_) => ProfileStore.I.isComplete ? ProfileOverviewScreen(initial: ProfileStore.I.profile!) : ProfileSetupScreen(initial: profile),
    ));
          ...
          child: loading
              ...
              : ListView(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 14),
                    _weightCard(),
                    CommunityHome(key: ValueKey(Api.I.accountId)),
                  ],
                ),
        ),
      ),
  ...
  List<Widget> _announcementCards() {
    final list = AnnouncementStore.I.loaded ? AnnouncementStore.I.items : (data['announcements'] is List ? data['announcements'] as List : const []);
    ...
    cards.addAll(rows.map((a) => Padding(
      ...
      child: AnnouncementCard(key: ValueKey(a['id']), data: a),
    )));
    return cards;
  }

  String _displayName(String value) {
    final first = value.trim().split(RegExp(r'\s+')).firstWhere((v) => v.isNotEmpty, orElse: () => '');
    if (first.isEmpty || !RegExp(r'^[A-Za-z]').hasMatch(first)) return first;
    return first[0].toUpperCase() + first.substring(1);
  }

  String _planDisplayName(String key) {
  ...
  Widget _header() {
    ...
    return Row(children: [
      Padding(padding: const EdgeInsetsDirectional.only(end: 12), child: Image.asset('assets/auth/brand_logo.png', width: 66, height: 60, fit: BoxFit.contain, cacheWidth: 198, cacheHeight: 180)),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          name.isEmpty
              ? _greeting()
              : '${_greeting()} يا ${_displayName(name)}',
          style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -0.3),
        ),
        const SizedBox(height: 6),
        const SizedBox(height: 2),
        Row(children: [
          const FormaIcon(Icons.bolt_rounded, size: 13, color: AppColors.nu2),
  ...
  String _greeting() {
    final now = DateTime.now();
    final variants = now.hour < 12
        ? ['صباح الخير', 'صباح النور', 'صباح التميز', 'صباح العزيمة', 'صباح الإنجاز', 'صباح الطاقة']
        : ['مساء الطاقة', 'مساء الأمل', 'مساء التفاؤل', 'مساء النشاط', 'مساء الحيوية', 'مساء القوة'];
    return variants[now.day % variants.length];
  }

  String _todayMessage() {
    if (error != null) return 'حاول تحديث يومك';
    if (activeSession != null) return 'كمل تمرينك من مكان ما وقفت';
    if (!ProfileStore.I.isComplete) return 'كمل بياناتك ونجهز خطتك';
    if (_isTodayCompleted()) return 'خلصت تمرينك اهتم بأكلك وراحتك';
    if (!ProfileStore.I.trains) return 'تابع وجباتك وخد خطوة لهدفك';
    if (!_hasPlan) return 'افتح قسم التمرين لتجهيز جدولك';
    if (_pendingWorkoutStart()) return 'خطتك جاهزة لأول حصة';
    final day = _todayPlanDay();
    if (day != null && _isRestDay(day)) return 'يوم راحة اهتم بنومك وتغذيتك';
    return 'تمرينك ووجباتك جاهزين ليومك';
  }
  ...
  Widget _profileBanner() => InkWell(
        ...
        child: Container(
          ...
          child: const Row(children: [
            FormaIcon(Icons.auto_awesome_rounded, color: AppColors.nu, size: 29),
            SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            FormaIcon(Icons.chevron_left_rounded, color: AppColors.nu),
          ]),
        ),
      );
  ...
  Widget _trialBanner() {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: FormaDecoration(
          color: const Color(0xFF267346).withValues(alpha: .10),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: const Color(0xFF267346).withValues(alpha: .35)),
        ),
        child: Row(children: [
          const FormaIcon(Icons.workspace_premium_rounded,
              color: Color(0xFF267346), size: 18),
          const SizedBox(width: 9),
          Expanded(child: Text(label,
              style: const TextStyle(color: Color(0xFF267346),
                  fontWeight: FontWeight.w700, fontSize: 13))),
          const FormaIcon(Icons.chevron_right_rounded,
              color: Color(0xFF267346), size: 18),
        ]),
      ),
    );
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: const FormaIcon(Icons.card_giftcard_rounded,
  ...
  Widget _activeWorkout() {
    ...
    return _actionCard(
      ...
      // على كارت البداية بس؛ لما الجلسة تبقى شغالة الكارت بيتغير فكان يختفي.
  ...
  Future<void> _quickCompleteActive() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (!finished.ok || finished.data['queued'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(finished.data['queued'] == true ? 'إنهاء الجلسة محفوظ على الهاتف وفي انتظار تأكيد الاتصال' : finished.friendlyError('تعذر إنهاء الجلسة حاول تاني'))));
        return;
      }
      final now = DateTime.now();
      _completedLocalDay =
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: const FormaIcon(Icons.check_rounded, color: green, size: 32),
          ),
          const SizedBox(width: 13),
          const Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text(
          'استمرارك النهارده بيقربك من هدفك ريح عضلاتك وكمل أكلك ومياهك وبكرة نكمل أقوى',
          style: TextStyle(
              color: Color(0xFFD7E9DE), fontSize: 12.5, height: 1.55),
        ),
        const SizedBox(height: 14),
        SizedBox(
          ...
          child: OutlinedButton(
            ...
            child: const Text('تابع تطور مستواك'),
          ),
        ),
      ]),
    );
  ...
  List<Map<String, dynamic>> _planDays() {
    ...
    if(planData['_locked']==true&&planData['previewDay'] is Map)return [Map<String,dynamic>.from(planData['previewDay'] as Map)];
  ...
  Map<String, dynamic>? _todayPlanDay() {
    ...
    final workoutPlan = _map(data['workoutPlan']);
    final planData = _map(workoutPlan?['data']);
    final serverIndex = planData?['_scheduleDayIndex'];
    if (serverIndex is num && serverIndex.toInt() >= 0 && serverIndex.toInt() < days.length) {
      return days[serverIndex.toInt()];
    }
  ...
  Widget _nextWorkout() {
    ...
    final day = _todayPlanDay();
    if (day == null) {
      return _actionCard(color: AppColors.wo, icon: Icons.refresh_rounded, eyebrow: 'تمرين اليوم',
        title: 'تعذر عرض حصة اليوم', subtitle: 'افتح جدولك للمحاولة مرة أخرى',
        button: 'افتح الجدول', onTap: () => widget.onGo(1));
    }
    if (_isRestDay(day)) {
      return _actionCard(
    ...
    final planLabel = _planDisplayName(planKey);
    ...
    return _actionCard(
      color: AppColors.wo,
      icon: Icons.fitness_center_rounded,
      eyebrow: planLabel.isEmpty ? 'تمرين النهاردة' : 'جدولك: $planLabel',
      title: _cleanDayName((day['name'] ?? 'تمرين اليوم').toString()),
      subtitle: '$exercises تمارين جاهزة ليك',
  ...
  Widget _coachCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          FormaIcon(Icons.insights_rounded, color: AppColors.nu, size: 19),
          SizedBox(width: 7),
          Text('ملخص النهاردة',
        ...
        Row(children: [
          ...
          Expanded(
              child: _stat(
                  ...
                  delta == null ? AppColors.muted : const Color(0xFF996214))),
        ]),
        const SizedBox(height: 13),
        Container(
  ...
  Widget _actionCard({required Color color, required IconData icon, required String eyebrow,
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ClipRRect(borderRadius: BorderRadius.circular(17), child: Image.asset(
            'assets/auth/hero_signup.webp', width: double.infinity, height: 138,
            // Drawn 138dp tall; decode no bigger than 3x that.
            fit: BoxFit.cover, cacheHeight: 414,
            alignment: const Alignment(0, -.15), excludeFromSemantics: true)),
          const SizedBox(height: 14),
          Row(children: [
            Container(width: 43, height: 43, decoration: FormaDecoration(color: color.withValues(alpha: .16), borderRadius: BorderRadius.circular(13)),
                child: FormaIcon(icon, color: color)),
            const SizedBox(width: 11),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _smallCard({required Color color, required IconData icon, required String title,
      ...
      InkWell(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [FormaIcon(icon, color: color, size: 21), const SizedBox(width: 7), Text(title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 12))]),
            const SizedBox(height: 13),
  ...
  Widget _weeklyProgress() {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          const FormaIcon(Icons.insights_rounded, color: AppColors.wo2),
          const SizedBox(width: 9),
  ...
  Widget _weightCard() {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.line)),
        child: Row(children: [
          const FormaIcon(Icons.monitor_weight_outlined, color: AppColors.nu, size: 26),
          const SizedBox(width: 11),
  ...
  Widget _errorCard() => Container(
        ...
        decoration: FormaDecoration(color: Colors.red.withValues(alpha: .08), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.red.withValues(alpha: .25))),
        child: Row(children: [
          const FormaIcon(Icons.cloud_off_rounded, color: AppColors.wo2),
          const SizedBox(width: 9),
