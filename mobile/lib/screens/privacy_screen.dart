...
class _PrivacyScreenState extends State<PrivacyScreen> {
  ...
  Future<void> _export() async {
    ...
    if (!response.ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(
          response.friendlyError('تعذر تصدير البيانات'))));
      return;
    }
    try {
  ...
  Future<void> _delete() async {
    ...
    if (response.ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(
          response.friendlyError('تعذر حذف الحساب'))));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ? const Center(child: FormaLoader(color: AppColors.nu)) : ListView(
  ...
  Widget _section(String title, String body) => Container(
        ...
        decoration: FormaDecoration(gradient: LinearGradient(colors: [AppColors.nu.withValues(alpha: .15), AppColors.nu.withValues(alpha: .04)]),
            borderRadius: BorderRadius.circular(18), border: Border.all(color: AppColors.nu.withValues(alpha: .25))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [const FormaIcon(Icons.verified_user_outlined, color: AppColors.nu), const SizedBox(width: 8),
  ...
  Widget _switch(IconData icon, String title, String subtitle, bool value, ValueChanged<bool> changed) => Container(
        ...
        child: SwitchListTile(value: value, activeColor: AppColors.nu, onChanged: changed,
          secondary: FormaIcon(icon, color: AppColors.nu), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
  ...
  Widget _action(IconData icon, String title, String subtitle, Color color, VoidCallback? action, bool busy) => Container(
        decoration: FormaDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(15), border: Border.all(color: color.withValues(alpha: .35))),
        child: ListTile(onTap: action, leading: FormaIcon(icon, color: color),
          ...
          trailing: busy ? const SizedBox(width: 20, height: 20, child: FormaLoader(strokeWidth: 2))
