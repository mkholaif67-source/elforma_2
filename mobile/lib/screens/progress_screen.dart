...
import 'package:elforma/widgets/monthly_wrap_up_card.dart';
...
class _ProgressScreenState extends State<ProgressScreen> {
  ...
  Future<void> _load() async {
    ...
    final responses = await Future.wait<ApiResult>([
      ...
      Api.I.progressStory(month: _selectedMonth),
    ]);
    if (!mounted) return;
    ...
    setState(() {
      ...
      if (storyResponse.ok && storyResponse.data['summary'] is Map) {
        ...
        _selectedMonth = '${storyResponse.data['month'] ?? ''}';
      }
    });
  ...
  Widget _content() => ListView(
        ...
        children: [
          if (story.isNotEmpty) ...[
            ...
            const SizedBox(height: 10),
            _loadingStory
                ? const SizedBox(height: 180, child: Center(child: FormaLoader(color: AppColors.nu)))
                : _storyCard(),
  ...
  Future<void> _loadStoryMonth(String month) async {
    if (_loadingStory || month == _selectedMonth) return;
    setState(() => _loadingStory = true);
    final response = await Api.I.progressStory(month: month);
    if (!mounted) return;
    setState(() {
      _loadingStory = false;
      if (response.ok && response.data['summary'] is Map) {
        story = Map<String, dynamic>.from(response.data);
        _selectedMonth = month;
      }
    });
    if (!response.ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(response.friendlyError('تعذر تحميل ملخص الشهر'))));
    }
  }

  Widget _monthPicker() {
    final months = story['availableMonths'] is List
        ? (story['availableMonths'] as List).map((item) => '$item').toList()
        : <String>[];
    ...
    return SizedBox(height: 42, child: ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        return ChoiceChip(
          ...
          onSelected: (_) => _loadStoryMonth(month),
          selectedColor: AppColors.nu.withValues(alpha: .15),
          labelStyle: TextStyle(fontWeight: FontWeight.w800,
              color: month == _selectedMonth ? AppColors.nu : AppColors.muted),
          side: BorderSide(color: month == _selectedMonth ? AppColors.nu : AppColors.line),
        );
      },
    ));
  }
  ...
  Widget _storyCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (story['closed'] == true && story['empty'] != true) ...[
          const SizedBox(height: 12),
          SizedBox(width: double.infinity, child: OutlinedButton.icon(
            onPressed: () => showMonthlyWrapUp(context, story),
            icon: const FormaIcon(Icons.auto_awesome_rounded, size: 18),
            label: const Text('افتح كارد حصاد الشهر', style: TextStyle(fontWeight: FontWeight.w900)),
          )),
        ],
      ]),
    );
