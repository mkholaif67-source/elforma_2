...
import 'package:elforma/widgets/gender_avatar.dart';
...
import 'package:elforma/screens/edit_account_screen.dart';
...
import 'package:elforma/screens/reminder_settings_screen.dart';
import 'package:elforma/screens/faq_screen.dart';
import 'package:elforma/screens/support_screen.dart';
import 'package:elforma/models/smart_coach_store.dart';
import 'package:elforma/widgets/social_bar.dart';
...
class _AccountScreenState extends State<AccountScreen> {
  ...
  void initState() {
    ...
    AnnouncementStore.I.addListener(_adsChanged);
    _load();
  }
  ...
  void dispose() {
    ...
    AnnouncementStore.I.removeListener(_adsChanged);
    super.dispose();
  }
  ...
  Future<void> _load({bool force = false}) async {
    ...
    final res = await Api.I.mobileAccount(force: force);
    final bootstrap = res;
    if (!mounted || generation != _loadGeneration) return;

    // This authenticated snapshot carries only what the account screen needs.
    if (!res.ok) {
      setState(() {
        _loading = false;
    ...
    setState(() {
      ...
      _profile = bootstrap.data['profile'] is Map
          ...
          : null;
      _announcements = (bootstrap.data['announcements'] is List
          ? (bootstrap.data['announcements'] as List)
              .whereType<Map>()
    ...
    if (bootstrap.data.containsKey('smartCoach')) {
      await SmartCoachStore.I
          .syncFromServer(bootstrap.data['smartCoach'] == true);
      if (mounted) setState(() {});
    }
  }
  ...
  Future<void> _logout() async {
    // [FIX-LOGOUT-NAV] بنمسك الـ Navigator قبل أي await: أثناء Api.logout()
    // إشعار accountChanges بينفذ والـ shell بيعيد بناء التبويبات ويدمر هذه
    // الشاشة، فأي Navigator.of(context) أو فحص mounted بعد الـ await كان بيلغي
    // الانتقال بهدوء — وده كان سبب إن الخروج ماكانش بيرجع لشاشة الدخول أبدا.
    // الـ Navigator المحفوظ بيفضل صالح لأنه تابع لجذر التطبيق مش للشاشة دي.
    ...
    try {
      ...
      // حتى لو فشل أي تنظيف محلي، إخراج المستخدم من الواجهة لازم يتم —
      // الجلسة اتمسحت محليا بالفعل وشاشة الدخول هتظهر في كل الأحوال.
    }
  ...
  Widget build(BuildContext context) {
    ...
    final trialing = _sub != null && _sub!['status'] == 'trialing' && pro;
    return Scaffold(
      appBar: AppBar(
        backgroundColor: AppColors.bg,
        title:
            const Text('حسابي', style: TextStyle(fontWeight: FontWeight.w900)),
        centerTitle: true,
      ),
      body: _loading
          ? const LoadingView()
                    ...
                    children: [
                      ...
                      const Padding(
                        ...
                        child: Align(
                            ...
                            child: Text('التقدم والمتابعة',
                                style: TextStyle(
                                    fontSize: 17,
                                    fontWeight: FontWeight.w900))),
                      ),
                      _navTile(Icons.insights_rounded, 'الوزن وقياسات الجسم',
                          'سجل تقدمك وشاهد الاتجاهات', const ProgressScreen()),
                      _navTile(
                          Icons.history_rounded,
                          'سجل التدريب',
                          'الأسابيع والأيام والتمارين المسجلة',
                          const WorkoutHistoryScreen()),
                      _navTile(
                          Icons.query_stats_rounded,
                          'تحليل الأداء',
                          'المدرب الذكي والحمل وأفضل أرقامك',
                          const WorkoutHistoryScreen(
                              mode: WorkoutHistoryMode.analysis)),
                      _navTile(
                      ...
                      _navTile(
                          ...
                          const FaqScreen()),
                      _navTile(Icons.support_agent_rounded, 'الدعم والتواصل',
                          'كلمنا على واتساب أو الإيميل', const SupportScreen()),
                      ...
                      Center(
                        child: TextButton(
                          ...
                          child: const Text(
                              'تسجيل الخروج من كل الأجهزة التانية',
                              style: TextStyle(
  ...
  List<Widget> _announcementCards() {
    final list =
        AnnouncementStore.I.loaded ? AnnouncementStore.I.items : _announcements;
    final rows = list
        .whereType<Map>()
        .map((e) => Map<String, dynamic>.from(e))
        .where((a) {
  ...
  Widget _smartCoachTile() {
    return Container(
      ...
      child: SwitchListTile(
        ...
        secondary:
            const FormaIcon(Icons.auto_awesome_rounded, color: AppColors.nu2),
    ...
    return Center(
      child: Container(
        ...
        decoration: FormaDecoration(
          ...
          color: AppColors.card,
          border:
              Border.all(color: AppColors.nu.withValues(alpha: .45), width: 2),
        ),
        ...
        child: ClipOval(
          child: Image.asset(
            ...
            fit: BoxFit.cover,
            // 118dp box minus 15dp padding = 88dp drawn. Capping the decode at
            // 3x that keeps it crisp on the densest phones without holding the
  ...
  Widget _profileTile() {
    return Container(
      ...
      child: ListTile(
        ...
        leading: const FormaIcon(Icons.tune_rounded, color: AppColors.nu),
        title: const Text('بيانات التدريب والتغذية',
            style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
        subtitle: Text(
          _profile?['onboardingComplete'] == true
              ? 'بياناتك محفوظة اضغط للتعديل'
              : 'أكمل بياناتك لبناء خطة أدق',
          style: const TextStyle(color: AppColors.muted, fontSize: 12),
        ),
        trailing: const FormaIcon(Icons.chevron_left, color: AppColors.muted),
      ),
    );
  }
  ...
  Widget _trialBanner() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const FormaIcon(Icons.hourglass_bottom_rounded, color: AppColors.nu2),
  ...
  Widget _navTile(IconData icon, String title, String subtitle, Widget target) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      decoration: FormaDecoration(
        color: AppColors.card,
      ...
      child: ListTile(
        ...
        leading: FormaIcon(icon, color: AppColors.nu),
        title: Text(title,
  ...
  Widget _row(IconData icon, String label, String value) {
    return Container(
      ...
      child: Row(
        children: [
          FormaIcon(icon, color: AppColors.nu, size: 22),
