...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  /// review summary.
  String _summary() => 'يمكنك مراجعة اختياراتك وحفظ التغييرات';
  ...
  Future<void> _save() async {
    ...
    final profile = <String, dynamic>{
      ...
      if (double.tryParse(targetWeight.text.trim()) != null)
        'targetWeight': double.tryParse(targetWeight.text.trim()) ?? 0,
      'goal': goal,
    ...
    // [FIX-CHAT-EXIT] الحفظ هنا سريع (بروفايل بس، بدون تجهيز الخطط التقيل).
    // توليد خطتي التمرين والتغذية بيحصل في صفحة التحليل اللي المستخدم بينتقل
    // ليها حالا — مش جوه الشات. كده الزر مابيعلقش في التحميل أبدا.
    ...
    if (response.ok) {
      ...
      final saved = response.data['profile'];
      if (saved is Map) await ProfileStore.I.apply(Map<String, dynamic>.from(saved));
      if (!mounted) return;
      if (!(_editMode && !_fromScratch)) {
        Navigator.of(context).pushReplacement(MaterialPageRoute(
          builder: (_) => AnalysisScreen(payload: ProfileStore.I.profile!, profileAlreadySaved: true)));
      } else {
        Navigator.pop(context, true);
      }
    } else {
      // البروفايل اتحفظ بس توليد الخطة فشل (503) — نكمل التدفق للأمام والخطة هتتولد دلوقتي 
      if (response.data['profileSaved'] == true && response.data['profile'] is Map) {
        final saved = response.data['profile'];
        await ProfileStore.I.apply(Map<String,dynamic>.from(saved as Map));
        if (!mounted) return;
        // البيانات اتحفظت تماما — نروح للتحليل زي نجاح والخطة هتاجي دقيقة تتولد
        if (!(_editMode && !_fromScratch)) {
          Navigator.of(context).pushReplacement(MaterialPageRoute(
            builder: (_) => AnalysisScreen(payload: ProfileStore.I.profile!, profileAlreadySaved: true)));
        } else {
      ...
      setState(() {
        error = response.friendlyError('تعذر حفظ بياناتك. جرب مرة أخرى');
        _thread.add(_Msg(true, 'حصلت مشكلة في الحفظ — جرب تاني'));
      });
      _scrollToEnd();
    }
  }
    ...
    return Row(
      ...
      children: [
        ...
        Container(
          ...
          decoration: const FormaDecoration(
  ...
  Widget _primaryButton(String label, VoidCallback? onTap, {required Color color, required Color fg, bool busy = false}) => SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: busy
              ? const SizedBox(width: 22, height: 22, child: FormaLoader(strokeWidth: 2, color: Colors.white))
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            child: Text(label,
                ...
                style: TextStyle(
                    color: active ? const Color(0xFFFFFFFF) : AppColors.text,
  ...
  Widget _reviewComposer() {
    ...
    // [FIX-CHAT-FINISH] أول مرة: آخر سؤال بيشغل الحفظ تلقائيا (انظر _commit)،
    // فبدل ما يفضل قدام المستخدم زرار باهت عليه مؤشر تحميل — وهو شكل بيوحي إن
    // التطبيق معلق — بنعرض حالة تجهيز واضحة ومقصودة طول ما الحفظ شغال.
    // الانتقال لصفحة التحليل بيتم من _save نفسها بعد ما الحفظ يخلص بنجاح،
    // فمفيش أي تغيير في اللوجيك. ولو الحفظ فشل saving بترجع false والزرار
    // بيرجع يظهر فعال وClickable لإعادة المحاولة.
    if (!(_editMode && !_fromScratch)) return saving ? _autoSavingPanel() : save;
  ...
  /// [FIX-CHAT-FINISH] حالة «بنجهز خطتك» أثناء الحفظ التلقائي في نهاية الشات.
  /// مؤشر تقدم حي ومقصود بدل الزرار الباهت: المستخدم يفهم إن الشات خلص وإن
  /// التطبيق بيجهز خطته الآن، وبعدها بينتقل تلقائيا (من _save) لما العملية
  /// تخلص بنجاح.
  Widget _autoSavingPanel() {
    return Container(
      height: 52,
      decoration: FormaDecoration(
        color: AppColors.card,
      ...
      child: Row(
        ...
        children: [
          const SizedBox(
            width: 20,
            height: 20,
            child: FormaLoader(strokeWidth: 2.2, color: AppColors.nu),
          ),
  ...
  Map<String, String> _rateLabels() {
    ...
    // [OWNER-RULE 2026-09] أنثى وزنها تحت 70 كجم: هدف الكيلو/أسبوع ملغي —
    // المحرك بيسقفه على 0.75 على أي حال، فمش هنعرضه كاختيار من الأساس.
    final w = double.tryParse(weight.text.trim()) ?? 0;
    if (gender == 'female' && w > 0 && w < 70) {
      return const {
        '0.25': '1/4 كيلو',
        '0.5': '1/2 كيلو',
        '0.75': '3/4 كيلو',
  ...
  Widget _selectionMark(bool selected, bool multi, Color accent) => AnimatedContainer(
        ...
        child: selected
            ? const FormaIcon(Icons.check_rounded, size: 15, color: Color(0xFFFFFFFF))
            : null,
      );
      ...
      Row(children: [
        Semantics(
          ...
          child: IconButton.filled(
            ...
            icon: const FormaIcon(Icons.arrow_upward_rounded, size: 22),
          ),
        ),
        const SizedBox(width: 10),
        Expanded(
...
class _TypingDotsState extends State<_TypingDots>
  ...
  void didChangeDependencies() {
    ...
    // An endlessly repeating ticker costs a vsync callback every frame, and a
    // pulsing indicator is exactly the kind of motion users switch off for
    // accessibility reasons. Honour that setting instead of always animating.
