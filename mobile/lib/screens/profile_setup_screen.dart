...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  Future<void> _save() async {
    ...
    // ليها حالًا — مش جوه الشات. كده الزر مابيعلقش في التحميل أبدًا.
    ...
    if (response.ok) {
      ...
      if (response.data['profileSaved'] == true && response.data['profile'] is Map) {
        ...
        if (!mounted) return;
        // البيانات اتحفظت تماماً — نروح للتحليل زي نجاح والخطة هتاجي دقيقة تتولد
        if (!(_editMode && !_fromScratch)) {
          Navigator.of(context).pushReplacement(MaterialPageRoute(
            builder: (_) => AnalysisScreen(payload: ProfileStore.I.profile!, profileAlreadySaved: true)));
        } else {
  ...
  Widget _reviewComposer() {
    ...
    // [FIX-CHAT-FINISH] أول مرة: آخر سؤال بيشغّل الحفظ تلقائيًا (انظر _commit)،
    // فبدل ما يفضل قدام المستخدم زرار باهت عليه مؤشر تحميل — وهو شكل بيوحي إن
    // التطبيق معلّق — بنعرض حالة تجهيز واضحة ومقصودة طول ما الحفظ شغال.
    ...
    // بيرجع يظهر فعّال وClickable لإعادة المحاولة.
  ...
  /// التطبيق بيجهز خطته الآن، وبعدها بينتقل تلقائيًا (من _save) لما العملية
  /// تخلص بنجاح.
      ...
      child: Row(
        ...
        children: [
          const SizedBox(
            width: 20,
            height: 20,
            child: CircularProgressIndicator(strokeWidth: 2.2, color: AppColors.nu),
          ),
