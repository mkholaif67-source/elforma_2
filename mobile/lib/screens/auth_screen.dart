...
class _AuthScreenState extends State<AuthScreen> with WidgetsBindingObserver {
  ...
  void _revealFocusedField() {
    ...
    _revealTimer = Timer(const Duration(milliseconds: 300), () {
      ...
      if (MediaQuery.viewInsetsOf(context).bottom <= 0) {
        _authScroll.animateTo(0, duration: const Duration(milliseconds: 180), curve: Curves.easeOut);
        return;
      }
      final target = _confirmationFocus.hasFocus ? _confirmationKey.currentContext : _passwordFocus.hasFocus ? _passwordKey.currentContext : null;
      if (target != null) Scrollable.ensureVisible(target, alignment: .65, duration: const Duration(milliseconds: 180), curve: Curves.easeOut);
    });
  }
  ...
  void initState() {
    ...
    _passwordFocus.addListener(_revealFocusedField);
    _confirmationFocus.addListener(_revealFocusedField);
  ...
  void dispose() {
    ...
    _revealTimer?.cancel();
    _passwordFocus.removeListener(_revealFocusedField);
    _confirmationFocus.removeListener(_revealFocusedField);
    _authScroll.dispose();
    _ident.dispose();
    _email.dispose();
    _phone.dispose();
        ...
        body: LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (constraints.maxWidth < 360 || textScale > 1.3) {
              return _buildReflowAuth(keyboardInset);
            }
            // Preserve the same scale when the keyboard opens. The viewport
            // shrinks instead, so Flutter can scroll the focused field above it.
            final fullHeight = constraints.maxHeight + keyboardInset;
            final widthScale =
                math.min(constraints.maxWidth, 430.0) / _designWidth;
            final passwordExtra =
                _pass.text.isNotEmpty && _passwordProblem(_pass.text) != null
                ? 18.0
                : 0.0;
            final designHeight = _login
                ? _designHeight
                : math.max(
                    _designHeight,
                    815.0 +
                        passwordExtra +
                        (_error != null ? 54 : 0) -
                        (_showConfirmation ? 0 : 56),
                  );
            // Fit typical phones; cap reduction at 10% so short screens keep
            // readable fields and use only the small necessary scroll range.
            final scale = math.min(
              widthScale,
              math.max(fullHeight / designHeight, widthScale * .9),
            );
            final canvasWidth = _designWidth * scale;
            final canvasHeight = designHeight * scale;
            final canScroll =
                keyboardInset > 0 || canvasHeight > constraints.maxHeight + .5;
  ...
  Widget _buildReflowAuth(double keyboardInset) {
    final passwordProblem = _pass.text.isEmpty
        ? null
        : _passwordProblem(_pass.text);
    return SafeArea(
      child: SingleChildScrollView(
        controller: _authScroll,
        padding: EdgeInsets.fromLTRB(20, 20, 20, 28 + keyboardInset),
        child: Center(
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  _login ? 'مرحبا بعودتك' : 'إنشاء حساب',
                ...
                _authPasswordField(
                  ...
                  hint: 'كلمة المرور',
                  autofillHints: _login ? const [AutofillHints.password] : const [AutofillHints.newPassword],
                  hidden: _hide,
                  onToggle: () => setState(() => _hide = !_hide),
                  textInputAction:
                      _login ? TextInputAction.done : TextInputAction.next,
                  onSubmitted: (_) =>
                      _login ? _submit() : _continueToConfirmation(),
                ),
                ...
                _orDivider(),
                const SizedBox(height: 18),
                SizedBox(height: 52, child: _googleButton()),
                const SizedBox(height: 16),
                TextButton(
                  onPressed: _busy ? null : () => _switchMode(!_login),
                  child: Text(
                    _login
          ...
          child: Row(
            ...
            children: [
              const Icon(Icons.error_outline_rounded,
                  color: Color(0xFFC04438), size: 19),
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(
            ...
            height: heroHeight,
            child: ClipPath(
              // The crest clears the logo; the photograph continues on both
              // sides. Only its visible edge changes, never the page geometry.
              clipper: _AuthHeroWaveClipper(crest: logoTop - 8),
          ...
          if (!_login)
            Positioned(
              ...
              child: Semantics(
                ...
                child: Tooltip(
                  message: 'الرجوع إلى تسجيل الدخول',
                  child: Material(
  ...
  List<Widget> _buildLoginContent() {
    final errorShift = _error == null ? 0.0 : 54.0;
    return [
      _positionedText(
        top: 306,
        height: 43,
  ...
  List<Widget> _buildSignupContent() {
    final passwordProblem = _pass.text.isEmpty
        ? null
        : _passwordProblem(_pass.text);
    final passwordShift = passwordProblem == null ? 0.0 : 32.0;
    final errorShift = _error == null ? 0.0 : 54.0;
    final confirmationShift = _showConfirmation ? 0.0 : -56.0;
    final lowerShift = passwordShift + errorShift + confirmationShift;
    ...
    return [
      ...
      Positioned(
        ...
        child: _authPasswordField(
          ...
          onToggle: () => setState(() => _hide = !_hide),
          textInputAction: TextInputAction.next,
          onSubmitted: (_) => _continueToConfirmation(),
        ),
      ),
      if (passwordProblem != null)
        Positioned(
      ...
      if (_error != null)
        _errorBanner(top: 603 + passwordShift + confirmationShift),
      AnimatedPositioned(
        key: const ValueKey('signup-submit-position'),
        duration: MediaQuery.disableAnimationsOf(context)
                ? Duration.zero
                : const Duration(milliseconds: 260),
        curve: Curves.easeOutCubic,
      ...
      AnimatedPositioned(
        key: const ValueKey('signup-divider-position'),
        duration: MediaQuery.disableAnimationsOf(context)
                ? Duration.zero
                : const Duration(milliseconds: 260),
        curve: Curves.easeOutCubic,
      ...
      AnimatedPositioned(
        key: const ValueKey('signup-google-position'),
        duration: MediaQuery.disableAnimationsOf(context)
                ? Duration.zero
                : const Duration(milliseconds: 260),
        curve: Curves.easeOutCubic,
      ...
      AnimatedPositioned(
        key: const ValueKey('signup-footer-position'),
        duration: MediaQuery.disableAnimationsOf(context)
                ? Duration.zero
                : const Duration(milliseconds: 260),
        curve: Curves.easeOutCubic,
  ...
  Widget _authField({
    ...
    return DecoratedBox(
      ...
      child: TextField(
        key: focusNode == _passwordFocus ? _passwordKey : focusNode == _confirmationFocus ? _confirmationKey : null,
        controller: controller,
        focusNode: focusNode,
        scrollPadding: const EdgeInsets.fromLTRB(20, 24, 20, 32),
        obscureText: obscure,
...
class _AuthHeroWaveClipper extends CustomClipper<Path> {
  const _AuthHeroWaveClipper({required this.crest});
  ...
  Path getClip(Size size) {
    ...
    return Path()
      ..moveTo(0, 0)
      ..lineTo(0, crest + 27)
      ..cubicTo(w * .19, crest + 57, w * .32, crest, w * .49, crest)
      ..cubicTo(w * .64, crest, w * .78, crest + 49, w, crest + 9)
      ..lineTo(w, 0)
      ..close();
  }

  @override
  bool shouldReclip(covariant _AuthHeroWaveClipper oldClipper) =>
      oldClipper.crest != crest;
}
...
class _AuthBackdropPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final front = Paint()
      ..color = const Color(0xFFD2E4D0).withValues(alpha: .62);
