...
class _WorkoutScreenState extends State<WorkoutScreen>
  ...
  Future<void> _generate({bool loadAlternatives = false, bool silent = false}) async {
    ...
    if (r.ok && r.data['ok'] == true && !storedNeedsRepair) {
    ...
    if (r.ok && r.data['ok'] == true) {
      ...
      if(_reloadRequested&&mounted){_reloadRequested=false;_generate(silent:_silentActive);}
    }
  }
...
// [OWNER-RULE] تحميل الخطة على التليفون كملف حقيقي — مش رابط بينتهي.
  ...
  Widget _gatedBody() {
    ...
    if(store.loadError!=null && store.profile==null){
      return ErrorView(message:store.loadError!,onRetry:()async{await store.ensureLoaded(force:true);if(mounted)_boot();});
    }
...
/// يعرض تمرين النهاردة أو شاشة الراحة بناء على الجدول الفعلي.
  ...
  Map<String, dynamic> _resolveToday(Map<String, dynamic> plan) {
    ...
    if (week.length < 7) {
      ...
      isRestDay = todayDay.isNotEmpty && _dayIsRest(todayDay);
    }
