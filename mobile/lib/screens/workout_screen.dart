...
class _WorkoutScreenState extends State<WorkoutScreen>
  ...
  bool _storedPlanNeedsRepair(dynamic rawWorkoutPlan) {
    final minExercises = exp == 'advanced' ? 6 : (gender == 'male' ? 5 : 0);
  ...
  bool _hasCompletedToday(dynamic rows) {
    ...
    return rows.whereType<Map>().any((s) => s['status'] == 'completed' &&
        '${s['finished_at'] ?? ''}'.startsWith(today));
  }
  ...
  Future<void> _generate() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _completedToday = _hasCompletedToday(bootstrap.data['recentSessions']);
    ...
    // الخطة المحفوظة السليمة تظهر فورًا؛ لا نعيد تشغيل المحرك عند كل فتح.
    if (storedFallback != null && !storedNeedsRepair) {
...
/// يعرض تمرين النهاردة أو شاشة الراحة بناء على الجدول الفعلي.
  ...
  Widget _completedWorkoutHero() => Container(
        width: double.infinity,
        ...
        child: const Column(children: [
          Icon(Icons.check_circle_rounded, color: AppColors.nu, size: 52),
          SizedBox(height: 12),
          Text('تم إنهاء وتسجيل جلسة النهاردة',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          SizedBox(height: 6),
          Text('زر بدء الجلسة اتقفل والجلسة محفوظة في سجل التدريب',
              textAlign: TextAlign.center,
