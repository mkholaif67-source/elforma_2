...
class _ExercisePickerScreenState extends State<ExercisePickerScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      if (response.ok && response.data['exercises'] is List) {
        ...
        error = response.friendlyError('تعذر تحميل البدائل');
      }
    });
  }
              ...
              : ListView(
                  ...
                  children: [
                    Container(
                      ...
                      decoration: FormaDecoration(color: AppColors.nu.withValues(alpha: .08),
                          borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.nu.withValues(alpha: .24))),
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const FormaIcon(Icons.health_and_safety_outlined, color: AppColors.nu),
  ...
  Widget _tile(Map<String, dynamic> exercise) => Container(
        ...
        child: ListTile(
          ...
          leading: Container(width: 40, height: 40, decoration: FormaDecoration(
              color: AppColors.wo.withValues(alpha: .12), borderRadius: BorderRadius.circular(11)),
              child: const FormaIcon(Icons.swap_horiz_rounded, color: AppColors.wo2)),
          title: Text((exercise['n'] ?? '').toString(),
              style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 14)),
          subtitle: Text('${exercise['mu'] ?? ''} · مستوى ${exercise['tier'] ?? 'A'}',
              style: const TextStyle(color: AppColors.muted, fontSize: 11)),
          trailing: const FormaIcon(Icons.chevron_left_rounded, color: AppColors.muted),
        ),
      );
}
