...
class _MealFavoritesScreenState extends State<MealFavoritesScreen>{
 ...
 @override Widget build(BuildContext context)=>Scaffold(
  appBar:AppBar(title:const Text('أطعمتك المفضلة',style:TextStyle(fontWeight:FontWeight.w800)),
   leading:IconButton(icon:const FormaIcon(Icons.arrow_back),onPressed:()=>Navigator.pop(context,_changed)),
  ...
  body:CustomScrollView(slivers:[
   SliverPadding(padding:const EdgeInsets.fromLTRB(16,8,16,0),sliver:SliverToBoxAdapter(child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
    Container(padding:const EdgeInsets.all(16),decoration:FormaDecoration(color:AppColors.nu2.withValues(alpha:.09),borderRadius:BorderRadius.circular(20),border:Border.all(color:AppColors.nu2.withValues(alpha:.28))),
    ...
    Wrap(spacing:8,runSpacing:8,children:[for(final slot in _labels.keys)ChoiceChip(
     avatar:FormaIcon(_icons[slot],size:18,color:_slot==slot?AppColors.nu2:AppColors.muted),
    ...
    const SizedBox(height:16),
    TextField(controller:_search,decoration:InputDecoration(hintText:'ابحث عن أكلة بتحبها',prefixIcon:const FormaIcon(Icons.search),filled:true,fillColor:AppColors.card,border:OutlineInputBorder(borderRadius:BorderRadius.circular(16))),
   ...
   if(_busy)const SliverToBoxAdapter(child:Padding(padding:EdgeInsets.all(32),child:Center(child:FormaLoader())))
    ...
    return Padding(padding:const EdgeInsets.only(bottom:8),child:Material(color:chosen?AppColors.nu2.withValues(alpha:.10):AppColors.card,
     ...
     clipBehavior:Clip.antiAlias,child:ListTile(contentPadding:const EdgeInsets.symmetric(horizontal:16,vertical:10),
      ...
      trailing:FormaIcon(chosen?Icons.favorite:Icons.favorite_border,color:chosen?AppColors.nu2:AppColors.muted,size:26),
