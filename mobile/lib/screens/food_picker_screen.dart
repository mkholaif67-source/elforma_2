...
class _FoodPickerScreenState extends State<FoodPickerScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      if (response.ok && response.data[sourceKey] is List) {
        ...
        error = response.friendlyError('تعذر تحميل الأطعمة');
      }
    });
  }
    ...
    final grams = await showDialog<num>(
      ...
      builder: (context) => StatefulBuilder(builder: (context, setDialogState) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: FoodPhoto(name: '${food['nameAr'] ?? food['nameEn'] ?? ''}', size: 96)),
          const SizedBox(height: 14),
          if (widget.replacementTarget != null) ...[
            Container(padding: const EdgeInsets.all(10), decoration: FormaDecoration(
                color: AppColors.nu.withValues(alpha: .09), borderRadius: BorderRadius.circular(11)),
                child: Row(children: [
                  const FormaIcon(Icons.balance_rounded, color: AppColors.nu, size: 19),
        ...
        actions: [
          ...
          FilledButton(
            style: FilledButton.styleFrom(backgroundColor: AppColors.nu, foregroundColor: const Color(0xFFFFFFFF)),
            onPressed: () {
      ...
      body: Column(children: [
        Padding(
          ...
          child: TextField(
            controller: search,
            onChanged: _onSearch,
            decoration: const InputDecoration(hintText: 'ابحث عن أكل: فراخ أرز بيض فاكهة', prefixIcon: FormaIcon(Icons.search_rounded)),
          ),
        ),
        _modeTabs(),
  ...
  Widget _mode(String value, String label, IconData icon) => Expanded(child: InkWell(
        ...
        child: Container(padding: const EdgeInsets.symmetric(vertical: 9),
          decoration: FormaDecoration(color: mode == value ? AppColors.nu : AppColors.card,
              borderRadius: BorderRadius.circular(11), border: Border.all(color: mode == value ? AppColors.nu : AppColors.line)),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            FormaIcon(icon, size: 16, color: mode == value ? const Color(0xFFFFFFFF) : AppColors.muted),
            const SizedBox(width: 5),
            Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900,
                color: mode == value ? const Color(0xFFFFFFFF) : AppColors.text)),
          ]),
        ),
      ));
        ...
        SizedBox(height: 42, child: ListView(scrollDirection: Axis.horizontal,
          ...
          children: categories.entries.map((entry) {
            ...
            return Padding(padding: const EdgeInsets.only(left: 7), child: ChoiceChip(
              ...
              backgroundColor: AppColors.card,
              labelStyle: TextStyle(color: active ? const Color(0xFFFFFFFF) : AppColors.text, fontWeight: FontWeight.w800),
  ...
  Widget _foodTile(Map<String, dynamic> food) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          FoodPhoto(name: '${food['nameAr'] ?? food['nameEn'] ?? ''}', size: 46),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              if (widget.replacementTarget != null) Container(
                ...
                decoration: FormaDecoration(color: match >= 75 ? AppColors.nu.withValues(alpha: .14) : AppColors.wo.withValues(alpha: .13), borderRadius: BorderRadius.circular(7)),
          ...
          IconButton(
            ...
            icon: FormaIcon(food['favorite'] == true ? Icons.star_rounded : Icons.star_border_rounded,
                color: food['favorite'] == true ? const Color(0xFF996214) : AppColors.muted),
          ),
        ]),
      ),
    );
