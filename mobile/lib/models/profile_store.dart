...
class ProfileStore extends ChangeNotifier {
  ProfileStore._() { Api.I.accountChanges.addListener(_resetSession); }
  void _resetSession() {
    _revision++;
    _inFlight=null;
    _profile=null;
  ...
  Future<void> ensureLoaded({bool force = false}) {
    ...
    // Register the shared future BEFORE _load notifies listeners. A listener
    // can synchronously call ensureLoaded again; it must join, never recurse.
    final completion = Completer<void>();
    final shared = completion.future;
    _inFlight = shared;
    unawaited(_load(force: force).then((_) {
      if (identical(_inFlight, shared)) _inFlight = null;
      completion.complete();
    }).catchError((Object error, StackTrace stack) {
      if (identical(_inFlight, shared)) _inFlight = null;
      _loading = false;
      loadError = 'تعذر تحميل بياناتك. حاول مرة أخرى';
      completion.complete();
      notifyListeners();
    }));
    return shared;
  ...
  Future<void> _load({bool force = false}) async {
      ...
      try {
        ...
        if (raw != null && raw.isNotEmpty) {
          ...
          if (decoded is Map) {
            ...
            final current = Api.I.accountScope ?? Api.I.accountId;
    ...
    try {
      ...
      if(revision!=_revision)return;
      if(response.ok){_loaded=true;}else{loadError=response.friendlyError('تعذر تحميل بياناتك. حاول مرة أخرى');}
  ...
  Future<void> _persist() async {
    final revision=_revision, owner=Api.I.accountScope ?? Api.I.accountId;
    final snapshot=Map<String,dynamic>.from(_profile??{});
    try {
      final sp = await SharedPreferences.getInstance();
      // بنختم الكاش بصاحبه عشان مايتقراش لحساب تاني على نفس الجهاز.
      if(revision!=_revision || owner!=(Api.I.accountScope ?? Api.I.accountId))return;
  ...
  Map<String, dynamic> workoutPayload() {
    return {
      ...
      'preferredDays': ((_profile?['preferredDays'] as List?) ?? const []).whereType<num>().map((d) => d.toInt()).toList(),
    };
  }
