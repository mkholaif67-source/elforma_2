...
class NutritionFoodItem {
  ...
  factory NutritionFoodItem.fromJson(Map raw) {
    ...
    return NutritionFoodItem(
      ...
      addonMaxGrams: _number(raw['addonMaxGrams'] ?? raw['_addonMaxGrams'] ?? 10),
    );
  }
  ...
  NutritionFoodItem atGrams(num newGrams) {
    ...
    final rounded = isAddon ? newGrams.round() : (newGrams / 5).round() * 5;
    final safeGrams = isAddon
        ? rounded.clamp(2, addonMaxGrams)
        : (rounded < 5 ? 5 : rounded);
    final applied = safeGrams / grams;
    return NutritionFoodItem(
      id: id,
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (isAddon) 'addonMaxGrams': addonMaxGrams,
      };
}
