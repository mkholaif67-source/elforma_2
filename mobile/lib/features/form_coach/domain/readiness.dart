...
String readinessCueAr(ReadinessIssue issue) {
  switch (issue) {
    ...
    case ReadinessIssue.lowConfidence:
      return 'التتبع مش واضح — وسّع المكان شوية';
  ...
  ReadinessReport update({
    ...
    for (final FcJoint joint in spec.requiredJoints) {
      ...
      if (spec.requireBothSides) {
        ...
        if (!leftOk && !rightOk) {
          missing = true;
        } else if (leftOk && (!rightOk || left.visibility >= right.visibility)) {
          used.add(fcLandmarkFor(joint, FcSide.left));
        } else {
          used.add(fcLandmarkFor(joint, FcSide.right));
        }
    ...
    final double? torso = FcGeometry.bodyScale(sample, minVisibility: spec.minVisibility);
    if (torso != null) {
    ...
    final double? spread =
        FcGeometry.shoulderSpreadRatio(sample, minVisibility: spec.minVisibility);
