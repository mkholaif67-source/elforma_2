...
class ReadinessEvaluator {
  ...
  ReadinessReport update({
    ...
    final visibleSide = sideConfidence(FcSide.left) >= sideConfidence(FcSide.right)
    ...
    for (final FcJoint joint in spec.requiredJoints) {
      ...
      final bool rightOk = right != null && right.visibility >= spec.minVisibility;
    ...
    final double? torso = FcGeometry.bodyScale(sample.metricSpace, minVisibility: spec.minVisibility);
    if (torso != null) {
    ...
    final double? spread =
        FcGeometry.shoulderSpreadRatio(sample.metricSpace, minVisibility: spec.minVisibility);
    ...
    if (spread != null) {
      ...
      if (spec.view == PreferredView.side && spread > FcTuning.sideViewMaxSpread) {
        issues.add(ReadinessIssue.wrongViewSide);
      } else if (spec.view == PreferredView.front && spread < FcTuning.frontViewMinSpread) {
        issues.add(ReadinessIssue.wrongViewFront);
      }
    }

    final double meanVisibility = used.isEmpty ? 0 : _meanVisibility(sample, used);
    if (used.isNotEmpty && meanVisibility < spec.minVisibility) {
      issues.add(ReadinessIssue.lowConfidence);
    }

    if (spec.checkLighting && luma != null && luma < FcTuning.lowLightMeanLuma) {
      issues.add(ReadinessIssue.lowLight);
    }
    if (subjectUnstable) issues.add(ReadinessIssue.cameraMoving);
    if (fps > 0 && fps < FcTuning.minUsableFps) issues.add(ReadinessIssue.lowFps);
    ...
    if (issues.isEmpty) {
      ...
      progress = spec.stableMs <= 0 ? 1 : (held / spec.stableMs).clamp(0.0, 1.0);
