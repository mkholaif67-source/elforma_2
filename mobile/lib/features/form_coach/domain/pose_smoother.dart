...
class _LandmarkFilters {
  _LandmarkFilters({
    ...
    required double derivativeCutoff,
  })  : x = OneEuroFilter(
          minCutoff: minCutoff,
          beta: beta,
...
class PoseSmoother {
  PoseSmoother({
    this.visibilityAlpha = FcTuning.visibilitySmoothingAlpha,
    this.minCutoff = FcTuning.smoothMinCutoff,
    this.beta = FcTuning.smoothBeta,
    this.derivativeCutoff = FcTuning.smoothDerivativeCutoff,
  });
  ...
  PoseSample smooth(PoseSample sample) {
    ...
    if (previousMs != null && (sample.timestampMs <= previousMs ||
        sample.timestampMs - previousMs > FcTuning.repAbortGapMs)) reset();
    ...
    for (final FcLandmark landmark in FcLandmark.values) {
      ...
      if (raw == null || !raw.x.isFinite || !raw.y.isFinite ||
          !raw.visibility.isFinite || raw.visibility < FcTuning.landmarkMinVisibility) {
        _filters.remove(landmark);
        continue;
      }
      ...
      final double visibility = math.min(raw.visibility,
    ...
    return PoseSample(timestampMs: sample.timestampMs, points: output, imageAspect: sample.imageAspect);
  }
