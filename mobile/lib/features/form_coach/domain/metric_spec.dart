...
enum MetricKind {
  ...
  /// Distance between the left and right instance of joints[0], / torso scale.
  bilateralDistance,
...
class MetricReading {
  const MetricReading(
      {required this.value, required this.confidence, this.side});
  ...
  @override
  String toString() =>
      '${value.toStringAsFixed(1)} (c=${confidence.toStringAsFixed(2)})';
}
...
class MetricEvaluator {
  ...
  MetricSet evaluate(
    ...
    sample = sample.metricSpace;
    final double? scale =
        FcGeometry.bodyScale(sample, minVisibility: minVisibility);
    ...
    for (final MetricSpec spec in specs) {
      ...
      if (raw != null) {
        out[spec.id] = MetricReading(
            value: raw.value, confidence: raw.confidence, side: raw.side);
      }
    }
    for (final MetricSpec spec in specs) {
      if (!spec.isDerived) continue;
  ...
  _Raw? _direct(
    ...
    switch (spec.side) {
      ...
      case MetricSide.activeSide:
        if (activeSide != null) {
          final _Raw? chosen =
              _forSide(spec, sample, activeSide, minVisibility, scale);
          return chosen;
        }
        return _bestConfidence(spec, sample, minVisibility, scale);
      case MetricSide.mean:
        final _Raw? left =
            _forSide(spec, sample, FcSide.left, minVisibility, scale);
        final _Raw? right =
            _forSide(spec, sample, FcSide.right, minVisibility, scale);
      ...
      case MetricSide.minValue:
        ...
        if (a == null) return b;
        if (b == null) return a;
        final bool takeA = spec.side == MetricSide.maxValue
            ? a.value >= b.value
            : a.value <= b.value;
        return takeA ? a : b;
    }
  ...
  _Raw? _forSide(
    ...
    switch (spec.kind) {
      ...
      case MetricKind.limbAbductionFromTorso:
        ...
        FcPoint? hip =
            sample.joint(FcJoint.hip, side, minVisibility: minVisibility);
        if (hip != null) {
      ...
      case MetricKind.normalizedVerticalOffset:
        ...
        break;
      case MetricKind.bilateralDistance:
        if (spec.joints.isEmpty || scale == null || scale <= 1e-3) return null;
        final left = fcLandmarkFor(spec.joints.first, FcSide.left);
        final right = fcLandmarkFor(spec.joints.first, FcSide.right);
        final a = sample.point(left, minVisibility: minVisibility);
        final b = sample.point(right, minVisibility: minVisibility);
        if (a == null || b == null) return null;
        used.addAll([left, right]);
  ...
  MetricReading? _derived(
    ...
    switch (spec.kind) {
      ...
      case MetricKind.absoluteDeltaFromBaseline:
        ...
        return MetricReading(
          value:
              spec.kind == MetricKind.deltaFromBaseline ? delta : delta.abs(),
