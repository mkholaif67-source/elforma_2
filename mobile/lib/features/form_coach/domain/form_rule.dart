...
class FormRuleTracker {
  ...
  FormRuleState update({
    ...
    if (!poseUsable) {
      ...
      blocked = 'no-metric';
    } else if (reading.confidence < rule.minConfidence) {
    ...
    if (violating) {
      ...
      if (_violationSinceMs == null ||
          (lastViolation != null && tMs - lastViolation > FcTuning.violationStreakGapMs)) {
        _violationSinceMs = tMs;
      }
      _lastViolationMs = tMs;
    } else {
      final int? lastViolation = _lastViolationMs;
      if (lastViolation != null && tMs - lastViolation > FcTuning.violationStreakGapMs) {
        _violationSinceMs = null;
      }
    }

    final int violationMs = _violationSinceMs == null ? 0 : tMs - _violationSinceMs!;
    ...
    if (violating && violationMs >= rule.persistenceMs) {
      if (consistency < rule.minConsistency) {
        ...
        if (_fires >= rule.maxFiresPerSet) {
          ...
          _fires++;
          _lastFiredMs = tMs;
          // The error must build up again before we speak a second time.
          _violationSinceMs = null;
          _window.clear();
        }
      }
