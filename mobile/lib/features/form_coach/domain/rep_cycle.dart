...
class RepCounter {
  ...
  int? _occlusionSinceMs;
  double? _preGapDriver;
  ...
  List<RepEvent> update({
    ...
    if (previousUsable != null &&
        tMs - previousUsable > FcTuning.repAbortGapMs) {
    ...
    if (!usable || driver == null) {
      if (_inCycle) {
        _occlusionSinceMs ??= tMs;
        _preGapDriver ??= _previousDriver;
      }
      _pendingZone = null;
      _returnSinceMs = null;
    ...
    if (_occlusionSinceMs != null) {
      final continuous = previousUsable != null &&
          tMs - previousUsable <= 200 &&
          _preGapDriver != null &&
          (driver - _preGapDriver!).abs() <=
              math.min(12, spec.fullRange * .12) &&
          _reachedBottom;
      if (!continuous) _gapInCycle = true;
      _occlusionSinceMs = null;
      _preGapDriver = null;
    }
    _lastUsableMs = tMs;
    _driverValue = driver;
    ...
    if (previous != null) {
      ...
      if (delta.abs() > math.min(.4, spec.fullRange * .01)) {
        _towardBottom = spec.descending ? delta < 0 : delta > 0;
      }
    }
    _previousDriver = driver;

    // The stable zone can still be top after a brief endpoint or dropout.
    // Require observed return persistence independently of that stale zone.
    if (_zone == _Zone.top &&
        candidate == _Zone.top &&
        (!_inCycle || tMs - _returnSinceMs! >= spec.minPhaseMs)) {
      if (_inCycle) {
        ...
        final int minCommitMs =
            math.max(spec.noiseFloorMs, FcTuning.hardMinRepMs);
        _inCycle = false;
    ...
    if (_inCycle) {
      ...
      final bool deeper =
          spec.descending ? driver < _extreme : driver > _extreme;
  ...
  RepEvent? interrupt(int tMs) {
    ...
    final event = _inCycle && amplitude >= spec.partialAmplitude
        ? RepEvent(
            kind: RepEventKind.cycleAborted,
            amplitude: amplitude,
            durationMs: math.max(0, tMs - _cycleStartMs),
            assessable: false)
        : null;
    abortCycle();
    return event;
