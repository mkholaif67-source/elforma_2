...
enum FormVerdict { tracking, correct, clearError, cannotAssess, finished }
...
class FormCoachSnapshot {
  const FormCoachSnapshot({
    ...
    this.cannotAssessAttempts = 0,
    this.trackedReps = 0,
    this.tooFastReps = 0,
    this.tempoImproved = false,
    this.corrections = const <CorrectionSummary>[],
  });
  ...
  /// Runtime-only counters; never persisted as workout progress.
  final int attempts;
  final int valid;
  final int incorrect;
  final int incomplete;
  final int cannotAssessAttempts;
  final int trackedReps;
  final int tooFastReps;
  final bool tempoImproved;
  final List<CorrectionSummary> corrections;
