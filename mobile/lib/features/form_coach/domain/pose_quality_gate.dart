...
class PoseQualityGate {
  PoseQualityGate({this.allowLowerBodyAnchor = false});
  final bool allowLowerBodyAnchor;
  int? _candidateSince;
  int? _lastMs;
  FcPoint? _center;
  double? _scale;
  ...
  PoseSample filter(PoseSample sample) {
    final points = <FcLandmark, FcPoint>{
      for (final e in sample.points.entries)
        if (e.value.x.isFinite &&
            e.value.y.isFinite &&
            e.value.z.isFinite &&
            e.value.visibility.isFinite &&
            e.value.visibility >= .65 &&
            e.value.x >= 0 &&
            e.value.x <= 1 &&
            e.value.y >= 0 &&
            e.value.y <= 1)
    ...
    double distance(FcPoint a, FcPoint b) =>
    ...
    if (torsos.isEmpty && allowLowerBodyAnchor) {
      for (final side in FcSide.values) {
        final hip = points[fcLandmarkFor(FcJoint.hip, side)];
        final knee = points[fcLandmarkFor(FcJoint.knee, side)];
        final ankle = points[fcLandmarkFor(FcJoint.ankle, side)];
        if (hip == null || knee == null || ankle == null) continue;
        final thigh = distance(hip, knee);
        final shin = distance(knee, ankle);
        if (thigh >= .06 &&
            thigh <= .65 &&
            shin >= thigh * .35 &&
            shin <= thigh * 1.6) {
    ...
    if (torsos.isEmpty ||
        points.length < (allowLowerBodyAnchor ? 3 : 4) ||
        !aspect.isFinite ||
        aspect <= 0) {
      reset();
    ...
    if (dt > 0 && dt < .5) {
      points.removeWhere((key, point) {
        ...
        return previous != null &&
            distance(previous, point) > scale * (.35 + 8 * dt);
      });
    }
    ...
    for (final side in FcSide.values) {
      for (final chain in [
        ...
        [FcJoint.ankle, FcJoint.heel],
        [FcJoint.ankle, FcJoint.footIndex],
      ]) {
        for (var i = 1; i < chain.length; i++) {
          ...
          final maxRatio =
              (chain[i] == FcJoint.heel || chain[i] == FcJoint.footIndex)
                  ? .65
                  : 1.6;
          if (parent == null ||
              (child != null && distance(parent, child) > scale * maxRatio)) {
            points.remove(key);
          }
        }
      }
    ...
    final discontinuity = _lastMs != null &&
        (sample.timestampMs <= _lastMs! ||
            sample.timestampMs - _lastMs! > 500 ||
