...
class FormCoachEngine {
  ...
  final PoseQualityGate _quality = PoseQualityGate();
  FcSide? _visibleSide;
  ...
  void _limitOverlay() {
    ...
    _pose = PoseSample(timestampMs: pose.timestampMs, imageAspect: pose.imageAspect,
      points: {for (final e in pose.points.entries) if (allowed.contains(e.key)) e.key: e.value});
  }
  ...
  void _selectSide(PoseSample? pose) {
    ...
    final joints = variant.readiness.requiredJoints;
    double confidence(FcSide side) => pose.confidenceOf(
        joints.map((joint) => fcLandmarkFor(joint, side)));
    final selected = _activeSide ?? _visibleSide;
    if (selected == null || confidence(selected) < variant.driverMinConfidence) {
      _visibleSide = confidence(FcSide.left) >= confidence(FcSide.right)
          ? FcSide.left : FcSide.right;
      if (_activeSide != _visibleSide) _counter?.abortCycle();
  ...
  void _interruptAttempt(int tMs) {
    ...
    if (event != null) { _attempts++; _cannotAssessAttempts++; }
  ...
  void _liveFrame(PoseSample? pose, int tMs, double? luma, bool unstable) {
    ...
    if (pose != null) {
      ...
      if (previousSide != _activeSide) {
        metrics = _evaluators[variant.id]!.evaluate(pose,
            activeSide: _activeSide, baselines: _baselines);
      }
    }
    _metrics = metrics;
    ...
    if (wasInCycle || (counter?.inCycle ?? false)) {
      _cycleHadError = _cycleHadError || states.any((state) => state.evaluable && state.candidate);
    }
    for (final RepEvent event in events) {
      if (!event.assessable) {
        _attempts++; _cannotAssessAttempts++; _cycleHadError = false;
        continue;
      }
      switch (event.kind) {
    ...
    if (reason == CannotAssessReason.none) candidates.addAll(_repFeedbackCues(variant, events, tMs, globalCooldownOk));
    ...
    final FormCue? chosen = reason == CannotAssessReason.none ? _arbitrate(candidates) : null;
    if (chosen != null) {
      _frameCue = chosen;
      _lastCue = chosen;
      _lastCueMs = tMs;
  ...
  List<FormCue> _repFeedbackCues(
    ...
    if (spec.hasTempoCue) {
      for (final RepEvent event in events) {
        ...
        // No explicit plan tempo means no defensible speed target.
        if (prescribed == null) continue;
        final bool fast = event.durationMs < prescribed * FcTuning.tempoFastRatio;
        if (fast) {
          _fastReps++;
        } else {
          _fastReps = 0;
  ...
  void _updateActiveSide(FormProfileVariant variant, MetricSet metrics, int tMs) {
    if ((_counter?.inCycle ?? false) || variant.sideSelection != SideSelection.activeSide) return;
    ...
    if (next != _activeSide) { _counter?.abortCycle(); _activeSide = next; }
  }
  ...
  String _liveTextAr() {
    if (_reason != CannotAssessReason.none) {
      return _report?.primaryIssue != null ? _report!.cueAr : cannotAssessReasonAr(_reason);
    }
    if (_counter != null && _counter!.phase == RepPhase.unknown) {
      final label = _counter!.spec.topLabelAr;
      return label.isEmpty ? 'ارجع لوضع البداية عشان أتابع العدة' : 'ابدأ من: $label';
    }
