...
class _FormCoachScreenState extends State<FormCoachScreen>
  ...
  Future<void> _enableWakelock() async {
    try {
      ...
      if (!_wakelockWasOn) {
        ...
        if (!mounted) await WakelockPlus.disable();
      }
    } catch (_) {
      // Best effort only.
    }
        ...
        builder: (BuildContext context, Widget? _) {
          ...
          return Stack(
            ...
            children: <Widget>[
              _preview(),
              RepaintBoundary(
                  child: ValueListenableBuilder<FormCoachSnapshot?>(
                valueListenable: _controller.poseFrames,
                builder: (context, pose, _) =>
                    pose == null ? const SizedBox.shrink() : _overlay(pose),
              )),
              _gradient(),
              SafeArea(
                child: LayoutBuilder(
        ...
        child: Center(
          child: SizedBox(
            ...
            child: CircularProgressIndicator(
                strokeWidth: 2.4, color: Colors.white24),
          ),
        ),
      );
    }
  ...
  Widget _overlay(FormCoachSnapshot snapshot) {
    ...
    return PoseOverlay(
      ...
      imageAspect: snapshot.pose?.imageAspect ?? aspect,
    ...
    return Padding(
      ...
      child: Row(
        children: <Widget>[
                ...
                children: <Widget>[
                  ...
                  Text(
                    <String>[
                      ...
                      if (widget.request.isFallback)
                        'متابعة احتياطية: عدات ومدى فقط',
                    ].join('  ·  '),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
          ...
          if (_controller.canSwitchCamera)
            IconButton(
          ...
          if (_controller.recordingAvailable)
            IconButton(
              onPressed: _controller.isTransitioning
                  ? null
                  : () => unawaited(_controller.toggleRecording()),
              icon: Icon(
                _controller.recordingInProgress
                    ? Icons.fiber_manual_record_rounded
                    : Icons.videocam_outlined,
                color: _controller.recordingInProgress
                    ? Colors.redAccent
                    : (_controller.recordingRequested
                        ? Colors.amber
                        : Colors.white),
              ),
              tooltip: _controller.recordingRequested
                  ? 'إلغاء تسجيل المجموعة'
                  : 'تسجيل المجموعة وحفظها على الموبايل',
            ),
          PopupMenuButton<FormAudioMode>(
            color: Colors.black87,
            initialValue: _controller.audioMode,
            onSelected: (FormAudioMode mode) =>
                unawaited(_controller.setAudioMode(mode)),
            itemBuilder: (BuildContext context) => FormAudioMode.values
                .map((FormAudioMode mode) => PopupMenuItem<FormAudioMode>(
                      value: mode,
                      child: Text(
                        formAudioModeLabelAr(mode),
                        style: const TextStyle(color: Colors.white),
                      ),
                    ))
                .toList(),
            icon: Icon(_audioIcon(_controller.audioMode), color: Colors.white),
            tooltip: 'وضع الصوت',
          ),
        ],
      ),
  ...
  IconData _audioIcon(FormAudioMode mode) {
    switch (mode) {
      ...
      case FormCoachStage.readyCheck:
        return Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ReadinessPanel(
                report: snapshot.readiness ?? ReadinessReport.unknownReport,
  ...
  Widget _liveBody(FormCoachSnapshot snapshot) {
    ...
    final String counterHint = durationMode
        ? (snapshot.targetSeconds == null
            ? 'ثانية'
            : 'من ${snapshot.targetSeconds} ثانية')
        : (snapshot.targetReps == null
            ? 'عدة'
            : 'من ${snapshot.targetReps} عدة');
      ...
      child: Column(
        children: <Widget>[
          Align(
            ...
            child: snapshot.verdict == FormVerdict.cannotAssess
                ? const SizedBox.shrink()
                : CueBanner(
                    cue: snapshot.cue ?? snapshot.lastCue, showDetail: false),
          ),
          const Spacer(),
          if (snapshot.verdict != FormVerdict.clearError)
            Container(
          ...
          Text(
            'محاولات ${snapshot.attempts}  ·  متابعة فقط ${snapshot.trackedReps}  ·  ناقصة ${snapshot.incomplete}  ·  غير قابلة للتقييم ${snapshot.cannotAssessAttempts}',
            textAlign: TextAlign.center,
            style: TextStyle(
          ...
          Container(
            ...
            child: Column(
              children: <Widget>[
                ...
                if (_controller.recordingMessage != null) ...<Widget>[
                  const SizedBox(height: 6),
                  Text(
                    _controller.recordingMessage!,
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: AppColors.wo, fontSize: 12),
                  ),
                ],
                const SizedBox(height: 4),
                Text(
                  'محاولات ${snapshot.attempts}  ·  متابعة فقط ${snapshot.trackedReps}  ·  اجتازت الفحوص المتاحة ${snapshot.valid}  ·  بها ملاحظات ${snapshot.incorrect}  ·  ناقصة ${snapshot.incomplete}  ·  غير قابلة للتقييم ${snapshot.cannotAssessAttempts}',
                  textAlign: TextAlign.center,
                  style: const TextStyle(
                      color: AppColors.textSoft, fontSize: 11.5),
                ),
                if (snapshot.tooFastReps > 0)
                  Text('أسرع من التيمبو المطلوب: ${snapshot.tooFastReps} عدة'
                      '${snapshot.tempoImproved ? ' — تحسّن في عدتين لاحقتين' : ''}',
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: AppColors.textSoft, fontSize: 12)),
                for (final correction in snapshot.corrections)
                  Text(
                      '${correction.cueAr}: ${correction.errorReps} عدة'
                      '${correction.improved ? ' — تحسنت في عدتين لاحقتين' : ''}',
                      textAlign: TextAlign.center,
                      style: const TextStyle(
...
class _ErrorView extends StatelessWidget {
  const _ErrorView(
      {required this.message, required this.failure, required this.onRetry});
