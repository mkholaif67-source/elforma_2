      ...
      body: AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? _) {
          ...
          return Stack(
            ...
            children: <Widget>[
              _preview(),
              RepaintBoundary(child: ValueListenableBuilder<FormCoachSnapshot?>(
                valueListenable: _controller.poseFrames,
                builder: (context, pose, _) => pose == null
                    ? const SizedBox.shrink() : _overlay(pose),
              )),
              _gradient(),
              SafeArea(
      ...
      child: Row(
        children: <Widget>[
                ...
                children: <Widget>[
                  ...
                  Text(
                    <String>[
                      ...
                      if (widget.request.isFallback) 'متابعة احتياطية: عدات ومدى فقط',
                    ].join('  ·  '),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
          ...
          if (_controller.canSwitchCamera)
            IconButton(
              onPressed: _controller.isTransitioning ? null : () => unawaited(_controller.toggleCamera()),
              icon: const Icon(Icons.flip_camera_ios_rounded, color: Colors.white),
              tooltip: 'تبديل الكاميرا',
            ),
          if (_controller.recordingAvailable)
            IconButton(
              onPressed: _controller.isTransitioning ? null : () => unawaited(_controller.toggleRecording()),
              icon: Icon(
                ...
                color: _controller.recordingInProgress
                    ? Colors.redAccent
                    : (_controller.recordingRequested ? Colors.amber : Colors.white),
              ),
        ...
        children: <Widget>[
          Align(
            ...
            child: snapshot.verdict == FormVerdict.cannotAssess
                ...
                : CueBanner(cue: snapshot.cue ?? snapshot.lastCue, showDetail: false),
          ),
          const Spacer(),
          ...
          Text(
            'محاولات ${snapshot.attempts}  ·  ناقصة ${snapshot.incomplete}  ·  غير قابلة للتقييم ${snapshot.cannotAssessAttempts}',
            textAlign: TextAlign.center,
          ...
          Container(
            ...
            child: Column(
              children: <Widget>[
                ...
                Text(
                  'محاولات ${snapshot.attempts}  ·  صحيحة ${snapshot.valid}  ·  خاطئة ${snapshot.incorrect}  ·  ناقصة ${snapshot.incomplete}  ·  غير قابلة للتقييم ${snapshot.cannotAssessAttempts}',
                  textAlign: TextAlign.center,
...
class _ErrorView extends StatelessWidget {
  const _ErrorView({required this.message, required this.failure, required this.onRetry});
