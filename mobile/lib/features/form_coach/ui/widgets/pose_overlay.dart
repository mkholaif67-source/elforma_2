...
class _PosePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint jointPaint = Paint()..color = Colors.white.withValues(alpha: 0.92);
    final Paint weakPaint = Paint()..color = Colors.white.withValues(alpha: 0.35);

    for (final FcLandmark landmark in FcLandmark.values) {
      final FcPoint? point = sample.points[landmark];
      if (point == null) continue;
      final double x = mirror ? 1 - point.x : point.x;
      final Offset center = Offset(dx + x * drawWidth, dy + point.y * drawHeight);
      final bool strong = point.visibility >= minVisibility;
      canvas.drawCircle(center, strong ? 4 : 2.6, strong ? jointPaint : weakPaint);

      if (showConfidence && strong) {
