...
class DebugPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ReadinessReport readiness =
        snapshot.readiness ?? ReadinessReport.unknownReport;
    return Container(
          ...
          child: Directionality(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if (performanceText.isNotEmpty) Text(performanceText),
                Text(
                ...
                Text(
                  ...
                  'view=${readiness.detectedView.name} '
                  'torso=${readiness.torsoFraction?.toStringAsFixed(2) ?? '-'} '
                  'spread=${readiness.spreadRatio?.toStringAsFixed(2) ?? '-'} '
                  'vis=${readiness.meanVisibility.toStringAsFixed(2)}',
                ),
                if (readiness.issues.isNotEmpty)
                  Text(
                      'issues=${readiness.issues.map((Object i) => (i as Enum).name).join(',')}'),
                ...
                Text(
                    'landmarks=${snapshot.pose?.points.length ?? 0}/${FcLandmark.values.length}'),
              ],
            ),
          ),
        ),
  ...
  List<Widget> _metricLines() {
    final Map<String, MetricReading> metrics = snapshot.metrics;
    if (metrics.isEmpty) return <Widget>[const Text('(none)')];
    final List<Widget> lines = <Widget>[
      Text('metrics=${metrics.length}'),
    ];
    final List<String> keys = metrics.keys.toList()..sort();
  ...
  List<Widget> _ruleLines() {
    ...
    return snapshot.ruleStates.map((FormRuleState state) {
      ...
      return Text(
        ...
        '${state.blockedReason.isEmpty ? '' : ' blocked=${state.blockedReason}'}',
      );
    }).toList();
  }
