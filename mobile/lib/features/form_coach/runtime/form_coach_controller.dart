...
class FormCoachController extends ChangeNotifier {
  ...
  String get performanceText => _pipeline == null
      ? ''
      : 'processing p50=${_pipeline!.processingP50Ms.toStringAsFixed(0)}ms '
          'p95=${_pipeline!.processingP95Ms.toStringAsFixed(0)}ms '
          'paced=${_pipeline!.pacedDrops} busy=${_pipeline!.busyDrops} '
          'interval=${_pipeline!.frameIntervalMs}ms';
  ...
  Future<void> _start() async {
    ...
    try {
      _subscription =
          pipeline.frames.listen(_onFrame, onError: _onPipelineError);
  ...
  void _onPipelineError(Object error) {
    ...
    poseFrames.value = null;
    _failure = CameraFailure.unavailable;
    _errorTextAr = error is FormCoachCameraException
        ? error.messageAr
  ...
  void _onFrame(PoseFrame frame) {
    ...
    final pose = raw == null
        ? null
        : PoseSample(
            timestampMs: tMs, points: raw.points, imageAspect: raw.imageAspect);
    ...
    if (snapshot.isFinished) {
      _pipeline?.stopAcceptingFrames();
      unawaited(_finishRecordingAndPause());
    }
    ...
    final important = previous == null ||
        previous.stage != snapshot.stage ||
        previous.attempts != snapshot.attempts ||
        previous.reps != snapshot.reps ||
  ...
  Future<void> _handleLifecycle(AppLifecycleState state) async {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      try {
        ...
        _failure = error is FormCoachCameraException
            ? error.failure
            : CameraFailure.unavailable;
        _errorTextAr = error is FormCoachCameraException
            ? error.messageAr
            : 'تعذر تشغيل الكاميرا — حاول تاني';
        notifyListeners();
      }
      return;
    }
  ...
  Future<void> _finishRecordingAndPause() {
    ...
    return _serial(_finishRecording)
        .whenComplete(() => _recordFinishing = false);
  }
  ...
  Future<void> _disposeRecordingAndPipeline(
      CameraPosePipeline? pipeline) async {
