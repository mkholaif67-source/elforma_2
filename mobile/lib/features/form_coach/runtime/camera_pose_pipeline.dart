...
class CameraPosePipeline {
  ...
  final StreamController<PoseFrame> _frames =
  ...
  final AnalysisPacing _pacing = AnalysisPacing(
      requestedFps: const int.fromEnvironment('FORM_COACH_ANALYSIS_FPS',
          defaultValue: 15));
  int get _minIntervalMs => _pacing.intervalMs;
  int _pacedDrops = 0;
  int _busyDrops = 0;
  double _conversionMs = 0;
  int _dropped = 0;
  double _fps = 0;
  ...
  bool get canSwitchCamera => _cameras.any(
      (CameraDescription camera) => camera.lensDirection != _lensDirection);
  int get droppedFrames => _dropped;
  int get pacedDrops => _pacedDrops;
  int get busyDrops => _busyDrops;
  double get processingP50Ms => _pacing.p50Ms;
  double get processingP95Ms => _pacing.p95Ms;
  double get fps => _fps;
  ...
  Future<void> _startCamera() async {
    ...
    final CameraController controller = _cameraFactory?.call(description) ??
        CameraController(
          description,
    ...
    _staleTimer = Timer.periodic(const Duration(milliseconds: 250), (_) {
      ...
      if (_streaming &&
          !_disposed &&
          _clock.elapsedMilliseconds - _lastResultMs > 500) {
  ...
  Future<void> switchCamera() async {
    ...
    if (_disposed) return;
    final CameraController next = _cameraFactory?.call(description) ??
        CameraController(
          description,
          ResolutionPreset.high,
          enableAudio: false,
  ...
  Future<void Function(CameraImage)> prepareRecording() async {
    ...
    _streaming = true;
    return (image) {
      if (generation == _streamGeneration && identical(controller, _controller))
        _onFrame(image);
    };
  }
    ...
    try {
      ...
      await controller.startImageStream((image) {
        if (generation == _streamGeneration &&
            identical(controller, _controller)) _onFrame(image);
      });
    } on CameraException catch (error) {
      _streaming = false;
      throw _mapException(error);
  ...
  void _onFrame(CameraImage image) {
    ...
    _conversionMs = conversionWatch.elapsedMicroseconds / 1000.0;
    if (prepared == null) {
      _dropped++;
  ...
  Future<void> _analyse(_PreparedFrame frame, int generation) async {
    ...
    try {
      ...
      _adaptPacing(_inferenceMs + _conversionMs);
  ...
  void _failStream() {
    ...
    if (!_disposed && !_frames.isClosed) {
      _frames.addError(const FormCoachCameraException(
          CameraFailure.unavailable, 'التتبع وقف — جرّب فتح الكاميرا تاني'));
    }
  }
  ...
  void _adaptPacing(double processingMs) {
    _pacing.observe(processingMs, recording: _recordingQuality);
  }
  ...
  InputImage? _toInputImage(
    ...
    final InputImageFormat? format = InputImageFormatValue.fromRawValue(
        image.format.raw is int ? image.format.raw as int : 0);
    ...
    if (Platform.isAndroid && image.planes.length == 3) {
      ...
      return null;
        ...
        bytes.length != image.width * image.height * 3 ~/ 2) {
      return null;
    }
  ...
  double _meanLuma(CameraImage image) {
    ...
    return sampleFrameLuma(plane.bytes,
            width: image.width,
            height: image.height,
            rowStride: plane.bytesPerRow,
            bgra: !Platform.isAndroid) ??
        0;
  }
