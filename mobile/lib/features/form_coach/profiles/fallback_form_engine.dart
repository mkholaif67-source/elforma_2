...
class FormFallbackEngine {
  ...
  static FallbackResolution resolve({
    ...
    if (match.family == FallbackMovementFamily.fly ||
        (match.family == FallbackMovementFamily.curl &&
            bodyRegion != 'upper body') ||
        (match.family == FallbackMovementFamily.raise &&
            bodyRegion != 'upper body')) {
      return _cannot('الحركة تحتاج ملفًا متخصصًا؛ زاوية المفصل وحدها لا تكفي');
    }
    ...
    final double confidence = (match.score +
            (bodyRegion.isNotEmpty ? 0.12 : 0) +
    ...
    final FallbackMechanics mechanics = FallbackMechanics(
      family: match.family,
      bodyRegion:
          bodyRegion.isEmpty ? _defaultRegion(match.family) : bodyRegion,
  ...
  static _FamilyMatch? _family(String text) {
    ...
    for (final _FamilyMatch candidate in candidates) {
      ...
      final bool ambiguous =
          candidate.family == FallbackMovementFamily.extension &&
              !_hasAny(text, <String>[
                'tricep',
                'triceps',
                'leg extension',
  ...
  static FormProfile _profileFor({
    ...
    final List<FcJoint> joints = hinge
        ? const <FcJoint>[FcJoint.shoulder, FcJoint.hip, FcJoint.knee]
    ...
    final FormProfileVariant variant = FormProfileVariant(
      ...
      readiness: ReadinessSpec(
        requiredJoints: raise
            ? const [FcJoint.shoulder, FcJoint.elbow, FcJoint.hip]
            : joints,
      ...
      repCycle: RepCycleSpec(
        ...
        partialAmplitude:
            mechanics.family == FallbackMovementFamily.fly ? 16 : 24,
    ...
    return FormProfile(
      ...
      supportLevel: FormSupportLevel.trackingOnly,
      setupHintAr:
          'ثبّت الهاتف من ${mechanics.cameraView == PreferredView.side ? 'الجانب' : 'الأمام'} وأظهر المفاصل المطلوبة',
      detectableAr: const <String>[
        'العدات التقريبية',
        'مرحلة الحركة',
        'المدى الظاهر'
      ],
