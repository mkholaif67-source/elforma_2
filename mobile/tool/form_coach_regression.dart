...
void main() {
    ...
    for(final v in p.variants) {
      ...
      test('${v.id}: no motion / threshold jitter',(){
        ...
        for(var t=0;t<=4000;t+=50) {
          c.update(tMs:t,driver:spec.topValue+(spec.descending?-1:1)*(spec.enterMargin+(t%100==0?1:-1)),usable:true);
        }
  ...
  test('end-to-end pose -> metrics -> completion -> counters',(){
    ...
    check(s.reps==1&&s.attempts==1&&s.valid==1,'${s.reps}/${s.attempts}/${s.valid}');
  });
  test('end-to-end landmark loss -> cannot assess, no correction',(){
    final e=engine();warm(e);final s=move(e,3000,loss:true);
    check(s.reps==0&&s.attempts==1&&s.cannotAssessAttempts==1&&s.incorrect==0,'loss misclassified');
  });
