...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/admin' || pathname === '/admin/' || pathname === '/admin.html'
      ...
      if (!cfg.isAdminEmail(adminUser.email)) {
        // [FIX-ADMIN-8] قبل كده كانت رسالة نص خام وحشة في وش المستخدم.
        // دلوقتي صفحة منسقة بهوية التطبيق + تحويل تلقائي للموقع بعد 5 ثواني.
        const notAdminHtml = [
          ...
          'background:rgba(16,185,129,.12);border:1px solid rgba(16,185,129,.35);font-size:28px}',
          ...
          '<div class="lock">\uD83D\uDD12</div>',
          '<h1>لوحة التحكم للأدمن بس</h1>',
          '<p>انت داخل بحساب <span class="em">' + String(adminUser.email || '').replace(/[<>&"]/g, '') + '</span>',
          ' ومش من حسابات الأدمن.</p>',
          '<p>روح للموقع وكمل تمرينك عادي، أو سجّل دخول بحساب الأدمن.</p>',
          '<div class="row"><a class="btn" href="/">الرجوع للموقع</a>',
          '<a class="btn gh" href="/login.html?next=%2Fadmin.html">دخول بحساب الأدمن</a></div>',
          '<div class="hint">بنحولك للموقع تلقائيًا خلال 5 ثواني…</div>',
          '</div></body></html>'
        ].join('');
