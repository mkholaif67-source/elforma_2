...
for (const wf of workflows) {
  ...
  for (const block of jobBlocks) {
    ...
    // Android is distributed with the app and verified before building.
    if (block.includes('tools/verify_android_project.py') &&
        fs.existsSync(path.join(MOBILE, 'android/app/src/main/AndroidManifest.xml'))) {
    ...
    for (const run of iconRuns) {
      ...
      for (const plat of ['android', 'ios']) {
        if (want[plat] === true) {
          check(label + ': targets ' + plat + ' and the platform is available: ' + plat,
