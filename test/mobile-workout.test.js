...
(async () => {
  ...
  const cairoNow = new Date(Date.now() + 3 * 3600000);
  const todaySaturdayIndex = (cairoNow.getUTCDay() + 1) % 7;
  const selectedDays = Array.from({length:7}, (_, i) => i)
    .filter((day) => day !== todaySaturdayIndex).slice(0, 2);
  const plan = await request('PUT', '/api/mobile/workout-plan', {
    key:'upper', selectedDays, plan:{key:'upper', name:'Upper', plan:[
      {key:'upper_1', name:'Upper 1', exercises:[{name:'Barbell Bench Press'}]},
      {key:'upper_2', name:'Upper 2', exercises:[{name:'Barbell Row'}]},
      {key:'rest', name:'Rest', isRest:true, exercises:[]},
      {key:'rest_2', name:'Rest 2', isRest:true, exercises:[]},
      {key:'rest_3', name:'Rest 3', isRest:true, exercises:[]},
      {key:'rest_4', name:'Rest 4', isRest:true, exercises:[]},
      {key:'rest_5', name:'Rest 5', isRest:true, exercises:[]},
    ]},
  }, cookie);
  if (plan.status !== 200) throw new Error('plan activation failed');
  if (plan.json.workoutPlan.data._firstWorkoutExceptionActive !== true ||
      plan.json.workoutPlan.data._firstWorkoutDayKey !== 'upper_1') {
    throw new Error('new-user first-workout marker was not persisted');
  }
  const shownDays = plan.json.workoutPlan.data.plan;
  const shownIndex = plan.json.workoutPlan.data._scheduleDayIndex;
  if (plan.json.workoutPlan.data._scheduleIsRest !== false ||
      !shownDays[shownIndex] || shownDays[shownIndex].key !== 'upper_1') {
    throw new Error('new user did not receive workout one immediately outside selected weekdays');
  }
  ...
  const persistedAfterFinish = JSON.parse(db.activeWorkoutPlan(signup.json.user.id).plan_json);
  if (persistedAfterFinish._firstWorkoutCompleted !== true) {
    throw new Error('first-workout completion marker was not persisted');
  }
  if (persistedAfterFinish._completedSequenceCount !== 1 ||
      persistedAfterFinish._nextWorkoutIndex !== 1) {
    throw new Error('completion-driven workout cursor was not persisted');
  }
  ...
  if (!Array.isArray(workoutHistory.json.trainingWeeks) || workoutHistory.json.trainingWeeks.length !== 1) {
    throw new Error('training week drill-down failed');
  }
  const loggedWeek = workoutHistory.json.trainingWeeks[0];
  if (loggedWeek.sessionCount !== 1 || loggedWeek.sessions.length !== 1 || loggedWeek.sessions[0].exercises.length !== 1) {
    throw new Error('completed training days were not grouped safely');
  }
  if (loggedWeek.sessions[0].exercises[0].sets[0].rir !== 2) {
    throw new Error('set RIR was not exposed in the trainee history');
  }
  if (!loggedWeek.insights.length || loggedWeek.comparison !== null) {
    throw new Error('first-week baseline analysis failed');
  }
