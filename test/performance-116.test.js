...
test('essential local services remain present', () => {
  for (const token of ['ExerciseVideoCatalog.I.load', 'SmartCoachStore.I.init', 'ConnectivityService.I.init', 'NotificationService.I.requestPermission', 'BackgroundPoll.setup']) assert(main.includes(token), token);
});
...
test('splash runs independent network checks concurrently', () => {
  ...
  // [FIX-BOOT-WAKE] النداءان اتوسعوا بباراميتر مهلة اختياري للإقلاع البارد —
  // العقد هو استمرار التزامن الفحصين في شاشة البداية، مش النص الحرفي للاستدعاء.
  assert(/UpdateGate\.check\(/.test(splash));
  assert(/Api\.I\.me\(/.test(splash));
...
test('subscription refresh still forces fresh state after changes', () => {
  // Same freshness contract, now through the lighter subscription endpoint.
  assert(sub.includes('await Api.I.mobileSubscription()'));
  assert(api.includes("_lightSnapshot('/api/mobile/subscription', force: true)"));
  assert(sub.includes('Api.I.invalidateBootstrap()'));
  assert(sub.includes('session != Api.I.sessionRevision'));
  assert(!sub.includes('await Api.I.mobileBootstrap('));
...
test('release versions are synchronized', () => {
  ...
  const match=read('mobile/pubspec.yaml').match(/^version: (.+)\+(\d+)$/m);
  assert(match);assert.strictEqual(match[1],version);
  assert(api.includes('const int kAppBuild = '+match[2]));
  assert(api.includes("const String kAppVersionName = '"+version+"'"));
});
