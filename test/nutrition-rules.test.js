...
t('الفطار فيه كل العناصر اللي اتقالت', function () {
  const want = ['فول مدمس','جبنة قريش','جبنة رومي','جبنة رودس طبيعي','جبنة بيضاء','بيض مقلي','بيض مسلوق','بطاطس','عيش بلدي','عيش أسمر','عيش أبيض','رايس كيك','زبادي'];
...
t('الغدا: فراخ/لحوم/أسماك/خضار مطبوخ وشوربة/كارب', function () {
  ...
  assert.strictEqual(DB.LUNCH_PROTEIN_MEAT.length, 5);
  assert.ok(DB.LUNCH_PROTEIN_CHICKEN.some(function (x) { return x.nameAr === 'كبد وقوانص فراخ'; }));
  assert.ok(DB.LUNCH_PROTEIN_MEAT.some(function (x) { return /كبدة إسكندراني/.test(x.nameAr); }));
  assert.strictEqual(DB.LUNCH_FISH.length, 5);
  assert.ok(DB.LUNCH_COOKED_VEG_SOUP.length >= 11);
  ['شوربة مرقة دجاج','شوربة مرقة لحمة','شوربة عدس'].forEach(function (name) { assert.ok(DB.LUNCH_COOKED_VEG_SOUP.some(function (x) { return x.nameAr === name; }), 'missing: ' + name); });
  assert.ok(DB.CARBS.some(function (x) { return x.nameAr === 'مكرونة'; }));
  assert.ok(DB.CARBS.some(function (x) { return /بني/.test(x.nameAr); }));
});

t('التونة عليها سقف 300جم يوميًا', function () {
  const tuna = DB.LUNCH_FISH.find(function (x) { return x.nameAr === 'تونة'; });
  assert.strictEqual(tuna.maxGramsPerDay, 300);
  assert.strictEqual(DB.DAILY_LIMITS.tunaMaxGrams, 300);
...
t('المكمّلات: زيت/زبدة 2..10جم، زيتون 40..50جم حد 5/أسبوع، طحينة حد 3/أسبوع', function () {
  ...
  assert.strictEqual(olives.gramsMin, 40); assert.strictEqual(olives.gramsMax, 50); assert.strictEqual(olives.maxPerDay, 1); assert.strictEqual(olives.maxPerWeek, 5);
...
t('قبل التمرين: قهوة سادة + مصدر طاقة واحد', function () {
  assert.strictEqual(DB.PRE_WORKOUT_BASE.nameAr, 'قهوة سادة');
  assert.strictEqual(DB.PRE_WORKOUT_MIN_ADDONS, 1);
  assert.strictEqual(DB.PRE_WORKOUT_MAX_ADDONS, 1);
  const ids = DB.PRE_WORKOUT_ADDONS.map(function (x) { return x.id; });
  ['banana','apple','dark_choco','dates','sweet_potato'].forEach(function (i) { assert.ok(ids.indexOf(i) >= 0, 'missing preworkout: ' + i); });
...
t('سمك + خضار مطبوخ/شوربة/بطاطس = مخالفة', function () {
  ...
  assert.ok(rules.validate(plan).some(function (x) { return x.code === 'no_cookedveg_with_banned_protein'; }));
});
...
t('البطاطا لا تظهر في وجبة رئيسية', function () {
  const plan = { meals: [meal('lunch', 'الغدا', [mkFood('بطاطا مشوية', 'carb', 150)])] };
  assert.ok(rules.validate(plan).some(function (x) { return x.code === 'sweet_potato_slot'; }));
});

t('العيش لا يجتمع مع الرز والمكرونة لا تجتمع مع الرز', function () {
  const plan = { meals: [meal('lunch', 'الغدا', [mkFood('عيش بلدي', 'carb', 60), mkFood('رز أبيض', 'carb', 150), mkFood('مكرونة', 'carb', 120)])] };
  const codes = rules.validate(plan).map(function (x) { return x.code; });
  assert.ok(codes.indexOf('bread_starch_conflict') >= 0);
  assert.ok(codes.indexOf('rice_pasta_conflict') >= 0);
});

t('الفاكهة خارج القائمة الموسمية لا تظهر', function () {
  const plan = { meals: [meal('snack', 'سناك', [mkFood('جريب فروت', 'fruit', 150)])] };
  assert.ok(rules.validate(plan).some(function (x) { return x.code === 'fruit_not_allowed'; }));
});

t('تكرار الصنف داخل الوجبة مخالفة', function () {
  const plan = { meals: [meal('lunch', 'الغدا', [mkFood('صدر فراخ', 'protein', 100), mkFood('صدر فراخ', 'protein', 50)])] };
  assert.ok(rules.validate(plan).some(function (x) { return x.code === 'duplicate_food'; }));
});

t('تونة > 300جم = مخالفة', function () {
  const plan = { meals: [meal('lunch', 'الغدا', [mkFood('تونة', 'fish', 350)])] };
...
t('enforce متكرّر (idempotent): تاني مرة مايعملش تغيير', function () {
  ...
  assert.ok(!rules.validate(first.plan).some(function(x){return ['no_rice_with_tuna','one_starch_per_meal','salad_exactly_three','oats_forbidden'].indexOf(x.code)>=0;}));
});

t('enforce بيقلّل التونة لـ 300جم', function () {
  const plan = { meals: [meal('lunch', 'الغدا', [mkFood('تونة', 'fish', 450)])] };
  const out = rules.enforce(plan);
  const tuna = out.plan.meals[0].foods.find(function (f) { return /تونة/.test(f.food.nameAr); });
  assert.strictEqual(tuna.grams, 300);
});
