...
// GitHub-hosted runners cannot create arbitrary directories under /data.
// Always use the operating system's writable temporary directory.
const DATA = fs.mkdtempSync(path.join(os.tmpdir(), 'eft-persist-'));
process.on('exit', () => { try { fs.rmSync(DATA, { recursive: true, force: true }); } catch (_) {} });
