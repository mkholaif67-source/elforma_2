...
const passwordBlock = auth.slice(
  ...
  auth.indexOf('if (!_login && _showConfirmation)'),
);
...
assert(!passwordBlock.includes('maxLines:'), 'Password errors flow without a line cap');
assert(auth.includes("label: _login ? 'إنشاء حساب' : 'تسجيل الدخول'"), 'Auth mode switch needs a functional semantics label');
...
assert(/builder:\s*\(context, constraints\)\s*=>\s*_buildReflowAuth\(constraints\)/.test(auth), 'Every auth viewport must use native reflow');
assert(!auth.includes('_buildAuthCanvas'), 'Do not scale the complete input canvas');
...
const authErrorBlock = auth.slice(
  auth.indexOf('Widget _inlineErrorBanner()'),
  auth.indexOf('Widget _authField('),
);
assert(!authErrorBlock.includes('TextOverflow.ellipsis'), 'Auth server errors must remain fully readable');
assert(authErrorBlock.includes('child: Text(') && !authErrorBlock.includes('maxLines:'), 'Auth server errors must wrap in natural flow without a line cap');
