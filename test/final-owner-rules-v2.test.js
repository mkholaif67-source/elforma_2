...
for(const mealCount of [2,3,4,5])for(const training of [false,true])for(let seed=0;seed<7;seed++){
 ...
 for(const m of plan.meals||[]){const ns=names(m),all=ns.join(' '),raws=ns.filter(n=>raw.test(n)),carbs=ns.filter(n=>starch.test(n));
  ...
  if(/breakfast|lunch|dinner/.test(String(m.slotKey||''))&&!(m.foods||[]).some(f=>rules._pred.isCookedVeg(f)||rules._pred.isSoup(f)))assert.equal(raws.length,3,'السلطة ليست 3 عناصر: '+all);
  ...
  if(/سمك|تونة|كفتة|كبدة|كبد وقوانص|قوانص/.test(all))assert.ok(!(m.foods||[]).some(f=>rules._pred.isCookedVeg(f)||rules._pred.isSoup(f)),'خضار مطبوخ مع بروتين ممنوع');
