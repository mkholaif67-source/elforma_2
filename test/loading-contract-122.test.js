...
check('mutation invalidates memory but preserves and patches the account-scoped durable plan',()=>{assert(api.includes('_epoch++'));assert(api.includes('_patchCachedWorkoutPlan'));assert(api.includes("data['workoutPlan'] = workoutPlan"));assert(api.includes('_accountScope ?? _accountId'));assert(api.includes('requestEpoch != _epoch'));assert(api.includes('sessionEpoch != _sessionEpoch'));});
check('cached responses are deep-copied, account-scoped, age-bounded and not used as current online reads',()=>{assert(api.includes('jsonDecode(jsonEncode(r.data))'));assert(!api.includes("substring(0,10)}:${path"));assert(api.includes('Duration(days: 7)'));assert(api.includes("data['stale']"));assert(!api.includes('unawaited(_refreshBootstrapNetwork())'));});
...
check('new-user completion repaints the gate even when profile values are unchanged',()=>{
 ...
 assert.match(callback,/setState\(\(\)\s*\{\}\);[\s\S]*if\s*\(signature\s*==\s*_lastProfileSignature\)\s*return;/);
