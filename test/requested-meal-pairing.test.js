...
for(let day=1;day<=14;day++){
  const ctx=bridge.buildEngineContext(profile,{});ctx.inputs['inp-week']=day<=7?1:2;ctx.inputs.dayOfCycle=(day-1)%7;
  const plan=host.computeMealPlan(ctx.profile,ctx.inputs).plan;
  let cookedToday=false,yogurtToday=false;
  for(const meal of plan.meals||[]){
    const foods=meal.foods||[], names=foods.map(f=>String(f.food&&f.food.nameAr||'').replace(/\s+/g,' ')), all=names.join(' ');
    for(const f of foods) assert.ok(rules.soupAllowedWithFoods(f,foods),'شوربة غير متوافقة في الوجبة النهائية: '+all);
    cookedToday ||= foods.some(f=>db.LUNCH_COOKED_VEG_SOUP.some(v=>v.id===f.food.id)); yogurtToday ||= /زبادي طبيعي/.test(all);
    for(const f of foods){const n=String(f.food&&f.food.nameAr||'').replace(/\s+/g,' ');if(/زيت زيتون|زبدة/.test(n)){assert.strictEqual(f._isAddon,true,'دهن مضاف ظهر مستقل');assert.ok(+f.grams>=2&&+f.grams<=+(f._addonMaxGrams||10));}}
    ...
    if(/lunch|dinner/.test(String(meal.slotKey||''))){
      ...
      assert.ok(/طماطم|خيار|فلفل|خس|جرجير/.test(all)||foods.some(f=>db.LUNCH_COOKED_VEG_SOUP.some(v=>v.id===f.food.id)),'وجبة رئيسية بلا خضار');
    }
  }
