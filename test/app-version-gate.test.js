...
ok('the client exposes appVersion() and an UpdateGate', () => {
  // [FIX-BOOT-WAKE] appVersion اتوسع بباراميتر مهلة اختياري للإقلاع البارد — العقد هو وجود الطريقة نفسها.
...
ok('splash checks the gate BEFORE routing into the app', () => {
  ...
  // [FIX-BOOT-WAKE] النداءان بقوا ياخدوا باراميتر مهلة اختياري — بنطابق بداية الاستدعاء بدل النص الحرفي الكامل.
  const gateAt = splash.slice(bootStart).search(/UpdateGate\.check\(/);
  const meAt = splash.slice(bootStart).search(/Api\.I\.me\(/);
