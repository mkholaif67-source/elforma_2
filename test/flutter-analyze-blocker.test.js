...
const retryAwait = /if\s*\(retryStale\)\s*return await _send\(/g;
assert(!api.includes('if (retryStale) return _send(') && (api.match(retryAwait) || []).length === 2, 'retry must await the request in both branches');
